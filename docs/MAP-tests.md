...
## research/a2_storage/test_refresh.py · 443 строк
...
- L126 `test_liquidity_refresh_is_part_of_the_chain()` — Свежие бары без свежей ликвидности — тот же мёртвый контур.
- L177 `make_zip(path, rows)` — Суточный/месячный архив в формате Binance.
- L190 `build_store(tmp, interval='1m', extra_day=False, gap_day=False)` — Сырьё месяца и сборка партиции НАСТОЯЩИМ build.py.
- L220 `test_readiness_accounts_for_new_files()` — Дозакачанный день ОБЯЗАН вызвать пересборку партиции.
- L267 `test_storage_edge_reads_a_real_partition()` — Край читается с НАСТОЯЩЕЙ партиции и без сторонних модулей.
- L293 `test_edge_is_the_end_of_CONTINUOUS_coverage()` — Край — конец НЕПРЕРЫВНОГО покрытия, а не максимальная метка.
- L317 `test_days_pilot_takes_the_FIRST_days()` — Пилот берёт дни ОТ КРАЯ, а не с конца: иначе он сам делает дыру.
- L325 `test_watchdog_daily_window()` — Секция сторожа гоняется НАСТОЯЩИМ блоком скрипта с заглушками.
- L409 `test_verdict_says_when_edge_did_not_move()` — Неподвижный край при непустой докачке — отказ, и он называется.
- L418 `main()`
...
## research/b1_book/headless_check.js · 6445 строк
...
## research/b1_book/test_book.py · 8333 строк
...
- L7962 `test_dca_chart_carries_the_liquidation_of_the_book()` — У ступеней позиции есть цена ликвидации — ядром, а не копией.
- L8010 `test_dca_chart_reads_the_journal_of_its_own_family()` — График берёт позиции из журнала СВОЕГО семейства, а не длинного.
- L8117 `test_dca_list_counts_rules_of_the_family_not_of_the_project()` — Список сделок книги живёт по версии правил СВОЕГО семейства.
- L8192 `main()`
...
- L23 `_leg(sym, at, arm, side='long')`
- L28 `test_agreement_needs_both_arms_and_the_same_side()`
...
- L126 `test_memory_guard_stops_the_run_itself()`
- L142 `test_unusable_cache_says_why(monkey=None)`
...
## research/dca_paper/test_pair.py · 520 строк
...
- L25 `H = 3600.0`
- L26 `T0 = TP.T0`
- L29 `_long(sym, at, pnl=0.1, hold_h=6.0, lev=4.0)`
- L33 `_short(sym, at, pnl=0.1, hold_h=6.0, lev=4.0)`
- L39 `_caches(longs, shorts, lk='safe', sk='safe_h')` — Кэши обеих книг в том виде, в каком их пишут сами прогоны.
...
## research/dca_paper/test_paper.py · 2894 строк
...
- L559 `table_shape(txt, needle)` — Форма markdown-таблицы, в шапке которой стоит `needle`.
- L578 `test_day_concentration_is_measured_and_not_faked()` — Один эпизод раздаёт деньги многим именам — колонка по именам слепа.
- L623 `test_day_says_what_each_side_brought()` — День делится по СТОРОНАМ, и сторона без сделок — прочерк.
- L674 `test_drawdown_says_whether_one_day_made_it()` — Просадка одним днём и просадка месяцем сползания — разные книги.
- L719 `test_short_record_says_not_measured_not_zero()` — Три дня из трёх вычитать нечем: прочерк, а не ноль.
- L739 `test_two_rulers_are_two_books_and_optimal_is_untouched()` — Одно решение живёт в ОБЕИХ книгах, и вторая не читается повтором.
- L785 `test_aggressive_gate_takes_only_levered_entries()` — Третий режим = та же линейка глубины плюс ГЕЙТ по плечу.
- L830 `test_declared_peak_is_checked_against_the_measured_one()` — Объявленный пик обязан быть не ниже измеренного, иначе крик.
- L860 `test_legacy_row_reads_as_the_ruler_it_was_written_with()` — Строка без поля `ruler` писана глубиной — и обязана попасть к ней.
- L880 `test_cash_refusals_reach_the_report_and_survive_restat()` — Отказы кассы — прямой ответ «что покупает депозит», и их нельзя терять пересборкой свода: `--restat` ничего н…
- L945 `_cache_run(cache_seed, legs, td)` — Настоящий `main` с подставным дорогим проходом.
- L1001 `test_journal_path_is_resolved_at_call_time()` — Прогон с подменённым журналом не смеет писать в НАСТОЯЩИЙ.
- L1032 `test_cache_replays_new_and_open_but_not_closed()` — Кэш реплея законен ровно для ЗАКРЫТЫХ позиций.
- L1073 `test_cache_of_other_rules_is_refused_out_loud()` — Кэш чужих правил не чинится молча.
- L1106 `test_rules_change_starts_a_fresh_record()` — Смена правил (билета) начинает запись заново, а не дописывает.
- L1135 `_control_no_split()` — Свод, складывающий наблюдение с пересчётом, — то, ради чего split.
- L1149 `_control_ticket_below_floor()` — Билет ниже пола биржи — часть сигналов физически неисполнима.
- L1163 `_control_one_per_name_off()` — Без правила биржи повтор по имени попадает в книгу.
- L1177 `_control_journal_overwrites()` — Журнал, переписывающий строку, позволяет подвинуть момент записи.
- L1198 `_control_day_concentration_by_one_day()` — Контроль: вычесть ОДИН лучший день вместо трёх — проверка обязана пасть.
- L1223 `_control_dedup_without_ruler()` — Контроль: дедуп без линейки — вторая книга не пишется вовсе.
- L1237 `_control_legacy_reads_as_safe()` — Контроль: прежняя строка объявлена безопасной — книга подменена.
- L1251 `_control_restat_drops_the_counts()` — Контроль: пересборка молча выбрасывает числа счётного прогона.
- L1269 `_control_dedup_without_rules_version()` — Контроль: дедуп БЕЗ версии правил — прежнее поведение дословно.
- L1299 `_control_contracts_are_money_not_coins()` — Контроль: контракты посчитаны ДЕНЬГАМИ рунга, а не монетами.
- L1330 `_control_walk_ignores_side()` — Контроль: ступени цели считаются без стороны (длинная геометрия).
- L1353 `_control_row_side_ignores_book()` — Контроль: сторона артефактной записи берётся умолчанием `ruler_of`, а не переданной книгой — короткая книга ч…
- L1372 `_control_gate_is_gone()` — Гейта нет вовсе: третий режим молча становится копией второй книги.
- L1392 `_control_gate_on_every_ruler()` — Порог назначен всем режимам: книга без поля теряет свои входы.
- L1406 `test_shape_counts_positions_not_days()` — Доля прибыльных СДЕЛОК и среднее время в сделке — свои меры.
- L1451 `test_worst_open_is_measured_and_missing_is_not_zero()` — Худшая ОТКРЫТАЯ позиция считается сервером, а пустое — прочерк.
- L1476 `test_journal_rotates_by_day_and_reader_takes_every_part()` — Ротация: запись идёт в СУТОЧНЫЙ файл по метке решения, а чтение берёт все куски и снимает перекрытие.
- L1541 `test_watchdog_runs_the_book_hourly_and_asks_when_it_last_counte…` — Сторож ведёт книгу САМ, и вопрос он задаёт правильный.
- L1568 `test_retired_books_stop_trading_but_keep_their_record()` — Снятая книга не торгует и не показывается, но её ЗАПИСЬ цела.
- L1601 `test_replay_signature_ignores_books_that_do_not_replay()` — Подпись кэша реплея описывает то, что реплей СЧИТАЕТ.
- L1632 `test_rules_snapshot_carries_only_living_books()` — Свод несёт ДЕЙСТВУЮЩИЙ список книг — тем же кодом, что и прогон.
- L1656 `test_watchdog_runs_short_books_by_the_same_rule()` — Короткие книги семейства h24 поднимаются тем же правилом: по метке «когда считали», раз в час, кроме часов об…
- L1688 `_run_watchdog_cases(block, art_name='DCA-paper.json')`
- L1755 `test_tail_marks_outcomes_and_refuses_an_entry_from_a_quote()` — Правило хвоста держит ОБЕ границы, и они про разное.
- L1798 `test_tail_reaches_the_core_and_the_replay_signature()` — Дорога правила до ядра и до кэша — отдельный предмет.
- L1831 `test_cut_position_gets_a_named_reason()` — Оборванная позиция получает ПРИЧИНУ, и причин три разных.
- L1877 `test_contracts_walk_matches_the_simulation()` — Контракты позиции = то, что купила симуляция, а не второй счёт.
- L1919 `test_short_books_are_declared_as_a_mirror()` — Реестр несёт шесть книг, и у коротких сторона объявлена полем.
- L1946 `test_take_rule_mirrors_the_promise_side()` — Цель шорта берётся у обещания ВНИЗ; лонг считается прежним числом.
- L1961 `test_short_legs_go_only_into_short_books()` — Нога идёт в книгу СВОЕЙ стороны, и обе стороны считаются одним проходом.
- L2001 `test_venue_cap_is_in_the_replay_signature()` — Предел плеча площадки входит в ПОДПИСЬ реплея.
- L2013 `test_venue_leverage_cap_reaches_the_fence()` — Предел плеча ПЛОЩАДКИ доезжает до забора, а не остаётся правилом.
- L2062 `test_replay_cache_asks_only_for_its_own_side()` — Кэш спрашивают парой СВОЕЙ стороны — иначе пересчёт вечен.
- L2083 `_smooth_rows(pairs, mode='optimal', dep=10000.0)` — Журнал из пар «(деньги лонга, деньги шорта)» по суткам.
- L2097 `test_smoothing_finds_it_and_stays_silent_without_it()` — Калибровка меры: находит сглаживание и молчит на его отсутствии.
- L2125 `test_journal_shard_rolls_over_by_size()` — Часть суточного файла ограничена по РАЗМЕРУ, и читатель видит все.
- L2161 `test_journal_reader_parses_only_changed_parts()` — Читатель журнала разбирает кусок ОДИН раз, пока кусок не менялся.
- L2257 `test_repack_splits_an_oversized_day()` — Перепаковка режет переросшие сутки и не теряет ни одного решения.
- L2296 `test_smoothing_reads_the_journal_pair()` — Дорога замера до журнала, а не только его формула.
- L2322 `test_smoothing_splits_the_capital_of_two_books()` — У пары книг капитал ВДВОЕ: их проценты не складываются в один.
- L2340 `TESTS = [test_net_rides_the_summary_with_reason…`
- L2392 `_control_journal_path_frozen()` — Путь журнала снова берётся значением по умолчанию: прогон с подменённым журналом пишет в настоящую запись кни…
- L2415 `_control_cache_reuses_open()` — Состояние в кэше не читается: открытая позиция берётся вчерашней.
- L2443 `_control_cache_sig_ignored()` — Подпись правил не сверяется: кэш чужой геометрии молча идёт в дело, и книга новых правил считалась бы наполов…
- L2475 `_control_floor_one_for_everyone()` — Пол назначен один на всех: режим с гейтом не может взять мелкий билет, и на $1k у него остаётся вчетверо мень…
- L2491 `_control_peak_from_the_pool()` — Пик берётся общий (прежнее поведение): режим с гейтом снова стоит недогруженным — ровно тот дефект, ради кото…
- L2506 `_control_state_ignored()` — Состояние позиции не читается: живая попадает в журнал закрытой — ровно тот дефект, ради которого состояние и…
- L2531 `_control_win_counts_days()` — Доля прибыльных считается по ДНЯМ, а не по сделкам: день с двумя плюсами и одним минусом объявляется целиком…
- L2552 `_control_missing_mark_reads_as_zero()` — Позиция без отметки читается ровной: «не измерено» подменяется нулём — ровно тот класс, от которого защищает…
- L2573 `_control_watchdog_asks_mtime()` — Сторож снова смотрит на mtime файла вместо метки счёта.
- L2598 `_poison_run_paper(lit, repl, probe)` — Прогнать `probe` на ИСПОРЧЕННОМ `run_paper.py` и вернуть файл.
- L2633 `_control_shard_ignores_size()` — Ротация только суточная — часть перерастает порог.
- L2651 `_control_repack_drops_the_tail()` — Перепаковка пишет только первую часть — решения теряются.
- L2674 `_control_smoothing_takes_the_pair_as_rows()` — Пара взята целиком за строки — дорога обязана упасть.
- L2699 `_control_tail_never_reaches_the_core()` — Хвост построен, но в дорогой проход не подан.
- L2711 `_control_tail_out_of_the_replay_signature()` — Подпись реплея не знает хвоста: кэш прежних правил взялся бы молча.
- L2718 `_control_venue_cap_out_of_the_replay_signature()` — Подпись реплея не знает предела площадки: кэш, посчитанный забором без предела, взялся бы молча — и книга ост…
- L2727 `_control_tail_entry_from_a_quote_allowed()` — Граница входа снята: хвост заводит сделки, которых у книги не было.
- L2751 `_control_cut_reason_is_one_for_all()` — Причина обрыва одна на всех: «книги нет вовсе» приписывается и тому имени, у которого книга дотянулась дальше…
- L2767 `_control_cut_reason_by_symbol_not_position()` — Причина берётся ПО ИМЕНИ: книга, дотянувшаяся у соседнего окна, объявляется дотянувшейся и здесь. Тогда «книг…
- L2789 `_control_venue_cap_not_passed()` — Забор зовут без предела площадки — дорога обязана упасть.
- L2833 `CONTROLS = [('хвост не доезжает до ядра', _control…`
- L2884 `main()`
...
- L31 `H = 3600.0`
- L32 `T0 = 1786320000.0`
- L35 `_picks(path, hours, arms=('gbm', 'nn'))` — Журнал выборов книги со сроком: у каждой руки своё имя в час.
- L47 `test_cache_signature_follows_the_cell_and_the_hold()`
- L65 `test_legs_come_from_both_arms_in_time_order()`
- L83 `test_needs_replay_asks_for_new_and_open_positions()`
- L96 `_end_to_end(tmp)`
- L117 `test_family_writes_its_own_journal_and_gates_the_aggressive_boo…`
- L169 `test_floor_is_per_book_and_the_cache_knows_it()` — Пол капитуляции — свой у книги, и кэш обязан это знать.
- L207 `test_replay_gives_each_ruler_its_own_floor()` — Симуляция линейки видит ИМЕННО её пол — проверка на самой дороге.
...
- L29 `AT = 1789084800.0 + 3600.0` — ровный час UTC, чтобы граница суток была в 24 часах позади
- L32 `_hour(ts)`
...
## research/mech_13a67a67/test_age_mode.py · 632 строк
...
- L34 `TESTS = []`
- L35 `H = 3600.0`
- L36 `T0 = TR._ts('2026-08-12-00')`
- L39 `test(fn)`
- L44 `quiet(*_a)`
- L50 `line(hour_i, arm, rows, at_ts=None)` — Строка журнала разбора в ЖИВОЙ форме.
- L64 `row(sym, side, got, net=None)`
- L71 `launch_of(**days)` — Символ → момент листинга, отсчитанный от начала окна фикстур.
- L76 `small(n_hours=6, per_hour=4)` — Небольшой живой на вид журнал: обе руки, обе стороны, дрожание.
- L91 `two_band(n_old=200, n_young=200, hit_old=0.7, hit_young=0.4, se…` — Выборы двух объявленных полос с заданными долями попаданий.
- L117 `dca_rec(sym, k, side='long', lev=3.0, pnl=0.02, hold_h=40)` — Запись реплея длинной книги в форме кэша `run_paper`.
- L135 `момент_решения_это_конец_часа()`
- L144 `будущее_не_меняет_прошлого()` — Переписать `at_ts` — прошлое не должно шелохнуться.
- L168 `знак_стороны_у_хода()`
- L182 `ничья_не_попадание()`
- L191 `пустая_ячейка_прочерк()`
- L200 `отказ_вместо_пустоты()` — Непустой вход, давший ноль наблюдений, — отказ, а не отчёт.
- L215 `слабый_бутстрап_прочерк()` — Годных повторов мало — интервала НЕТ, и это не пара нулей.
- L230 `возраст_неизвестен_своя_полоса()`
- L241 `полосы_вердикта_объявлены()` — Границы взяты у `pair_age`, а не назначены здесь заново.
- L253 `порог_измеримости_кусается()`
- L264 `разность_ниже_порога_убивает()`
- L275 `интервал_поверх_нуля_убивает()`
- L290 `интервала_нет_не_значит_нуля()` — Прочерк интервала обязан убивать так же, как накрытый ноль.
- L304 `лотерея_вердикта_не_выносит()` — Медиана хуже, среднее лучше — подпись лотереи, вердикта нет.
- L320 `фраза_вердикта_из_чисел()` — Вердиктовая фраза выводится из чисел, а не стоит рядом с ними.
- L336 `час_единица_независимости()` — Выборы одного часа делят рынок: интервал по часам обязан быть шире.
- L368 `единица_бутстрапа_объявлена()`
- L382 `калибровка_подсадка_ложится()` — Подделка обязана сперва доказать, что легла, — числом.
- L401 `калибровка_подсадка_переворачивает()`
- L412 `калибровка_перемешивание_внутри_часа()` — Возраста мешаются ВНУТРИ часа: состав часа обязан устоять.
- L432 `калибровка_перемешивание_уравнивает()`
- L446 `подсаженный_режим_находится()` — Первая нога пары: меру видно там, где эффект подсажен.
- L457 `на_шуме_молчит()` — Вторая нога пары: на случайном блуждании мера обязана молчать.
- L470 `сторона_фильтра_не_трогает_чужую()` — Фильтр видит СВОЮ сторону; чужие записи проходят нетронутыми.
- L488 `контроль_размера_из_своей_стороны()` — Случайный контроль берёт столько же и ИЗ СВОЕЙ стороны.
- L512 `ничья_контроля_фильтру_не_в_зачёт()`
- L520 `отношение_без_просадки_прочерк()`
- L528 `вердикт_переноса_из_чисел()`
- L548 `книга_считается_чужим_ядром()` — Деньги длинной книги — `run_paper`, своей симуляции здесь нет.
- L566 `перенос_считает_обе_ветки()` — Шаг 2: фильтр возраста и контроль того же размера — одной дорогой.
- L587 `справочник_пуст_отказ()` — Пустой справочник — отказ, а не запись целиком в «неизвестен».
- L598 `повтор_выбора_не_удваивает()` — Один выбор дважды удвоил бы вес часа в бутстрапе.
- L608 `лист_без_журнала_не_ноль()` — Нет файла — причина словами, а не пустая ячейка листа.
- L617 `main()`

## research/mech_193cbb87/test_name_pause.py · 640 строк
...
- L42 `T0 = 1786147200.0`
- L43 `NOW = T0 + 60 * DAY`
- L44 `CTX = {'error': 'издержки в проверке не счита…`
- L45 `BOOK = 'safe_h'`
- L46 `RULER = 'safe_s'`
- L47 `DEP = 10000.0`
- L50 `test(fn)`
- L55 `quiet(*_a)`
- L59 `class NoMarket` — Рынка в проверке нет: охрана не срабатывает ни разу.
  - L67 `NoMarket.__init__(self)`
  - L70 `NoMarket.k_star(self, _at, _pct, _last)`
- L84 `rec(sym, at, pnl=0.25, exit='срок', exit_ts=None, hold_h=24, le…` — Запись реплея в ЖИВОЙ форме: поля те же, что пишет прогон.
- L109 `cache_of(rows, ruler=RULER)`
- L113 `launch_of(rows, days=400.0)`
- L117 `packed_of(rows)`
- L121 `spread(days=14, per_day=3, pnl=None, exit_of=None)` — Книга на несколько суток: колонок концентрации без четырёх суток нет.
- L134 `with_repeats(rows=None)` — Живая книга плюс имя, где книга вошла СРАЗУ ПОСЛЕ своего пола.
- L149 `rows_of(recs, book=BOOK, dep=DEP, now=NOW)` — Строки кассы книги на этих записях — тем же ядром, что прогон.
- L160 `пауза_не_берёт_имя_после_своего_хвостового_выхода()` — Вход в имя запрещён, пока не прошло P часов после пола или ликвидации.
- L175 `срок_и_тейк_паузы_не_дают()` — Триггер — только исходы хвоста; обычный исход книгу не останавливает.
- L188 `пауза_кончается_ровно_через_P_часов()` — Граница окна объявлена: вход ровно через P часов уже разрешён.
- L198 `не_заглядывает_в_будущее()` — Переписали будущее — прошлое не шелохнулось.
- L220 `две_трактовки_триггера_различаются()` — Правило смотрит ПРЕДЫДУЩУЮ позицию, шаг 0 — последний хвостовой выход.
- L233 `триггер_только_взятая_позиция()` — Решение, которого книга НЕ взяла, паузы не даёт.
- L254 `повторы_снимаются_по_порядку_времени()` — Снимается САМАЯ РАННЯЯ запрещённая запись, и состав считается дальше.
- L273 `отказ_вместо_тишины_при_переполнении()` — Правило, снявшее больше предела записей, роняет прогон вслух.
- L291 `размер_контроля_равен_вычеркнутым_взятым()` — Спорить можно только о ВЗЯТЫХ позициях: снятое решение, которого касса и так не брала, денег книги не меняет.
- L308 `спор_идёт_о_названной_величине()` — Доля зёрен считается по той величине, о которой спор, а не по числу сделок: сделок у правила и у случайного у…
- L325 `вердикт_выводится_из_числа()` — Фраза вердикта считается из доли зёрен, а не стоит рядом литералом.
- L335 `шаг_0_закрывает_инертное_правило()` — Убийца шага 0 стоит на ДВУХ числах сразу: и счёте, и доле сделок.
- L354 `непосчитанное_идёт_прочерком()` — Полоса без обеих групп — прочерк с причиной, а не ноль.
- L365 `перестановка_меток_кусается()` — Нуль перестановкой: разрыв, которого нет, обязан быть обычным.
- L392 `издержки_вычитаются_в_каждой_сделке()` — Деньги книги — НЕТТО: издержки накладывает общая функция проекта.
- L417 `сверка_с_ячейкой_семейства_кусается()` — Расхождение с ячейкой семейства обязано быть видно числом.
- L445 `ускорение_сверяется_бит_в_бит()` — Узкий проход по одному депозиту равен полному — числами, а не верой.
- L467 `кэш_реплея_не_переписывается()` — Запрет — механизм, а не обещание в комментарии.
- L484 `калибровка_находит_подсадку()` — Подсаженный эффект обязан находиться: иначе «эффекта нет» неотличимо от сломанной загрузки.
- L509 `калибровка_молчит_на_шуме()` — Там, где урезать НЕЧЕГО, правило обязано быть неотличимо от случайной выборки того же размера: иначе «работае…
- L540 `пустота_не_выдаёт_себя_за_результат()` — Ноль записей при непустом входе — отказ с причиной, а не отчёт.
- L549 `книга_вердикта_без_позиций_есть_отказ()` — Записи есть, а книга вердикта пуста — тоже отказ, а не прочерки.
- L560 `метка_тейка_проверяется_на_данных()` — Исхода, которого в книге нет вовсе, плацебо не бывает — и молчать об этом нельзя: пустое плацебо читалось бы…
- L575 `прогон_целиком_и_отчёт()` — Механика целиком на живой по форме фикстуре: числа, убийцы, отчёт.
- L608 `ось_и_ячейка_объявлены_заданием()` — Ось, ячейка вердикта, исходы и пороги — из задания, а не отсюда.
- L625 `main()`

## research/mech_1d5e7287/test_rr_supply.py · 605 строк

Проверки механики 1d5e7287 (гейт RR: подача или отбор).

- L24 `HERE = os.path.dirname(os.path.abspath(__file_…`
- L25 `RESEARCH = os.path.dirname(HERE)`
- L26 `ROOT = os.path.dirname(RESEARCH)`
- L45 `HOUR = 3600`
- L46 `MINUTE = 60`
- L47 `T0 = 1786000000`
- L48 `FAILS = []`
- L51 `noop(*_a, **_k)`
- L55 `check(name, ok, detail='')`
- L65 `bars_for(sym, t_start, minutes, seed, shock_at=None, shock_mult…` — Минутные бары: (время, открытие, максимум, минимум, закрытие, объём).
- L90 `class FakeSrc` — Источник баров, который отдаёт ВЕСЬ ряд символа, не глядя на окно.
  - L98 `FakeSrc.__init__(self, series)`
  - L103 `FakeSrc.bars(self, sym, _t0, _t1)`
- L107 `legs_fixture()` — Ноги двух рук: полоса, гейт, промежуток и нога без отношения.
- L130 `sheets_fixture(path)` — Журнал листов живого образца: часы, руки, оба квантиля пути.
- L161 `replay_fixture(shock_mult=1.0, tmp=None)` — Настоящие записи позиций по синтетическим барам.
- L191 `rows_fixture(recs, n_gate=None)` — Строки «решение к решению» с проставленными издержками.
- L207 `fake_rows(n, base, seed, exits=None, unmeasured=0)` — Набор строк с объявленным исходом: для мер и зёрен.
- L226 `t_band_source()` — Полоса читается у оси `rr_band`, а не своим числом.
- L239 `t_rr_unknown()` — Нога без отношения не проходит НИ полосу, ни гейт.
- L252 `t_edge_gate()` — Край книги режет обе полосы одинаково.
- L262 `t_hi_is_live_gate()` — Полоса `hi` тождественна ЖИВОМУ гейту книг (`run_d6.gated_legs`).
- L284 `t_stream_reader()` — Потоковый читатель тождествен `tournament.legs_from_sheets`.
- L300 `t_sample()` — Объявленная выборка: размер, зерно номером, воспроизводимость.
- L320 `t_rows_match_build_rows()` — Своя сборка строк совпадает с кассой живых книг ДО ЦЕНТА.
- L338 `t_lookahead()` — Переписать будущее — прошлое не должно шелохнуться.
- L358 `t_calibration_literals()` — Литералы калибровочной пары объявлены ЗДЕСЬ и до чисел.
- L368 `t_calibration_bites()` — Калибровочная пара молчит на копии и ловит подсаженный сдвиг.
- L382 `t_seed_same_size()` — Случайная выборка берётся ТОГО ЖЕ размера, что гейтованный набор.
- L392 `t_verdict_from_number()` — Вердикт выведен из числа, а не стоит рядом с ним.
- L402 `t_refuse_on_empty()` — Ноль наблюдений при непустом входе — ОТКАЗ, а не прочерк.
- L414 `t_net_not_mixed()` — Нетто-мера не смешивает брутто с нетто.
- L424 `t_tail_exits()` — Хвостовой исход — пол и ликвидация, тейк и срок хвостом не бывают.
- L433 `t_dash_not_zero()` — Величина, которой нет, — прочерк, а не ноль.
- L441 `t_measurable_from_number()` — Измеримость выведена из числа суток, а не объявлена словом.
- L454 `t_mech_direction()` — Механизм назван верно только когда доливов У ПОЛОСЫ больше.
- L465 `t_shape_uses_pool_rule()` — Форма судится ПРАВИЛОМ ПУЛА, а не своими порогами.
- L478 `t_own_cache()` — Кэш сестры — свой файл со своей подписью; живое не трогается.
- L497 `t_cache_check_bites()` — Сверка с живым кэшем ловит расхождение ядра.
- L515 `t_lev_bands_disjoint()` — Полосы плеча дизъюнктны: решение не считается дважды.
- L537 `t_one_per_name()` — Биржевое правило «одна позиция на имя» книга соблюдает.
- L557 `t_report_has_no_stale_prose()` — Отчёт собирается и несёт ВЫВЕДЕННЫЕ вердикты, а не литералы.
- L580 `main()`

## research/mech_258a860c/test_budget_book.py · 574 строк
...
- L41 `T0 = 1786147200.0`
- L42 `NOW = T0 + 40 * DAY`
- L43 `CTX = {'error': 'издержки в проверке не счита…`
- L44 `TICKET = 222.0`
- L45 `FLOOR = 25.0`
- L46 `B20 = 2000.0`
- L49 `test(fn)`
- L54 `quiet(*_a)`
- L60 `rec(sym, at, pnl, hold_h=24, lev=3.0, fwd=100.0, state='closed')` — Запись реплея в ЖИВОЙ форме: поля те же, что пишет прогон.
- L83 `cache_of(rows, ruler=None)`
- L88 `tight(n=25, pnl=BB.PLANT_PNL, at=None)` — Тесный час: n позиций одной секунды, имена РАЗНЫЕ.
- L102 `spread(days=12, per_day=3, pnl=None)` — Книга на несколько суток: колонок концентрации без четырёх суток нет.
- L114 `launch_of(cache, days=400.0)`
- L118 `class NoMkt`
- L122 `class HotMkt` — Рынок, который ИНОГДА срабатывает: без него охрана непроверяема.
  - L131 `HotMkt.__init__(self, k=3, every=3)`
  - L135 `HotMkt.k_star(self, at, _pct, last)`
- L142 `form(cache, maker=None, dep=BB.DEP, now=NOW, ctx=CTX)`
- L147 `margins(sz)`
- L155 `обёртка_без_правила_не_меняет_книгу()` — Обёртка, ничего не решающая, обязана дать ТУ ЖЕ книгу.
- L172 `бюджет_режет_тесный_час()` — Тесный час стоит книге не больше бюджета — с точностью до цента.
- L191 `половина_остатка_а_не_весь()` — Половина остатка — половина: правило целого остатка даёт другой счёт.
- L207 `остаток_считается_от_открытых()` — Деньги возвращаются в бюджет в секунду выхода, не раньше и не позже.
- L231 `ниже_пола_не_открывается()` — Позиция, которой остаётся меньше пола билета, не открывается.
- L250 `счёт_обёртки_равен_кассе()` — Свой счёт эквити обязан совпасть с итогом самой кассы.
- L264 `сверка_счёта_кусается()` — Сама сверка обязана падать на расхождении, а не молчать.
- L276 `будущее_не_меняет_прошлого()` — Переписать будущее — маржа прошлых решений не шелохнётся.
- L312 `равномерное_уменьшение_не_двигает_укус()` — Нога «молчать»: те же позиции с маржой ×0.5 — тот же укус.
- L331 `калибровка_находит_подсадку()` — Нога «найти»: подсаженный тесный час обязан быть срезан бюджетом.
- L346 `форма_равна_кассе()` — Форма книги обязана совпасть с ячейкой семейства на общих полях.
- L372 `укус_берётся_у_меры_проекта()` — Укус — `stability.stats`, и он сходится с укусом кассы.
- L383 `дня_нет_прочерк_а_не_ноль()` — Дня нет в своде книги — прочерк с причиной, а не нулевые деньги.
- L393 `загрузка_считает_часы_жизни()` — Загрузка считается по всем часам жизни позиции, а не по часу входа.
- L415 `розыгрыш_контроля_выбирает_число()` — Какой контроль считать, решает доля вычеркнутых полом, а не автор.
- L426 `перестановка_меняет_раскладку()` — Контроль перестановкой обязан РАЗДАВАТЬ те же множители иначе.
- L448 `число_взятых_сходится_или_названо()` — Разница взятых объясняется полом билета — или называется числом.
- L470 `доля_зёрен_считается_мерой_проекта()` — Доля «не хуже» — одна мера на проект; укус едет в неё со знаком.
- L478 `вердикт_выведен_из_числа()` — Фразы убийц собираются из чисел и переворачиваются вместе с ними.
- L502 `пустота_не_выдаёт_себя_за_результат()` — Ноль наблюдений при непустом входе — отказ, и ПРИЧИНА своя.
- L524 `прогон_и_отчёт_целиком()` — Прогон проходит все шаги, а показ выдерживает все их состояния.
...
## research/mech_374e2591/test_floor_sister.py · 604 строк
...
- L47 `test(fn)`
- L52 `quiet(*_a)`
- L58 `rec(sym, i_h, cums, lev=3.0, exit='срок', entry=100.0, at=None,…` — Запись кэша с ЯВНЫМ путём: `cums` — накопленный pnl по часам 1..K.
- L83 `cache_of(rows, ruler=None)`
- L88 `launch_of(cache, days=400.0)`
- L92 `class StubMkt` — Рынок по часам: час срабатывания охраны задаётся по моменту входа.
  - L95 `StubMkt.__init__(self, hour_of=None)`
  - L99 `StubMkt.k_star(self, at, _pct, kmax)`
- L106 `flat_cache(n=12, step_h=8)` — Кэш на несколько суток, где пол не срабатывает НИ РАЗУ.
- L115 `deep_cache(n=12, step_h=8)` — Кэш, где часть позиций уходит глубоко под воду.
- L129 `views_of(cache)`
- L136 `уровень_пола_формулой_ядра()` — Края оси доли — тождества: 0 есть ликвидация, 1 есть вход.
- L155 `уровень_зависит_от_плеча()` — У крупного плеча ликвидация ближе, и четверть пути до неё дешевле.
- L164 `запись_без_рунгов_не_считается()` — Нотионал, не сходящийся с рунгами, — отказ с причиной, а не пол.
- L174 `уровень_не_отрицателен_выбывает()` — Пол стоит ПРОТИВ позиции: неотрицательный уровень правилом не станет.
- L184 `сверка_ликвидации_кусается()` — Выведенная ликвидация против записи: подделка цены обязана всплыть.
- L206 `правила_берутся_у_книги()` — Числа правила читаются у источников, а не назначены здесь.
- L234 `час_среза_строго_до_выхода()` — Час выхода в счёт не идёт: правило не узнаёт исхода заранее.
- L248 `будущее_не_меняет_прошлого()` — Переписать будущее — час и цена выхода не шелохнутся.
- L270 `порез_по_уровню_равен_полу()` — Порез по уровню кладёт исход РОВНО на пол, а по отметке — на отметку.
- L286 `калибровочная_пара()` — Подсаженный ход находится, подсаженный шум молчит.
- L307 `форма_равна_кассе()` — Своей кассы нет: форма совпадает с ячейкой семейства, охрана в ней.
- L328 `порядок_пола_и_охраны()` — Раньше — тот, кто раньше; ничья остаётся за полом.
- L345 `окно_показа_не_пересчитывает_кассу()` — Окно режет ПОКАЗ, а не счёт: раздача денег остаётся на всей истории.
- L362 `порог_убийцы_А_доля_измеренного()` — Порог (А) — доля ИЗМЕРЕННОГО худшего дня базовой, не литерал.
- L374 `вердикт_выводится_из_числа()` — Фраза следует за числом, а не стоит рядом с ним литералом.
- L388 `убийца_Б_из_доли_зёрен()` — Контроль судит долей зёрен, и порог объявлен заданием.
- L398 `убийца_В_обе_половины()` — Медиана дня И «без 3 лучших дней» — убивает любая из двух.
- L411 `убийца_Г_прочерк_без_форварда()` — Форварда у необъявленной сестры нет: прочерк с причиной, не «жива».
- L422 `итог_собирается_из_вердиктов()`
- L434 `ноль_не_выдаёт_себя_за_результат()` — Ни одного пореза при непустом входе — отказ, а не отчёт.
- L451 `величина_которой_нет_прочерк()` — Ноль означает «измерено и равно нулю»; неизмеренное — прочерк.
- L465 `потолок_считается_целиком()` — Шаг 1 отдаёт колонки спора, контроль и вердикты по всем убийцам.
- L484 `отчёт_не_падает_на_пустом()` — Каждая дорога до показа проверяется отдельно, включая пустую.
- L500 `кэш_сестры_свой_и_с_полом()` — Записи сестры не трогают кэш книги, а пол входит в подпись.
- L513 `пол_возвращается_после_реплея()` — Пол симуляции — на время прогона, и возвращается даже при падении.
- L534 `изменённые_позиции_по_реплею()` — Пол умеет только укорачивать: изменённая позиция вышла РАНЬШЕ.
- L571 `разрыв_через_пол_считается()` — Исход ниже уровня — разрыв; ликвидация считается отдельно.
- L589 `main()`
...
## research/mech_9dd65163/test_funding_margin.py · 526 строк
...
- L34 `TESTS = []`
- L35 `HOUR = 3600.0`
- L37 `T0 = 1786183200.0` — Начало часа в живом окне журнала коротких книг (08.08–18.09).
- L38 `RULERS = {'safe_s': 0.1, 'optimal_s': 0.5}`
- L41 `test(fn)`
- L46 `quiet(*_a)`
- L52 `rec(cum, sym='BLUAIUSDT', at=T0, lev=8.0, exit='срок', entry=0.…` — Запись кэша реплея в ЖИВОЙ форме: те же поля, что пишет `run_d10`.
- L83 `series(rate, at=T0, every_h=4, n=12, start_h=-6, jitter=True)` — Ряд funding площадки: моменты через `every_h` часов, ставка `rate`.
- L98 `series_at(times, rate)` — Ряд с НАЗВАННЫМИ моментами начислений (число событий — свойство имени).
- L104 `ctx_of(ser, sym='BLUAIUSDT', asset='BLUAI')`
- L109 `cache_of(*recs)`
- L118 `funding_считается_знаком_costs()` — Знак и набор событий — из `costs.funding_usd`, не из своей копии.
- L136 `начисления_не_теряют_точность_на_округлении()` — Событие округляется до 4 знаков — масштаб маржи обязан это снимать.
- L152 `начисление_на_границе_часа_не_в_прошлое()` — Начисление ровно на границе часа принадлежит СЛЕДУЮЩЕМУ часу.
- L164 `будущее_не_меняет_прошлого()` — Переписать будущее — прошлое не шелохнётся.
- L188 `знак_ставки_читается()` — Расход двигает пол к входу, доход — от входа.
- L204 `пол_книги_берётся_из_реестра()` — Пол у книг РАЗНЫЙ, и он читается реестром, а не константой.
- L220 `сдвиг_строго_до_записанного_выхода()` — Сработавшее на самом часе выхода сдвигом не является.
- L230 `съеденная_маржа_даёт_ликвидацию_с_крышкой()` — Начисление, съевшее маржу, ликвидирует позицию независимо от цены.
- L247 `ликвидация_не_глубже_маржи()` — Учёт после факта приписывал ликвидации убыток глубже всей маржи.
- L260 `нетто_сохраняет_круг_издержек()` — Поправка меняет ЦЕНОВОЙ pnl; круг издержек сделки остаётся её.
- L272 `close_at_тождествен_охране_рынком()` — Без подмены исхода закрытие на часе — ровно `wave.guard_record`.
- L284 `непокрытый_ряд_прочерк_а_не_ноль()` — Ряд, не покрывающий жизнь позиции, даёт «не измерено», а не нуль.
- L298 `пустой_вход_отказ_а_не_отчёт()` — Ноль наблюдений при непустом входе — отказ, а не отчёт с прочерками.
- L313 `отодвинутый_пол_только_при_полученном_funding()` — Следствие «б» считает записи с ПОЛУЧЕННЫМ funding; у полосы свой нуль.
- L324 `остаток_ниже_минус_100_считается_числом()` — «По построению не остаётся» — утверждение, и оно считается числом.
- L357 `калибровочная_пара_кусается()` — Молчать на нуле и находить подсаженное — обе ноги числами.
- L371 `контроли_смысла_кусаются()` — Знак наоборот обязан дать ДРУГОЕ число сдвинутых; иначе тревога.
- L388 `нулевые_ставки_ничего_не_двигают()` — Нулевой funding — кэш тождествен исходному запись в запись.
- L405 `_cell(days, exits=None)`
- L412 `форма_книги_чужой_мерой()` — Укус и медиана хорошего дня — `factory/stability`, не своя копия.
- L424 `без_трёх_лучших_дней_прочерк_на_коротком_ряде()` — Трёх дней из трёх вычитать нечего — прочерк, а не ноль.
- L430 `_s_for(shift_share, postponed_share, d_usd, d_bite, cover=1.0)`
- L446 `вердикт_выводится_из_числа()` — Фраза собирается из чисел; смена числа обязана менять фразу.
- L460 `покрытие_ниже_порога_блокирует()` — Ряды не покрывают журнал — блок с причиной, а не вердикт по книгам.
- L471 `обе_трактовки_следствия_б_печатаются()` — Трактовка не выбирается молча: судит строгая, вторая — рядом.
- L482 `отчёт_печатает_вердикт_и_покрытие()` — Отчёт без чисел покрытия и вердикта читать нельзя.
- L511 `main()`

## research/mech_a47008e1/test_guard_sister.py · 792 строк
...
- L40 `T0 = 1786176000.0`
- L41 `NOW = T0 + 60 * H`
- L42 `CTX = {'error': 'издержки в проверке не счита…`
- L45 `test(fn)`
- L50 `quiet(*_a)`
- L56 `marks_of(at, pnl, n_h)` — Почасовые приращения, сумма которых РАВНА исходу.
- L69 `rec(sym, i_h, pnl, hold_h=24, lev=3.0, fwd=100.0, at=None)`
- L83 `cache_of(rows, ruler=None)` — Кэш реплея в живой форме: ключ — (линейка, имя, момент).
- L89 `small_cache(n=12, step_h=8)` — Кэш на НЕСКОЛЬКО суток: колонки концентрации без четырёх дней не существуют, и фикстура на одних сутках прята…
- L99 `launch_of(cache, days=400.0)`
- L103 `class StubMkt` — Рынок по часам: волна задаётся функцией от часа с входа И от момента.
  - L111 `StubMkt.__init__(self, wave_of=None)`
  - L115 `StubMkt.wave(self, t0, t1)`
  - L122 `StubMkt.move(self, _sym, _t0, _t1)`
  - L125 `StubMkt.beta_pre(self, _sym, _at, **_kw)`
- L129 `views_of(cache, wave_of=None)`
- L134 `window_wave(cache, hi=0.03, k_min=4)` — Волна ≥ `hi` после часа `k_min`, но ТОЛЬКО внутри окна фикстуры.
- L149 `bars_of(t0, t1, px_of, step=M)` — Минутные бары по функции цены: [t, o, h, l, c, объём].
- L160 `planted_px(at, jumps)` — Цена прокси: ступеньки, объявленные парами (минута с входа, цена).
- L172 `planted_wave(at, jumps, hours=26, proxies=None, min_have=None, …` — Волна на подсаженных барах — тем же кодом, что живая.
- L199 `flat_paths(cache, n=GS.HOLD_MIN, pnl_of=None)` — Минутные пути: pnl минуты i = i/1000, время бара = at + i·минута.
- L220 `форма_равна_кассе()` — Форма книги обязана совпадать с ячейкой семейства бит в бит.
- L240 `фильтр_возраста_применён()` — Молодое имя книга не берёт — и ровно это делает касса.
- L260 `колонка_дня_прочерк_а_не_ноль()` — Дня нет в своде — прочерк с причиной; есть — число.
- L275 `калибровка_подсаженная_волна()` — Подсаженный ход +2.5 % находится с точностью до минуты.
- L291 `калибровка_шум_молчит()` — На шуме без хода волна порога не пересекает — ни одной минуты.
- L312 `будущее_не_меняет_прошлого()` — Переписать будущее — минута пересечения не шелохнётся.
- L336 `меньше_пяти_прокси_нет_волны()` — Имён с ценой меньше порога — волны нет, а не среднее по двум.
- L355 `база_волны_до_входа()` — Ход считается от закрытия ПЕРЕД входом, а не от первой минуты сделки.
- L369 `минутный_срез_равен_часовому()` — На границе часа минутный срез обязан совпасть со срезом оси.
- L388 `срез_держит_сумму_отметок()` — Сумма приращений отметок среза равна исходу среза.
- L401 `розыгрыш_как_у_оси()` — Розыгрыш контроля обязан быть ТОТ ЖЕ, что у оси в path_screen.
- L427 `контроль_по_колонке_спора()` — Контроль считается по ВСЕМ объявленным колонкам, а не по итогу.
- L451 `охрана_строго_до_выхода()` — Пересечение после фактического выхода сделку не трогает.
- L472 `задержка_минуты_считается()` — Выход берётся минутой позже пересечения — и это видно числом.
- L488 `пустая_охрана_отказ()` — Охрана не сработала — отказ с причиной, а не колонки прочерками.
- L501 `реплей_считает_колонки_и_контроль()` — Сработавшая охрана даёт колонки и контроль по тем же колонкам.
- L515 `пути_собираются_по_пачкам()` — Пути копятся по всем пачкам, чужая линейка не идёт, отказы — числом.
- L564 `пачки_не_рвут_символ()` — Символ не делится между пачками: бары имени читаются раз на пачку.
- L576 `метки_ядра_на_середине_минуты()` — Метка ядра попадает в свою минуту, а не в предыдущую.
- L593 `убийца_один_из_числа()` — Фраза убийцы 1 выведена из чисел, а не стоит рядом литералом.
- L614 `убийца_два_сравнивает_с_базой()` — Убийца 2 судит сестру против базы на ТЕХ ЖЕ сутках и числами.
- L629 `порядок_шагов_соблюдён()` — Реплей не считается, пока потолок не пройден; --force снимает.
- L651 `отказ_вместо_пустоты()` — Закрытых позиций линейки нет — отказ, а не отчёт с прочерками.
- L661 `нуль_сдвига_волны()` — Нуль сдвига обязан быть ДРУГИМ рядом и гасить прибавку.
- L682 `форвард_не_выдаёт_себя_за_пересчёт()` — Без дня объявления форвардных суток НОЛЬ, как бы pool ни считал.
- L702 `связь_берётся_у_фабрики()` — Модулей `ceiling.py` в проекте два, и связь считает ФАБРИЧНЫЙ.
- L717 `сетка_берётся_у_оси()` — Порог и сетка не назначаются здесь: они у path_screen.AXES.
- L726 `отчёт_говорит_числами()` — Отчёт печатает вердикт, колонки и причины — без падений на прочерках.
- L777 `main()`
