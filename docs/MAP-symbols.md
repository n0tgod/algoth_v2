...
## research/dca_paper/pair_gate.py · 367 строк
...
- L184 `run(dep=None, log=print, ctx=None, long_cache=None, short_cache…`
- L256 `_u(x)`
- L260 `_p(x, d=1)`
- L264 `report(s)`
- L336 `publish(name)`
...
- L368 `_PAIR_NOTE = 'Общий счёт: длинная книга режима и кор…`
- L398 `PAIR_SHORT_SHARE = {'pair_safe': 1.0, 'pair_optimal': 0.25…` — Доля билета СТОРОНЫ в общем счёте — решение владельца 2026-09-07, объявленное ДО прогона по этому правилу. Ос…
- L404 `FAMILY_RULES = {'pair': 2}` — Версия правил СЕМЕЙСТВА. Общая версия `RULES` тут не годится: её смена обнулила бы запись длинных и коротких…
- L407 `family_rules(key)` — Версия правил семейства книги. Нет версии — нет и поля в строке.
- L412 `pair_share_mult(pair_key, book_key)` — Во сколько раз билет СТОРОНЫ меньше её собственного в общем счёте.
- L423 `ticket_in(pair_key, book_key, deposit)` — Билет стороны в общем счёте: свой билет × доля, но НЕ НИЖЕ пола.
- L436 `share_in(pair_key, book_key, deposit)` — Доля счёта на позицию стороны в общем счёте.
- L441 `parts_of(key)` — Из каких книг собран общий счёт. Обычная книга — источник один.
- L454 `RETIRED_AT = {'safe_s': '2026-09-07', 'optimal_s': '…` — --- снятые книги (решение владельца 2026-09-07) ------------------------ Зеркала `*_s` на ситуационном листе…
- L458 `retired(key)` — Снята ли книга. Дата снятия — в `RETIRED_AT`, одним местом.
- L467 `RULER_ORDER = [k for k in RULER_ORDER_ALL if not reti…` — Действующий порядок: снятая книга выпадает отсюда, и ВСЕ читатели — прогон, свод, отчёт, страница — перестают…
- L470 `family_of(key)` — Семейство книги: `sit` — по листу ситуационной, `h24` — по `h24`.
- L475 `order_of(family='sit', with_retired=False)` — Порядок книг семейства — один список на всех читателей.
- L487 `hold_of(key)` — Срок удержания книги, ч. Не объявлен — общий срок книг.
- L492 `DEFAULT_RULER = 'optimal'` — Запись БЕЗ поля `ruler` писана этой линейкой: до 2026-09-04 книга была одна и считалась глубиной. Умолчание д…
- L495 `ruler_of(row)` — Линейка строки журнала; у строк прежнего образца поля нет.
- L501 `notional_of(row)` — Нотионал позиции = маржа × плечо; None, если чего-то из двух нет.
- L517 `avg_walk(fills, entry=None, notional=None, take_frac=None, side…` — Плавающая ТВХ: средняя цена входа ПОСЛЕ каждого рунга.
- L612 `open_stats(positions)` — ХУДШАЯ из открытых позиций — та, что просела глубже всех СЕЙЧАС.
- L639 `ruler_title(key)`
- L643 `side_of(key)` — Сторона книги. Ключ без поля — длинная: до 2026-09-05 других нет.
- L654 `row_side(row, ruler=None)` — Сторона ЗАПИСИ позиции — одно правило на всех читателей.
- L674 `min_lev_of(key)` — Порог входа по плечу у режима; НЕТ поля — гейта нет вовсе.
- L692 `ONE_PER_NAME = True` — --- биржевое правило, которое реплей D-серии НЕ соблюдал --------------- На одном счёте в одностороннем режим…
- L701 `AHEAD_H = HOLD_H + 48` — --- «записано вперёд» --------------------------------------------------- Решение считается записанным ВПЕРЁД…
- L703 `HERE = os.path.dirname(os.path.abspath(__file_…`
- L704 `OUT = os.path.join(HERE, 'out')`
- L705 `JOURNAL = os.path.join(OUT, 'journal.jsonl')`
- L706 `ARTIFACT = os.path.join(OUT, 'DCA-paper.json')`
- L710 `H24_JOURNAL = os.path.join(OUT, 'short.jsonl')` — Журнал и артефакт семейства `h24` — СВОИ. Имя не «journal-…»: ротация длинной книги глобит `journal-*.jsonl`,…
- L711 `H24_ARTIFACT = os.path.join(OUT, 'DCA-short.json')`
- L716 `PAIR_JOURNAL = os.path.join(OUT, 'pair.jsonl')` — Общий счёт — тоже своё семейство со своим журналом: строки его книг ДРУГИЕ (та же сделка получает другую марж…
- L717 `PAIR_ARTIFACT = os.path.join(OUT, 'DCA-pair.json')`
- L720 `floor_of(ruler)` — Пол билета РЕЖИМА: биржевой минимум, переведённый в маржу.
- L732 `peak_of(ruler)` — Пик РЕЖИМА, измеренный по журналу. Неизвестный режим — пик пула.
- L742 `ticket(deposit, ruler)` — Билет книги: не меньше пола режима и не больше доли на все места.
- L755 `slots(deposit, ruler)` — Сколько мест помещается в депозит при билете этой книги.
- L760 `share(deposit, ruler)` — Доля счёта на позицию — ровно билет, выраженный долей.
- L765 `ahead(decided_at, written_at, hours=AHEAD_H, since=None)` — Записано ли решение вперёд, а не восстановлено пересчётом.
- L783 `shard_of(path, at=None)` — Файл журнала, в который идёт решение: СУТКИ по метке решения.
- L801 `shard_day(at=None)` — Дата суток решения строкой — ключ ротации, один на всех.
- L816 `SHARD_CAP = 4 * 1024 * 1024` — Порог ЧАСТИ суточного файла. Сутки оказались единицей недостаточной: одно решение живёт во всех книгах разом…
- L819 `shard_parts(path, at=None, day=None)` — Части суток по порядку: `journal-<дата>.jsonl`, затем `.01`, `.02`…
- L833 `shard_place(path, day, lines, cap=SHARD_CAP)` — Разложить строки суток по частям, не переступая порог.
- L858 `journal_parts(path=JOURNAL)` — Все куски журнала: старый цельный файл и суточные, по порядку.
- L870 `journal_key(r)` — Ключ решения: тем же составом, каким дедуплицирует запись.
- L882 `read_journal(path=JOURNAL, stats=None)` — Строки журнала как есть — из ВСЕХ его кусков, БЕЗ повторов.
- L919 `split_rows(rows, hours=AHEAD_H)` — Наблюдение и пересчёт — ДВА списка, и складывать их нельзя.

## research/dca_paper/run_pair.py · 428 строк
...
- L127 `collisions(rows, ruler)` — Имена, которые общий счёт держит РАЗОМ длинной и короткой.
- L144 `_side_series(rows, ruler, side)`
- L148 `link(rows, ruler)` — Связь дневных денег сторон ВНУТРИ общего счёта.
- L164 `separate(rows_by_book, dep)` — Те же книги на РАЗДЕЛЬНЫХ счетах — для сравнения с общим.
- L179 `one_sided(book, lk, sk)` — Какой стороны в общем счёте НЕТ. Пусто — обе на месте.
- L193 `run(log=print, now=None, journal=None, long_cache=None, short_c…`
- L272 `_p(x, d=2)`
- L276 `_u(x)`
- L280 `report(s)`
- L399 `publish(name)`
...
## tools/watchdog_book.sh · 335 строк
