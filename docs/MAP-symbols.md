...
## research/dca_paper/rules.py · 1022 строк
...
- L504 `family_rules(key)` — Версия правил семейства книги. Нет версии — нет и поля в строке.
- L509 `pair_share_mult(pair_key, book_key)` — Во сколько раз билет СТОРОНЫ меньше её собственного в общем счёте.
- L520 `ticket_in(pair_key, book_key, deposit)` — Билет стороны в общем счёте: свой билет × доля, но НЕ НИЖЕ пола.
- L533 `share_in(pair_key, book_key, deposit)` — Доля счёта на позицию стороны в общем счёте.
- L538 `parts_of(key)` — Из каких книг собран общий счёт. Обычная книга — источник один.
- L551 `RETIRED_AT = {'safe_s': '2026-09-07', 'optimal_s': '…` — --- снятые книги (решение владельца 2026-09-07) ------------------------ Зеркала `*_s` на ситуационном листе…
- L555 `retired(key)` — Снята ли книга. Дата снятия — в `RETIRED_AT`, одним местом.
- L564 `RULER_ORDER = [k for k in RULER_ORDER_ALL if not reti…` — Действующий порядок: снятая книга выпадает отсюда, и ВСЕ читатели — прогон, свод, отчёт, страница — перестают…
- L567 `family_of(key)` — Семейство книги: `sit` — по листу ситуационной, `h24` — по `h24`.
- L572 `order_of(family='sit', with_retired=False)` — Порядок книг семейства — один список на всех читателей.
- L584 `hold_of(key)` — Срок удержания книги, ч. Не объявлен — общий срок книг.
- L589 `DEFAULT_RULER = 'optimal'` — Запись БЕЗ поля `ruler` писана этой линейкой: до 2026-09-04 книга была одна и считалась глубиной. Умолчание д…
- L592 `ruler_of(row)` — Линейка строки журнала; у строк прежнего образца поля нет.
- L598 `notional_of(row)` — Нотионал позиции = маржа × плечо; None, если чего-то из двух нет.
- L614 `avg_walk(fills, entry=None, notional=None, take_frac=None, side…` — Плавающая ТВХ: средняя цена входа ПОСЛЕ каждого рунга.
- L709 `open_stats(positions)` — ХУДШАЯ из открытых позиций — та, что просела глубже всех СЕЙЧАС.
- L736 `ruler_title(key)`
- L740 `side_of(key)` — Сторона книги. Ключ без поля — длинная: до 2026-09-05 других нет.
- L751 `row_side(row, ruler=None)` — Сторона ЗАПИСИ позиции — одно правило на всех читателей.
- L771 `min_lev_of(key)` — Порог входа по плечу у режима; НЕТ поля — гейта нет вовсе.
- L789 `ONE_PER_NAME = True` — --- биржевое правило, которое реплей D-серии НЕ соблюдал --------------- На одном счёте в одностороннем режим…
- L798 `AHEAD_H = HOLD_H + 48` — --- «записано вперёд» --------------------------------------------------- Решение считается записанным ВПЕРЁД…
- L800 `HERE = os.path.dirname(os.path.abspath(__file_…`
- L801 `OUT = os.path.join(HERE, 'out')`
- L802 `JOURNAL = os.path.join(OUT, 'journal.jsonl')`
- L803 `ARTIFACT = os.path.join(OUT, 'DCA-paper.json')`
- L807 `H24_JOURNAL = os.path.join(OUT, 'short.jsonl')` — Журнал и артефакт семейства `h24` — СВОИ. Имя не «journal-…»: ротация длинной книги глобит `journal-*.jsonl`,…
- L808 `H24_ARTIFACT = os.path.join(OUT, 'DCA-short.json')`
- L813 `PAIR_JOURNAL = os.path.join(OUT, 'pair.jsonl')` — Общий счёт — тоже своё семейство со своим журналом: строки его книг ДРУГИЕ (та же сделка получает другую марж…
- L814 `PAIR_ARTIFACT = os.path.join(OUT, 'DCA-pair.json')`
- L817 `floor_of(ruler)` — Пол билета РЕЖИМА: биржевой минимум, переведённый в маржу.
- L829 `peak_of(ruler)` — Пик РЕЖИМА, измеренный по журналу. Неизвестный режим — пик пула.
- L839 `ticket(deposit, ruler)` — Билет книги: не меньше пола режима и не больше доли на все места.
- L852 `slots(deposit, ruler)` — Сколько мест помещается в депозит при билете этой книги.
- L857 `share(deposit, ruler)` — Доля счёта на позицию — ровно билет, выраженный долей.
- L862 `ahead(decided_at, written_at, hours=AHEAD_H, since=None)` — Записано ли решение вперёд, а не восстановлено пересчётом.
- L880 `shard_of(path, at=None)` — Файл журнала, в который идёт решение: СУТКИ по метке решения.
- L898 `shard_day(at=None)` — Дата суток решения строкой — ключ ротации, один на всех.
- L913 `SHARD_CAP = 4 * 1024 * 1024` — Порог ЧАСТИ суточного файла. Сутки оказались единицей недостаточной: одно решение живёт во всех книгах разом…
- L916 `shard_parts(path, at=None, day=None)` — Части суток по порядку: `journal-<дата>.jsonl`, затем `.01`, `.02`…
- L930 `shard_place(path, day, lines, cap=SHARD_CAP)` — Разложить строки суток по частям, не переступая порог.
- L955 `journal_parts(path=JOURNAL)` — Все куски журнала: старый цельный файл и суточные, по порядку.
- L967 `journal_key(r)` — Ключ решения: тем же составом, каким дедуплицирует запись.
- L979 `read_journal(path=JOURNAL, stats=None)` — Строки журнала как есть — из ВСЕХ его кусков, БЕЗ повторов.
- L1016 `split_rows(rows, hours=AHEAD_H)` — Наблюдение и пересчёт — ДВА списка, и складывать их нельзя.
...
## research/dca_paper/run_paper.py · 1435 строк
