...
## research/dca_paper/run_pair.py · 377 строк
...
## research/dca_paper/run_paper.py · 1312 строк
...
- L561 `summarize(path=None, live=None, keys=None)` — Свод по книгам: ОДНА кривая, и в ней помечено, что бэктест.
- L650 `_pct(x, d=2)`
- L654 `_tail_words(s)` — Числа хвоста словами. Нет чисел — так и сказано, а не ноль.
- L689 `costs_block(s)` — Раздел «издержки» — ОДИН на весь отчёт: деньги ниже уже нетто.
- L746 `report(s)`
- L1104 `rules_snapshot(keys=None)` — Правила прогона в свод: страница описывает ИМИ то, что видит.
- L1133 `publish(name)`
- L1139 `main()`
