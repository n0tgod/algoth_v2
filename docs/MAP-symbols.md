...
## research/dca_paper/rules.py · 1005 строк
...
- L484 `FAMILY_RULES = {'pair': 5}` — Версия правил СЕМЕЙСТВА. Общая версия `RULES` тут не годится: её смена обнулила бы запись длинных и коротких…
- L487 `family_rules(key)` — Версия правил семейства книги. Нет версии — нет и поля в строке.
- L492 `pair_share_mult(pair_key, book_key)` — Во сколько раз билет СТОРОНЫ меньше её собственного в общем счёте.
- L503 `ticket_in(pair_key, book_key, deposit)` — Билет стороны в общем счёте: свой билет × доля, но НЕ НИЖЕ пола.
- L516 `share_in(pair_key, book_key, deposit)` — Доля счёта на позицию стороны в общем счёте.
- L521 `parts_of(key)` — Из каких книг собран общий счёт. Обычная книга — источник один.
- L534 `RETIRED_AT = {'safe_s': '2026-09-07', 'optimal_s': '…` — --- снятые книги (решение владельца 2026-09-07) ------------------------ Зеркала `*_s` на ситуационном листе…
- L538 `retired(key)` — Снята ли книга. Дата снятия — в `RETIRED_AT`, одним местом.
- L547 `RULER_ORDER = [k for k in RULER_ORDER_ALL if not reti…` — Действующий порядок: снятая книга выпадает отсюда, и ВСЕ читатели — прогон, свод, отчёт, страница — перестают…
- L550 `family_of(key)` — Семейство книги: `sit` — по листу ситуационной, `h24` — по `h24`.
- L555 `order_of(family='sit', with_retired=False)` — Порядок книг семейства — один список на всех читателей.
- L567 `hold_of(key)` — Срок удержания книги, ч. Не объявлен — общий срок книг.
- L572 `DEFAULT_RULER = 'optimal'` — Запись БЕЗ поля `ruler` писана этой линейкой: до 2026-09-04 книга была одна и считалась глубиной. Умолчание д…
- L575 `ruler_of(row)` — Линейка строки журнала; у строк прежнего образца поля нет.
- L581 `notional_of(row)` — Нотионал позиции = маржа × плечо; None, если чего-то из двух нет.
- L597 `avg_walk(fills, entry=None, notional=None, take_frac=None, side…` — Плавающая ТВХ: средняя цена входа ПОСЛЕ каждого рунга.
- L692 `open_stats(positions)` — ХУДШАЯ из открытых позиций — та, что просела глубже всех СЕЙЧАС.
- L719 `ruler_title(key)`
- L723 `side_of(key)` — Сторона книги. Ключ без поля — длинная: до 2026-09-05 других нет.
- L734 `row_side(row, ruler=None)` — Сторона ЗАПИСИ позиции — одно правило на всех читателей.
- L754 `min_lev_of(key)` — Порог входа по плечу у режима; НЕТ поля — гейта нет вовсе.
- L772 `ONE_PER_NAME = True` — --- биржевое правило, которое реплей D-серии НЕ соблюдал --------------- На одном счёте в одностороннем режим…
- L781 `AHEAD_H = HOLD_H + 48` — --- «записано вперёд» --------------------------------------------------- Решение считается записанным ВПЕРЁД…
- L783 `HERE = os.path.dirname(os.path.abspath(__file_…`
- L784 `OUT = os.path.join(HERE, 'out')`
- L785 `JOURNAL = os.path.join(OUT, 'journal.jsonl')`
- L786 `ARTIFACT = os.path.join(OUT, 'DCA-paper.json')`
- L790 `H24_JOURNAL = os.path.join(OUT, 'short.jsonl')` — Журнал и артефакт семейства `h24` — СВОИ. Имя не «journal-…»: ротация длинной книги глобит `journal-*.jsonl`,…
- L791 `H24_ARTIFACT = os.path.join(OUT, 'DCA-short.json')`
- L796 `PAIR_JOURNAL = os.path.join(OUT, 'pair.jsonl')` — Общий счёт — тоже своё семейство со своим журналом: строки его книг ДРУГИЕ (та же сделка получает другую марж…
- L797 `PAIR_ARTIFACT = os.path.join(OUT, 'DCA-pair.json')`
- L800 `floor_of(ruler)` — Пол билета РЕЖИМА: биржевой минимум, переведённый в маржу.
- L812 `peak_of(ruler)` — Пик РЕЖИМА, измеренный по журналу. Неизвестный режим — пик пула.
- L822 `ticket(deposit, ruler)` — Билет книги: не меньше пола режима и не больше доли на все места.
- L835 `slots(deposit, ruler)` — Сколько мест помещается в депозит при билете этой книги.
- L840 `share(deposit, ruler)` — Доля счёта на позицию — ровно билет, выраженный долей.
- L845 `ahead(decided_at, written_at, hours=AHEAD_H, since=None)` — Записано ли решение вперёд, а не восстановлено пересчётом.
- L863 `shard_of(path, at=None)` — Файл журнала, в который идёт решение: СУТКИ по метке решения.
- L881 `shard_day(at=None)` — Дата суток решения строкой — ключ ротации, один на всех.
- L896 `SHARD_CAP = 4 * 1024 * 1024` — Порог ЧАСТИ суточного файла. Сутки оказались единицей недостаточной: одно решение живёт во всех книгах разом…
- L899 `shard_parts(path, at=None, day=None)` — Части суток по порядку: `journal-<дата>.jsonl`, затем `.01`, `.02`…
- L913 `shard_place(path, day, lines, cap=SHARD_CAP)` — Разложить строки суток по частям, не переступая порог.
- L938 `journal_parts(path=JOURNAL)` — Все куски журнала: старый цельный файл и суточные, по порядку.
- L950 `journal_key(r)` — Ключ решения: тем же составом, каким дедуплицирует запись.
- L962 `read_journal(path=JOURNAL, stats=None)` — Строки журнала как есть — из ВСЕХ его кусков, БЕЗ повторов.
- L999 `split_rows(rows, hours=AHEAD_H)` — Наблюдение и пересчёт — ДВА списка, и складывать их нельзя.
