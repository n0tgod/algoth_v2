...
## research/a1_universe/funding_refresh.py · 222 строк
...
- L48 `read_rows(path)`
- L58 `last_day(rows)`
- L64 `merge(old, new)` — Объединение по ВРЕМЕНИ: новая точка побеждает старую с той же меткой.
- L72 `refresh_symbol(sym, today, fetch=None, read=None)` — Хвост одного символа. Возвращает (было, добавлено, край, ошибка).
- L96 `write_tmp(rows, tmp)`
- L104 `traded_now(log=print)` — Символы, которые площадка торгует СЕЙЧАС.
- L131 `symbols(assets, extra=None)`
- L140 `run(syms, today, workers=B.WORKERS, log=print, fetch=None)`
- L164 `report(s)`
- L179 `publish(name)`
- L185 `main(argv=None)`

## research/a1_universe/instruments_refresh.py · 183 строк
...
- L33 `OUT = B.OUT`
- L34 `PATH = os.path.join(OUT, 'instruments.json')`
- L37 `read_old(path=None)`
- L48 `merge(old, fresh)` — Свежая запись побеждает, старая не исчезает.
- L64 `write(data, path=None)` — Класс B: сперва временный файл, потом атомарная замена.
- L73 `launches(path=None)` — Символ → момент листинга (секунды). Одно место на весь проект.
- L93 `age_days(launch, sym, at)` — Возраст имени на МОМЕНТ РЕШЕНИЯ, сутки. Нет даты — None, не ноль.
- L101 `launch_days(data, at=None)` — Символ → возраст в сутках на момент `at`. Нет даты — символа нет.
- L115 `run(log=print, collect=None, path=None)`
- L139 `report(s)`
- L154 `publish(name)`
- L159 `main(argv=None)`
...
## research/b1_book/collect.py · 7369 строк
...
- L697 `class Collector`
  ...
  - L3617 `Collector.dca_marks(self, dep=None, ruler=None)` — Живая переоценка открытых позиций DCA-книги — частый опрос.
  - L3697 `Collector.dca_paper(self, dep=None, ruler=None, full=None)` — Бумажные DCA-книги: свод из артефакта, сделки из журнала.
  - L3927 `Collector._dca_family(self, DR, now, art_attr, journal_attr, family, what)` — Свод СЕМЕЙСТВА книг из его артефакта — одним кодом на все.
  - L3967 `Collector.paper_book(self, at=None)` — Бумажная месячная книга: свод из артефакта, транши из журнала.
  - L4054 `Collector.learning(self)` — Умнеет ли модель и переходит ли это в деньги — по дням.
  - L4152 `Collector.book_days(self, hz)` — Дневная статистика ОДНОЙ книги — по просьбе владельца.
  - L4269 `Collector._day_cell(rows)` — Числа одной клетки «день × рука». Одно определение на день, на итог и на обе руки: три реализации одного счёт…
  - L4301 `Collector.market_vol(self)` — Волатильность рынка по часам — из наших же почасовых сводок.
  - L4394 `Collector.vol_vs_models(self)` — Влияет ли волатильность рынка на результат книг.
  - L4549 `Collector.model_glossary(self)` — Справочник: какие ситуации модель вообще способна читать.
  - L4654 `Collector.model_tournament(self)` — Полный лист турнира политик: все 72 ветки и селектор.
  - L4728 `Collector._run_row(r, now)` — Строка прогона для показа: заметка урезается, не выбрасывается.
  - L4745 `Collector._produced(root, rel, now)` — Файл, который роль обязана была оставить.
  - L4767 `Collector.owner_asks(self)` — Чего система ждёт ОТ ВЛАДЕЛЬЦА, и чем это подтверждено.
  - L4809 `Collector.agents_state(self)` — Автономная система: конвейер, границы и что уже построено.
  - L4997 `Collector._cand_live(self, cid)` — Живая книга кандидата: закрытые сделки и деньги по рукам.
  - L5038 `Collector.factory_built(self)` — Что автономная система объявила: механика в корне, книги ветками.
  - L5214 `Collector._cand_decisions(self, cid, rec)` — Множество РЕШЕНИЙ живой книги кандидата.
  - L5235 `Collector.factory_strategy(self, cid)` — Полная карточка ОДНОЙ стратегии автономной системы.
  - L5375 `Collector.model_tree(self)` — Дерево моделей: две руки и их книги, с логикой каждой ветки.
  - L5555 `Collector.entry_px(self, picks)` — Цены входа для выборов, которые их не несут.
  - L5572 `Collector.paths(self, trades, hold_h=None)` — Просадка по каждой сделке — из тех же почасовых сводок.
  - L5593 `Collector.dd_money(trades)` — Просадку в деньги и в доли депозита — ПОСЛЕ расчёта счёта.
  - L5604 `Collector.marks(self, trades)` — Текущая середина по символам открытых сделок.
  - L5622 `Collector.model_marks(self, hz=None)` — Только переоценка открытых сделок — для частого опроса.
  - L5679 `Collector.trade_by_id(self, tid)` — Сделка по короткому id — поиск по всем книгам разом.
  - L5753 `Collector._jsonl(path)`
  - L5810 `Collector._jsonl_trim()`
  - L5820 `Collector.trades(self, sym=None)` — История бумажных сделок и сводка — по требованию, не в опросе.
  - L5848 `Collector.disk_view(self)` — Диск в человеческих единицах, с запасом хода в сутках.
  - L5871 `Collector.diskstat(self)` — Сколько занято, с какой скоростью растёт и надолго ли хватит.
  - L5903 `Collector.statuser(self)`
  - L5923 `Collector.reporter(self)` — Строка в журнал раз в минуту: прогон, который молчит, неотличим от повисшего.
  - L5947 `Collector.metrics_poll(self)` — Funding, открытый интерес и базис — раз в 5 минут, один запрос на все символы. Ставка и интерес доказали ценн…
  - L5971 `Collector.sit_load_positions(self, books)` — Открытые позиции КАЖДОЙ книги сканера, без исключений.
  - L5990 `Collector.sit_watch(self)` — Живой сторож выходов ситуационной книги.
  - L6158 `Collector.sit_noise(self, sym, now)` — Живой шум монеты: минутный размах середины, б.п. (v12).
  - L6225 `Collector.sit_absorb_now(self, mdir)` — Живое поглощение событий книги: pnl сразу после закрытия.
  - L6269 `Collector._sit_scan(self, root, sheet, want, books, now, armed)` — Один тик сканера входов: лист сечения против живых цен.
  - L6593 `Collector.brake_watch(self)` — Дневной тормоз: реализованный день торгуемых книг против порога −1 % суммарного капитала (`trades.DAY_BRAKE_S…
  - L6657 `Collector.run(self, hours)`
- L6693 `sit_scan_entry(row, mid, wave_bp, min_edge, min_rr, min_disc, n…` — Живой вход по ситуации: якорим прогноз листа к живой цене.
- L6838 `sit_cross(side, entry_px, adv, mid, fav=None, hi=None, lo=None)` — Дошёл ли живой ход цены до обещанного уровня.
- L6894 `take_limit_fill(side, entry_px, fav, hi, lo)` — Цена исполнения тейка-лимитки, если принты прошли уровень.
- L6930 `sit_exit_event(pos, mid, hi, lo, now)` — Событие живого выхода по уровню — или None.
- L6963 `sit_watched(want, root)` — Каталоги книг, у которых 5-секундный сторож ведёт УРОВНИ.
- L6981 `sit_open_levels(picks, reviews, entries=None)` — Открытые позиции ситуационной книги с уровнями против.
- L7031 `_unfinished(rows)` — Записи об открытии, у которых нет парного закрытия.
- L7043 `warm_start(root, symbols, collector, log, hours=4, trade_hours=…` — Поднять историю из собственных файлов сборщика.
- L7144 `stable_token(root)` — Ключ доступа, переживающий перезапуск.
- L7172 `selftest(root)` — Прогнать поддельный поток через путь записи и показать итог.
- L7212 `dropped_symbols(root, syms, days=3)` — Символы, по которым на диске есть свежие ряды, а в запуске их нет.
- L7255 `main()`
...
## research/b1_book/web.py · 12115 строк
...
- L5359 `FEATJS = <текст, 98 строк>` — Перевод признаков на человеческий — ОДИН на все страницы, которые его показывают (разбор сделки и справочник)…
- L5465 `TRADEINFO = '<!doctype html><meta charset="utf-8">\…` — Страница разбора ОДНОЙ сделки — просьба владельца: у каждой сделки значок «i», по нему страница, где простыми…
- L5814 `LEARNPAGE = '<!doctype html><meta charset="utf-8">\…` — Справочник — просьба владельца: страница со всеми «стратегиями» модели и подробным объяснением каждой простым…
- L5986 `BOOKDAYS = '<!doctype html><meta charset="utf-8">\…` — Дневная статистика ОДНОЙ книги — просьба владельца: «кликаем на 4-hour book, и открывается страница, где стат…
- L6298 `DCAPAGE = '<!doctype html><meta charset="utf-8">\…` — Бумажная месячная книга (`research/paper_monthly`). Своего показа у неё не было вовсе: книга писала отчёт фай…
- L7773 `PAPERPAGE = '<!doctype html><meta charset="utf-8">\…`
- L8104 `LIVEPAGE = '<!doctype html><meta charset="utf-8">\…`
- L8543 `VOLPAGE = '<!doctype html><meta charset="utf-8">\…`
- L8788 `GLOSSARY_PAGE = '<!doctype html><meta charset="utf-8">\…`
- L9042 `TREEPAGE = '<!doctype html><meta charset="utf-8">\…` — Страница дерева моделей — просьба владельца: разветвление от основных ML и AI, и по каждой ветке простыми сло…
- L9539 `TOURPAGE = '<!doctype html><meta charset="utf-8">\…` — Страница турнира политик — просьба владельца: весь лист веток и подветок отдельной страницей. Данные — артефа…
- L9944 `LEAGUE = '<!doctype html><meta charset="utf-8">\…` — Страница лиги — просьба владельца: наблюдение за каждой стратегией и моделью отдельно (что ведёт себя лучше)…
- L10274 `BUILTPAGE = '<!doctype html><meta charset="utf-8">\…` — Страница автономной системы: конвейер ролей и механических шагов, границы и то, что уже построено. Тексты — и…
- L10630 `STRATPAGE = '<!doctype html><meta charset="utf-8">\…`
- L11057 `ASKSPAGE = '<!doctype html><meta charset="utf-8">\…`
- L11225 `AGENTSPAGE = '<!doctype html><meta charset="utf-8">\…`
- L11755 `serve(collector, port, token, log)` — Поднять сервер наблюдения в отдельном потоке.
...
- L34 `FLAT_MMR = 0.02` — Ставка поддерживающей маржи, когда тиров у имени НЕТ (делистнутая нога, новый листинг без таблицы): §10, мода…
- L37 `liq_price(p_avg, qty, capital, mmr, side='long')` — Цена ликвидации позиции при кросс-марже; сторона параметром.
- L64 `liq_frac(p_ref, p_liq)` — Насколько ниже опорной цены стоит ликвидация, долей (0..1).
- L72 `mmr_for_notional(tiers, notional, flat=None)` — Ставка maintenance margin тира, чей верх нотионала ≥ позиции.
- L90 `lev_cap_for_notional(tiers, notional, flat=None)` — Предел плеча ТИРА площадки — потолок, который биржа не даст перейти.
- L114 `fully_loaded(rung_prices, weights, capital, leverage)` — Состояние ПОЛНОСТЬЮ набранной лестницы при данном плече.
- L138 `max_leverage(rung_prices, weights, capital, base_px, d_max, mmr…` — Максимальное плечо, при котором забор §5 выполняется.
- L205 `sigma_rungs(base_px, sigma_frac, n_rungs, spacing_sig, side='lo…` — Цены рунгов по σ-сетке: база плюс равные шаги в единицах σ.
- L229 `structural_rungs(entry, level_prices, min_gap, n_rungs, side='l…` — Цены рунгов DCA: вход плюс СТРУКТУРНЫЕ уровни против позиции.
- L264 `open_mark(px, avg, capital, leverage, weights_filled, side='lon…` — Отметка ОТКРЫТОЙ лестницы: доля капитала позиции при цене `px`.
- L292 `_fill_rungs(filled, cash, qty, lo, rung_prices, weights, notion…` — Заполнить рунги долива, до которых дошёл крайний ход бара `lo`.
- L325 `simulate_ladder(closes, lows, rung_prices, weights, capital, le…` — Пройти путь цены лестницей доливов вниз. Чистая функция.
...
- L50 `OUT = os.path.join(HERE, 'out')`
- L51 `DEP = 10000`
- L52 `SHORT_BOOK = 'optimal_s'`
- L53 `LONG_BOOKS = ('optimal', 'safe')`
- L57 `SHORT_KEYS = ['fence:none:t2', 'fence:none:t1', 'c3:…` — Ячейки объявлены ДО прогона: три из D12 (лучшая по итогу, цель ×1 и потолок плеча 3×) плюс структурные доливы…
- L58 `DAY = 86400.0`
- L61 `_day(ts)`
- L65 `series(rows, key='exit_ts')` — Дневной ряд денег: сутки UTC по моменту ВЫХОДА.
- L81 `align(a, b)` — Два ряда на ОБЩЕЙ оси суток: пересечение окон, дырки — нули.
- L108 `curve_dd(vals, dep)` — Просадка по дневной кривой счёта, долей депозита.
...
- L54 `ARMS = ('gbm', 'nn')`
- L55 `ARM_TITLE = {'gbm': 'деревья', 'nn': 'сеть'}`
- L56 `MAIN_DEP = 10000`
- L57 `N_MIN = 30`
- L60 `arm_title(arm)`
- L64 `key_of(sym, side, at)` — Ключ соединения: имя, сторона, момент решения с точностью записи.
- L74 `legs_index(legs)` — (имя, сторона, момент) → список (рука, модуль прогноза).
- L84 `cache_owners(legs, log=print)` — Кто владеет записью реплея — по ЧИСЛАМ самой записи.
- L111 `attribute(rows, idx, owners=None, log=print)` — Каждой строке журнала — рука; счётчики причин рядом, не молча.
- L146 `_median(xs)`
- L154 `stats(rows, mid=None)` — Деньги руки: сумма, медиана позиции в % маржи, доля плюсов, половины.
- L174 `window(rows)`
- L184 `by_day(legs, rows, ruler=None, dep=None)` — Активность по суткам: решения листа по руке и строки книги.
- L207 `run(rows=None, legs=None, owners=None, log=print)`
- L270 `verdict(s)` — По книге на главном депозите: у какой руки плюс и держится ли он на обеих половинах окна. Меньше `N_MIN` сдел…
- L293 `_u(x)`
- L297 `_p(x)`
- L301 `report(s)`
- L400 `publish(name)`
...
- L382 `extra_assets(to_asset, funding_dir=None)` — Символы с рядом funding, которых НЕТ в универсуме.
- L401 `context(need=None, log=None)` — Справочник комиссий и ряды funding — ОДИН раз на свод книг.
- L436 `apply_to_rows(rows, ctx, slip_bp=None)` — Издержки — В КАЖДУЮ СДЕЛКУ: деньги строки становятся нетто.
- L514 `net_view(rows, dep, ctx, stats_fn, slip_bp=None)` — Нетто-свод подмножества строк книги: форма по дням от денег за вычетом комиссии и проскальзывания и с funding…
- L561 `run(rows=None, funding=None, assets=None, log=print, slip_bp=No…`
- L623 `verdict(s)` — Из чисел: у каких книг знак держится после комиссии и funding, и помогает ли гейт по ставке (парно, по медиан…
- L647 `_u(x)`
- L651 `_p(x, d=2)`
- L655 `_b(x)`
- L659 `report(s)`
...
## research/dca_paper/pair_age.py · 363 строк

Возраст имени как фильтр входа короткой стороны общего счёта.

- L37 `HERE = os.path.dirname(os.path.abspath(__file_…`
- L38 `ROOT = os.path.dirname(os.path.dirname(HERE))`
- L49 `DAYS = (0, 3, 7, 14, 30, 60)`
- L52 `BAND_EDGES = (0.0, 3.0, 7.0, 14.0, 30.0, 60.0, float…` — Полосы возраста для разреза «почему»: те же границы, что у порогов, чтобы таблица фильтра и таблица механизма…
- L53 `BAND_NAMES = ('<3 сут', '3–7 сут', '7–14 сут', '14–3…`
- L55 `UNKNOWN = 'возраст неизвестен'`
- L58 `TAIL_EXITS = ('пол', 'ликвидация')` — Исходы, которые и делают хвост короткой стороны (замер `short_why`): пол капитуляции и ликвидация. Тейк и сро…
- L59 `SEEDS = 200`
- L60 `SEED = 20260908`
- L61 `INSTR = IR.PATH`
- L64 `launches(path=None)` — Символ → момент листинга. Читает ЯДРО справочника, своей копии нет.
- L69 `age_days(launch, sym, at)` — Возраст имени на момент решения, сутки. Нет даты — None.
- L74 `band_of(age)` — Полоса возраста. Нет даты листинга — своя полоса, а не «старое».
- L84 `bands(rows, launch)` — Исполненные короткие сделки по возрасту имени — механизм фильтра.
- L130 `pick(shorts, launch, min_days, seed=SEED, n_random=None)` — Решения после фильтра возраста плюс счётчики отказов.
- L158 `run(dep=None, log=print, ctx=None, long_cache=None, short_cache…`
- L233 `_u(x)`
- L237 `_p(x, d=1)`
- L241 `report(s)`
- L326 `publish(name)`
...
- L64 `SEED = 20260907`
- L69 `SEEDS = 20` — Контроль размера считается на МНОЖЕСТВЕ зёрен: одна случайная выборка сама по себе шум — на первом прогоне дв…
- L72 `held_intervals(rows, dep)` — Что длинная книга ДЕРЖАЛА: имя → список (вход, выход).
...
## research/dca_paper/rules.py · 1138 строк
...
- L501 `FAMILY_RULES = {'pair': 5, 'h24': 1}` — Версия правил СЕМЕЙСТВА. Общая версия `RULES` тут не годится: её смена обнулила бы запись длинных и коротких…
- L507 `FAMILY_SINCE = {'pair': '2026-09-08', 'h24': '2026-09-…` — ДЕНЬ смены правил семейства. Смена версии обнуляет «записанное вперёд»: решения, писанные другими правилами,…
- L510 `family_since(key)` — С какого дня у семейства книги идёт запись вперёд. Нет — None.
- L515 `family_rules(key)` — Версия правил семейства книги. Нет версии — нет и поля в строке.
- L520 `pair_share_mult(pair_key, book_key)` — Во сколько раз билет СТОРОНЫ меньше её собственного в общем счёте.
- L531 `ticket_in(pair_key, book_key, deposit)` — Билет стороны в общем счёте: свой билет × доля, но НЕ НИЖЕ пола.
- L544 `share_in(pair_key, book_key, deposit)` — Доля счёта на позицию стороны в общем счёте.
- L549 `parts_of(key)` — Из каких книг собран общий счёт. Обычная книга — источник один.
- L562 `RETIRED_AT = {'safe_s': '2026-09-07', 'optimal_s': '…` — --- снятые книги (решение владельца 2026-09-07) ------------------------ Зеркала `*_s` на ситуационном листе…
- L566 `retired(key)` — Снята ли книга. Дата снятия — в `RETIRED_AT`, одним местом.
- L575 `RULER_ORDER = [k for k in RULER_ORDER_ALL if not reti…` — Действующий порядок: снятая книга выпадает отсюда, и ВСЕ читатели — прогон, свод, отчёт, страница — перестают…
- L578 `family_of(key)` — Семейство книги: `sit` — по листу ситуационной, `h24` — по `h24`.
- L583 `order_of(family='sit', with_retired=False)` — Порядок книг семейства — один список на всех читателей.
- L595 `hold_of(key)` — Срок удержания книги, ч. Не объявлен — общий срок книг.
- L600 `DEFAULT_RULER = 'optimal'` — Запись БЕЗ поля `ruler` писана этой линейкой: до 2026-09-04 книга была одна и считалась глубиной. Умолчание д…
- L603 `ruler_of(row)` — Линейка строки журнала; у строк прежнего образца поля нет.
- L609 `notional_of(row)` — Нотионал позиции = маржа × плечо; None, если чего-то из двух нет.
- L625 `avg_walk(fills, entry=None, notional=None, take_frac=None, side…` — Плавающая ТВХ: средняя цена входа ПОСЛЕ каждого рунга.
- L720 `open_stats(positions)` — ХУДШАЯ из открытых позиций — та, что просела глубже всех СЕЙЧАС.
- L747 `ruler_title(key)`
- L751 `side_of(key)` — Сторона книги. Ключ без поля — длинная: до 2026-09-05 других нет.
- L762 `row_side(row, ruler=None)` — Сторона ЗАПИСИ позиции — одно правило на всех читателей.
- L782 `min_lev_of(key)` — Порог входа по плечу у режима; НЕТ поля — гейта нет вовсе.
- L800 `ONE_PER_NAME = True` — --- биржевое правило, которое реплей D-серии НЕ соблюдал --------------- На одном счёте в одностороннем режим…
- L809 `AHEAD_H = HOLD_H + 48` — --- «записано вперёд» --------------------------------------------------- Решение считается записанным ВПЕРЁД…
- L811 `HERE = os.path.dirname(os.path.abspath(__file_…`
- L812 `OUT = os.path.join(HERE, 'out')`
- L813 `JOURNAL = os.path.join(OUT, 'journal.jsonl')`
- L814 `ARTIFACT = os.path.join(OUT, 'DCA-paper.json')`
- L818 `H24_JOURNAL = os.path.join(OUT, 'short.jsonl')` — Журнал и артефакт семейства `h24` — СВОИ. Имя не «journal-…»: ротация длинной книги глобит `journal-*.jsonl`,…
- L819 `H24_ARTIFACT = os.path.join(OUT, 'DCA-short.json')`
- L824 `PAIR_JOURNAL = os.path.join(OUT, 'pair.jsonl')` — Общий счёт — тоже своё семейство со своим журналом: строки его книг ДРУГИЕ (та же сделка получает другую марж…
- L825 `PAIR_ARTIFACT = os.path.join(OUT, 'DCA-pair.json')`
- L833 `RISK_LIMITS = os.path.join(os.path.dirname(HERE), 'a1…` — Таблица риск-лимитов площадки (тиры maintenance margin) — данные ПЛОЩАДКИ, не правило книги, но читаются они…
- L835 `_TIERS = {'at': None, 'data': {}}`
- L838 `risk_tiers(path=None)` — Тиры площадки: символ → ступени. Перечитывается по времени файла.
- L855 `_ladder()` — Ядро лестницы. Импорт ленивый: страница не тянет его без нужды.
- L865 `mmr_look(sym, path=None)` — Функция «нотионал → ставка поддерживающей маржи» для имени.
- L879 `liq_walk(steps, lev, side='long', look=None, mmr=None)` — Цена ликвидации ПОСЛЕ каждого рунга — ядром лестницы, не копией.
- L907 `journal_of(key)` — Журнал СЕМЕЙСТВА книги. Одно место на всех читателей.
- L923 `artifact_of(key)` — Артефакт СЕМЕЙСТВА книги: открытые позиции живут в нём, не в журнале.
- L933 `floor_of(ruler)` — Пол билета РЕЖИМА: биржевой минимум, переведённый в маржу.
- L945 `peak_of(ruler)` — Пик РЕЖИМА, измеренный по журналу. Неизвестный режим — пик пула.
- L955 `ticket(deposit, ruler)` — Билет книги: не меньше пола режима и не больше доли на все места.
- L968 `slots(deposit, ruler)` — Сколько мест помещается в депозит при билете этой книги.
- L973 `share(deposit, ruler)` — Доля счёта на позицию — ровно билет, выраженный долей.
- L978 `ahead(decided_at, written_at, hours=AHEAD_H, since=None)` — Записано ли решение вперёд, а не восстановлено пересчётом.
- L996 `shard_of(path, at=None)` — Файл журнала, в который идёт решение: СУТКИ по метке решения.
- L1014 `shard_day(at=None)` — Дата суток решения строкой — ключ ротации, один на всех.
- L1029 `SHARD_CAP = 4 * 1024 * 1024` — Порог ЧАСТИ суточного файла. Сутки оказались единицей недостаточной: одно решение живёт во всех книгах разом…
- L1032 `shard_parts(path, at=None, day=None)` — Части суток по порядку: `journal-<дата>.jsonl`, затем `.01`, `.02`…
- L1046 `shard_place(path, day, lines, cap=SHARD_CAP)` — Разложить строки суток по частям, не переступая порог.
- L1071 `journal_parts(path=JOURNAL)` — Все куски журнала: старый цельный файл и суточные, по порядку.
- L1083 `journal_key(r)` — Ключ решения: тем же составом, каким дедуплицирует запись.
- L1095 `read_journal(path=JOURNAL, stats=None)` — Строки журнала как есть — из ВСЕХ его кусков, БЕЗ повторов.
- L1132 `split_rows(rows, hours=AHEAD_H)` — Наблюдение и пересчёт — ДВА списка, и складывать их нельзя.

## research/dca_paper/run_pair.py · 526 строк

Общий счёт: длинная книга и короткая на ОДНОМ депозите.

- L47 `HERE = os.path.dirname(os.path.abspath(__file_…`
- L48 `ROOT = os.path.dirname(os.path.dirname(HERE))`
- L63 `MEM_LIMIT_MB = 1200` — Предел памяти прогона. На VPS 7.7 ГБ без свопа, и тяжёлый прогон рядом с часовым циклом убивает НЕ СЕБЯ (OOM…
- L66 `long_recs(cache=None, log=print)` — Позиции длинных книг из кэша реплея `run_paper`, по книгам.
- L95 `short_recs(cache=None, log=print)` — Позиции коротких книг `h24` из их кэша, по книгам семейства.
- L113 `pack(longs, shorts, keys=None)` — Решения общей книги: обе стороны в одном списке, с меткой источника.
- L129 `gate_shorts(shorts, pk, ctx, log=print)` — Короткие решения под гейтом по ставке funding — правило книги.
- L177 `collisions(rows, ruler)` — Имена, которые общий счёт держит РАЗОМ длинной и короткой.
- L194 `_side_series(rows, ruler, side)`
- L198 `link(rows, ruler)` — Связь дневных денег сторон ВНУТРИ общего счёта.
...
## research/dca_paper/run_paper.py · 1440 строк
...
- L522 `_stats(rows, deposit)` — Итог, просадка и форма по дням — на ЭТОМ подмножестве строк.
- L598 `_book_costs(rows)` — Что вычтено у книги: суммы издержек и число неизмеренных сделок.
- L643 `summarize(path=None, live=None, keys=None, ctx=None)` — Свод по книгам: ОДНА кривая, и в ней помечено, что бэктест.
- L744 `_pct(x, d=2)`
- L748 `_tail_words(s)` — Числа хвоста словами. Нет чисел — так и сказано, а не ноль.
- L783 `costs_block(s)` — Раздел «издержки» — ОДИН на весь отчёт: деньги ниже уже нетто.
- L869 `report(s)`
- L1227 `rules_snapshot(keys=None)` — Правила прогона в свод: страница описывает ИМИ то, что видит.
- L1261 `publish(name)`
- L1267 `main()`

## research/dca_paper/run_short.py · 349 строк
...
- L57 `CELL = ('fence:none:t2', 'fence', 'none', 't2')` — Ячейка сетки D10, которой торгует семейство: плечо забора, без доливов, цель ×2. Объявлена здесь одним местом…
- L61 `BOOKS = {'safe_h': 'safe_s', 'optimal_h': 'opti…` — Книга → линейка забора, на которой считается её позиция. «Агрессивная» считается на той же линейке, что «опти…
- L62 `ARMS = ('gbm', 'nn')`
...
- L88 `legs(arms=ARMS, limit=None, log=print, path=None)` — Короткие выборы `h24` ОБЕИХ рук: решение владельца «оставить обе».
- L103 `needs_replay(cache, legs_)` — Какие решения считать заново: новых нет в кэше, открытые стареют.
- L116 `read_cache(path=None, log=print)` — Кэш реплея семейства. Непригодный не чинится молча.
- L130 `write_cache(cache, path=None)`
- L137 `replay(need, src=None, log=print)` — Досчёт недостающих решений: одна ячейка, отметки и заполнения.
- L170 `run(limit=None, src=None, log=print, legs_=None, journal=None, …`
- L232 `_p(x, d=2)`
- L236 `_u(x)`
- L240 `report(s)`
- L315 `publish(name)`
...
- L39 `HERE = os.path.dirname(os.path.abspath(__file_…`
- L40 `ROOT = os.path.dirname(os.path.dirname(HERE))`
- L54 `DAYS = (0, 7)` — Оси объявлены ДО прогона. Порог возраста — уже объявленный (7 суток) и ноль как «как сейчас»; доли билета — т…
- L55 `SHARES = (1.0, 0.5, 0.25)`
- L56 `SEEDS = 200`
- L57 `SEED = 20260908`
...
- L108 `supply(recs, bk, ctx, launch, days=14, dep=None, now=None)` — Подача листа по суткам: что предложено, что срезал возраст, что взято.
- L147 `run(log=print, ctx=None, cache=None, keys=None, now=None, seeds…`
- L224 `run_supply(log=print, ctx=None, cache=None, keys=None, now=None…` — Подача по суткам у всех книг семейства.
- L245 `supply_report(s)`
- L285 `_u(x)`
- L289 `_p(x, d=1)`
- L293 `report(s)`
- L363 `publish(name)`
...
- L46 `DEP = 10000.0`
- L49 `LEV_EDGES = (0.0, 3.0, 5.0, 10.0, 20.0, 1000000000.…` — Полосы плеча объявлены до прогона и одни на все книги: сравнивать книги можно только на общей сетке.
- L50 `LEV_NAMES = ('<3×', '3–5×', '5–10×', '10–20×', '≥20…`
- L53 `SHARES = (1.0, 0.75, 0.5, 0.25, 0.1)` — Доли билета короткой стороны для сетки «выровнять кривую». Единица — нынешняя книга; ниже — та же книга меньш…
- L56 `load(path, ruler, dep=DEP, ctx=None)` — Строки книги ТЕКУЩИХ правил, деньги — НЕТТО (издержки в сделке).
- L68 `by_exit(rows)` — Деньги по исходу. Ликвидация — своя строка, а не «прочее».
- L90 `by_lev(rows)` — Деньги по полосе плеча — на объявленной сетке.
- L116 `concentration(rows, dep=DEP)` — Сколько дают худшие позиции и худший день. Хвост трёх сделок и хвост всей книги лечатся разным, и различает и…
- L146 `funding_top(rows, k=5)` — Позиции, которые заплатили funding больше всех: чем они особенны.
- L165 `pair_grid(long_rows, short_rows, shares=SHARES, dep=DEP)` — «Выровнять кривую»: пара при УМЕНЬШЕННОМ билете короткой стороны.
- L188 `run(dep=DEP, log=print, short_path=None, long_path=None, ctx=No…`
- L230 `_u(x)`
- L234 `_p(x, d=1)`
- L238 `report(s)`
- L327 `publish(name)`
...
## research/factory/candidate.py · 248 строк
...
- L45 `DAY = 86400.0`
- L50 `RR_LO_MAX = 1.5` — Полоса отношения: «низкое» и «высокое» — те же края, которыми живут книги sit_lo и sit (потолок 1.5 против по…
- L51 `RR_HI_MIN = 2.0`
- L54 `passes(lg, rule)` — Проходит ли нога гейты правила (без учёта мест и согласия).
- L80 `agreed_keys(legs)` — Ключи (цель, час, имя, сторона), которые выбрали ОБЕ руки.
- L101 `order_value(lg, rule)` — Чем меряется место в очереди за слотом.
- L115 `weight(lg, rule)` — Вес ноги в дневном итоге по правилу размера.
- L147 `simulate(legs, outs, rule)` — Сделки книги кандидата.
- L206 `geometry(rule)` — Ось геометрии → тройка (стоп, тейк, предел возраста) турнира.
- L218 `with_geometry(rule)` — Правило плюс поля геометрии, которых ждёт `simulate`.
- L226 `daily_net(trades)` — День выхода → взвешенный нетто книги в б.п. гросса.
- L243 `trade_counts(trades)`
...
## research/factory/declare.py · 166 строк
...
- L43 `_day(ts)`
- L47 `read_json(path)`
- L57 `gate(ceil, prop, run_at, now, state, ceil_at=None)` — Можно ли объявлять. Возвращает (правило, причина отказа).
- L107 `main(argv=None)`

## research/factory/horizon.py · 424 строк

Горизонт цели в листе сечения: что лист НЕСЁТ и как из этого ноги.

- L50 `HERE = os.path.dirname(os.path.abspath(__file_…`
- L51 `RESEARCH = os.path.dirname(HERE)`
- L59 `SHEETS = os.path.join(RESEARCH, 's8_loop', 'out'…` — Журнал листов сечения. Путь живёт ЗДЕСЬ и один: суточный прогон, объявление и приёмка обязаны смотреть в один…
- L66 `BASE_TARGET = 'fwd_4h'` — Базовая цель — та, которую читает `tournament.legs_from_sheets` без всякого перевода полей. Ноги старого обра…
- L72 `CANON = ('fwd', 'fwd_z', 'mae', 'mfe', 'mae_q',…` — Канонические имена, которых ждёт `tournament._leg`. Перечислены здесь затем, чтобы вид строки под другой гори…
- L78 `FIELDS = {'fwd_4h': {'fwd': 'fwd', 'fwd_z': 'fwd…` — Поля горизонта в строке листа. Имена 24 ч — дословно те, которые задание велит роли `fix` дописать в `scan_sh…
- L87 `REQUIRED = ('fwd', 'mae', 'mfe')` — Без этих трёх цели в строке нет вовсе: `_leg` вернул бы None, а подставить их неоткуда — соседний горизонт ес…
- L93 `TAIL_RECORDS = 24` — Сколько последних листов читается, чтобы ответить, что лист несёт СЕЙЧАС. Число диагностическое: решение прин…
- L94 `TAIL_STEP = 1 << 20`
- L95 `TAIL_MAX = 64 << 20`
- L98 `_tn()` — Турнир политик — ленивым импортом (см. шапку модуля).
- L109 `leg_target(lg)` — Горизонт ноги. Нога без поля — базовая: она читала его поля.
- L116 `row_caps(row)` — Что несёт ОДНА строка листа: цель → (порядок в σ, квантили).
- L135 `record_caps(rec)` — Что несёт один ЛИСТ: цель → {строк, порядок в σ, квантили}.
- L156 `_tail_lines(path, n)` — Последние `n` полных строк файла. (строки, причина пустоты).
- L188 `sheet_caps(path, tail=TAIL_RECORDS)` — Что лист сечения несёт СЕЙЧАС — по содержимому, а не по константе.
- L236 `targets_of(caps)` — Цели, которые лист несёт. Нет разбора — объявленный образец.
- L252 `phrase(caps)` — Фраза о листе, ВЫВЕДЕННАЯ из чисел, а не стоящая рядом с ними.
- L273 `report_lines(caps)` — Таблица «что несёт лист» для отчёта суточного прогона.
- L302 `_view(row, target)` — Строка листа глазами ОДНОЙ цели, или None.
- L322 `_records(paths, log=print)` — Листы журнала по порядку. Битая строка пропускается молча — так же, как её пропускает существующий загрузчик.
- L341 `_legs_for(paths, target, TN, log=print)` — Ноги ОДНОЙ небазовой цели тем же `_leg`, что у базовой.
- L363 `_order(g)` — Очередь ног — та же, что у существующего загрузчика, плюс горизонт последним ключом: при одном горизонте поря…
- L372 `legs_from_sheets(paths, caps=None, log=print, stats=None)` — Ноги ВСЕХ целей, которые несёт лист.
- L408 `supply_gap(caps, stats)` — Цель есть в листе, а ног из неё ноль — причина отказа или None.
...
## research/factory/run_day.py · 799 строк
...
- L50 `OUT = os.path.join(HERE, 'out')`
- L51 `PROPOSALS = os.path.join(HERE, 'proposals.jsonl')`
- L58 `PROPOSAL_NAME = 'proposal.json'` — Заявка предлагающего — та самая, которую судит ПОТОЛОК, и она ещё не объявлена: между ней и реестром стоит ша…
- L59 `NULL_SEEDS = 10`
- L60 `DAY = 86400.0`
- L65 `read_proposals(path=None)` — Предложения ассистента: правило и, необязательно, довод.
- L99 `pending_rule(state, path, caps=None)` — Заявка, которую судит потолок: (правило, причина отсутствия).
- L136 `run_pending(rule, legs, outs)` — Числа заявки тем же реплеем, что у живых кандидатов.
- L149 `declare_today(base, now, seed, log=print, per_day=None, caps=No…` — Объявить предложенных и добрать контрольную руку жребием.
- L182 `declare_rules(base, now, seed, fresh, log=print, per_day=None, …` — Объявить названные правила и добрать контрольную руку жребием.
- L225 `needed_legs(legs, rules, log=print)` — Ноги, которые возьмёт ХОТЯ БЫ ОДИН живой кандидат.
- L241 `record_days(legs)` — Длина ЗАПИСИ в сутках — сколько календарных суток вообще есть в журнале листов сечения.
- L271 `load_legs(sheets, log=print, caps=None, stats=None)` — Ноги журнала листов — по каждой цели, которую лист несёт.
- L287 `outcomes_for(legs, root, geoms, log=print)` — Исходы всех ног при всех нужных геометриях.
- L319 `_adv(lg, stop)`
- L325 `_fav(lg, take)`
- L329 `geometries()` — Все тройки геометрии, какие может попросить пространство.
- L343 `LAST_TRADES = 40` — Сколько последних сделок кандидата кладётся в артефакт. Полный список — десятки тысяч строк на кандидата: арт…
- L346 `run_candidates(state, legs, outs, log=print, caps=None)` — Сделки и дневной нетто по каждому живому кандидату.
- L391 `null_daily(legs, outs, rule, seeds=NULL_SEEDS)` — Дневной нетто книги на ПЕРЕМЕШАННЫХ внутри часа исходах.
- L434 `null_median(nulls, now, window_d=PL.WINDOW_D)` — Медиана нуля за окно вылета — то число, с которым сравнивается кандидат. Одно на группу, а не на книгу.
- L451 `_med(xs)`
- L459 `verdict(sel, ctl, n_days)` — Фраза вердикта ВЫВОДИТСЯ из чисел, а не стоит рядом с ними.
- L485 `write_report(path, meta, cands, st, nulls_med, log=print, pendi…`
- L637 `publish(path, log=print, msg='фабрика: суточный прогон')` — Публикация — часть прогона, а не отдельный шаг: шаг, который можно забыть, рано или поздно забывают (урок D1).
- L653 `main(argv=None)`

## research/factory/runlog.py · 1169 строк
...
- L575 `SCOUT_MIN = {'title': 8, 'claim': 40, 'mechanism': …`
- L577 `SCOUT_MAX_IDEAS = 5`
- L578 `SCOUT_MIN_WHY = 120`
- L579 `SCOUT_SEEN = 'scout.jsonl'`
- L582 `scout_seen(base, before=None)` — Заголовки уже принесённых идей. Журнал ведёт машина.
- L621 `scout_record(text, base)` — Дописать принесённое в журнал. Возвращает число записанных.
- L666 `check_scout(text, seen=())` — Меню разведчика проверяемо? Возвращает (годно, список бед).
- L725 `check_needs_owner(d)` — Форма просьб к владельцу в отчёте роли. Возвращает список бед.
- L756 `_owner_asks(out, d, src)` — Записать просьбы к владельцу. Молча не теряем ни одной.
- L765 `_close_mechanism(out, d)` — Отметить механику построенной либо упершейся в владельца.
- L785 `OUT_REL = 'research/factory/out'`
- L788 `check_role(role, root, since=None, out=None, record=False)` — Контракт роли: выполнен ли. Возвращает (годно, список бед).
- L930 `BUILD_MIN_WHY = 120`
- L940 `BUILD_MAX_CONTROLS = 24` — Предел числа подделок и общий бюджет времени на их проверку. Первая версия ставила предел 8 и ОТВЕРГЛА починк…
- L941 `BUILD_CONTROLS_BUDGET = 1800`
- L943 `BUILD_TEST_TIMEOUT = 900` — Сколько ждать прогон тестов кандидата.
- L946 `_run_tests(root, tests)` — Прогнать тесты кандидата. Возвращает (прошли, вывод).
- L979 `_own_path(rel, roots)` — Путь принадлежит одному из своих каталогов и не вылезает наружу.
- L986 `mech_dir(out)` — Каталог механики, лежащей СЕЙЧАС в задании строителя.
- L1001 `check_build(text, root, out_dir=None)` — Постройка годна? Возвращает (годно, список бед).
- L1115 `ADV_MIN_TRIES = 3`
- L1116 `ADV_MIN_HOW = 40`
- L1117 `ADV_MIN_WHY = 100`
- L1118 `ADV_VERDICTS = ('veto', 'pass', 'undetermined')`
- L1121 `check_adversary(text, root)` — Разбор адверсария годен? Возвращает (годно, список бед).
...
## research/factory/space.py · 307 строк
...
- L63 `GEOMETRY = {'timer': ('no', False, 24), 'stop_take…` — Геометрия сделки одной таблицей: (стоп, тейк, предел возраста). Живёт ЗДЕСЬ, в модуле пространства, а не рядо…
- L70 `AXIS_NAMES = tuple((a for a, _ in AXES))`
- L71 `VALUES = dict(AXES)`
- L76 `TOTAL = 1` — Полный перебор. Число закреплено ЛИТЕРАЛОМ в проверке: оно и есть знаменатель испытаний фабрики, и молчаливое…
- L82 `_SHORT = {'target': {'fwd_4h': 'h4', 'fwd_24h': …` — Короткие ярлыки для ключа кандидата: ключ уезжает в имя каталога и в адрес страницы, поэтому только латиница,…
- L94 `validate(rule)` — Строка параметров годна? Возвращает причину отказа или None.
- L115 `key(rule)` — Ключ кандидата — из САМОГО правила, а не из счётчика.
- L137 `describe(rule, lang='ru')` — Объяснение книги ИЗ ПАРАМЕТРОВ — страница обязана его печатать.
- L173 `_rand(seed, i)` — Зерно выводится ЧИСЛОМ, а не `hash()`: хеш строки в Python солится на каждый процесс, и нуль, который нельзя…
- L182 `index_to_rule(idx)` — Номер сочетания → правило. Порядок осей объявлен, значит номер и правило переводятся друг в друга однозначно.
- L192 `draw(seed, n, exclude=(), available_only=True, caps=None)` — `n` РАЗЛИЧНЫХ правил равномерно случайно из пространства.
- L238 `DECLARED_SHEET = {'targets': {'fwd_4h': {'z': True, 'q':…` — --- что из пространства сегодня исполнимо --------------------------- Пространство объявлено целиком и не сжи…
- L239 `SHEET_TARGETS = tuple(DECLARED_SHEET['targets'])`
- L242 `unavailable(rule, caps=None)` — Причина, по которой правило сегодня не исполнимо, или None.
- L295 `available_total(caps=None)` — Сколько сочетаний исполнимо сегодня — знаменатель, который печатается рядом с числом потраченных испытаний.
...
## research/mech_12cc2578/_ask.py · 61 строк

Дописать в отчёт постройки просьбу к владельцу — ДОСЛОВНЫМ повтором уже открытой просьбы `4c923cf1`.

- L17 `HERE = os.path.dirname(os.path.abspath(__file_…`
- L18 `ROOT = os.path.dirname(os.path.dirname(HERE))`
- L23 `ASKS = os.path.join(ROOT, 'research', 'factory…`
- L24 `BUILD = os.path.join(ROOT, 'research', 'factory…`
- L26 `WHY = 'Шаг (2) задания построен и проверен (r…`
- L38 `main()`

## research/mech_12cc2578/controls_check.py · 97 строк
...
## research/mech_13a67a67/age_mode.py · 611 строк

Механика 13a67a67 — возраст имени как РЕЖИМ ПРОГНОЗА.

- L64 `HERE = os.path.dirname(os.path.abspath(__file_…`
- L65 `RESEARCH = os.path.dirname(HERE)`
- L77 `BAND_NAMES = PA.BAND_NAMES` — --- чужие величины: берутся У ХОЗЯЕВ, здесь не назначаются -------------
- L78 `BAND_EDGES = PA.BAND_EDGES`
- L79 `UNKNOWN = PA.UNKNOWN`
- L84 `YOUNG_MAX_D = BAND_EDGES[2]` — Объявленные полосы вердикта. Оба числа — ГРАНИЦЫ `pair_age`, а не новые пороги: заявка сравнивает «моложе 7»…
- L85 `OLD_MIN_D = BAND_EDGES[5]`
- L86 `YOUNG = f'моложе {YOUNG_MAX_D:g} сут'`
- L87 `OLD = f'старше {OLD_MIN_D:g} сут'`
- L88 `SEED_PICK = PA.SEED`
- L89 `SEEDS = PA.SEEDS`
- L93 `MIN_CHOICES = 100` — --- своё, объявлено ДО прогона ---------------------------------------- Убийца (0): полоса тоньше этого — «не…
- L95 `MIN_DIFF_PP = 5.0` — Убийца (1): разность долей попаданий (старые − молодые), п.п.
- L98 `MAX_BEAT = 0.1` — Убийца (3): доля зёрен, где случайная выборка того же размера бьёт фильтр возраста. Порог не новый — им приня…
- L99 `BOOT = 2000`
- L100 `CI = 0.95`
- L101 `SEED = 20260910`
- L104 `SPIKE_BP = 200.0` — Калибровка: подсаженный сдвиг к `got` молодой полосы обязан перевернуть разность в пользу молодых.
- L116 `BOOT_MIN_USED = 0.9` — Бутстрап, у которого годных повторов меньше этой доли, интервала НЕ даёт: он печатается прочерком с числом, а…
- L118 `SHEETS = (('h4', 'model'), ('h24', 'model_h24'),…`
- L119 `SIDES = ('long', 'short')`
- L120 `UNION = 'объединение'`
- L125 `decision_ts(line)` — Момент РЕШЕНИЯ строки журнала разбора, секунды. None — метки нет.
- L140 `favour(got, side)` — Ход В СТОРОНУ СДЕЛКИ, б.п. Лонгу `+got`, шорту `−got`.
- L152 `choices(lines, sheet, log=None)` — Выборы листа из строк журнала разбора.
- L210 `with_age(chs, launch)` — Дописать выборам возраст на момент решения и полосу.
- L224 `band_key(age)` — Полоса вердикта: молодая, старая или ни та ни другая.
- L241 `hit_share(fav)` — Доля выборов с ходом В СТОРОНУ сделки, P(fav > 0).
- L251 `cell_stats(chs)` — Ячейка: число выборов, доля попаданий, медиана и среднее.
- L285 `cells(chs, sheets=None)` — Все ячейки: лист × сторона × полоса, плюс объединение листов.
- L306 `split_bands(chs, side)` — Молодые и старые выборы одной стороны — только объявленные полосы.
- L317 `_diff_pp(old_fav, young_fav)` — Разность долей попаданий (старые − молодые), процентные пункты.
- L323 `boot_diff(old, young, unit='выборы', reps=BOOT, seed=SEED)` — Интервал разности долей попаданий одним из двух пересчётов.
- L390 `verdict(old_cell, young_cell, boots)` — Вердикт по одной стороне. Ни одна фраза не стоит рядом с числом.
- L462 `spike(chs, bp=SPIKE_BP)` — Подсадить сдвиг `bp` к ходу молодых выборов В ИХ ПОЛЬЗУ.
- L488 `shuffle_ages(chs, seed=SEED)` — Перемешать ВОЗРАСТА выборов ВНУТРИ ЧАСА, ход оставить на месте.
- L521 `pick_side(recs, launch, min_days, side=None, seed=SEED_PICK, n_…` — `pair_age.pick` со СТОРОНОЙ: фильтр видит только свою сторону.
- L551 `ratio_of(st)` — Доход на просадку книги. Просадки нет — отношения НЕ существует.
...
## tools/resolve_maps.sh · 38 строк
