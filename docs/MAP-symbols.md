...
## research/b1_book/web.py · 12091 строк
...
- L7749 `PAPERPAGE = '<!doctype html><meta charset="utf-8">\…`
- L8080 `LIVEPAGE = '<!doctype html><meta charset="utf-8">\…`
- L8519 `VOLPAGE = '<!doctype html><meta charset="utf-8">\…`
- L8764 `GLOSSARY_PAGE = '<!doctype html><meta charset="utf-8">\…`
- L9018 `TREEPAGE = '<!doctype html><meta charset="utf-8">\…` — Страница дерева моделей — просьба владельца: разветвление от основных ML и AI, и по каждой ветке простыми сло…
- L9515 `TOURPAGE = '<!doctype html><meta charset="utf-8">\…` — Страница турнира политик — просьба владельца: весь лист веток и подветок отдельной страницей. Данные — артефа…
- L9920 `LEAGUE = '<!doctype html><meta charset="utf-8">\…` — Страница лиги — просьба владельца: наблюдение за каждой стратегией и моделью отдельно (что ведёт себя лучше)…
- L10250 `BUILTPAGE = '<!doctype html><meta charset="utf-8">\…` — Страница автономной системы: конвейер ролей и механических шагов, границы и то, что уже построено. Тексты — и…
- L10606 `STRATPAGE = '<!doctype html><meta charset="utf-8">\…`
- L11033 `ASKSPAGE = '<!doctype html><meta charset="utf-8">\…`
- L11201 `AGENTSPAGE = '<!doctype html><meta charset="utf-8">\…`
- L11731 `serve(collector, port, token, log)` — Поднять сервер наблюдения в отдельном потоке.
...
## research/dca_paper/rules.py · 773 строк
...
- L329 `H24_HOLD_H = 24`
- L332 `family_of(key)` — Семейство книги: `sit` — по листу ситуационной, `h24` — по `h24`.
- L337 `order_of(family='sit')` — Порядок книг семейства — один список на всех читателей.
- L342 `hold_of(key)` — Срок удержания книги, ч. Не объявлен — общий срок книг.
- L347 `DEFAULT_RULER = 'optimal'` — Запись БЕЗ поля `ruler` писана этой линейкой: до 2026-09-04 книга была одна и считалась глубиной. Умолчание д…
- L350 `ruler_of(row)` — Линейка строки журнала; у строк прежнего образца поля нет.
- L356 `notional_of(row)` — Нотионал позиции = маржа × плечо; None, если чего-то из двух нет.
- L372 `avg_walk(fills, entry=None, notional=None, take_frac=None, side…` — Плавающая ТВХ: средняя цена входа ПОСЛЕ каждого рунга.
- L467 `open_stats(positions)` — ХУДШАЯ из открытых позиций — та, что просела глубже всех СЕЙЧАС.
- L494 `ruler_title(key)`
- L498 `side_of(key)` — Сторона книги. Ключ без поля — длинная: до 2026-09-05 других нет.
- L509 `row_side(row, ruler=None)` — Сторона ЗАПИСИ позиции — одно правило на всех читателей.
- L529 `min_lev_of(key)` — Порог входа по плечу у режима; НЕТ поля — гейта нет вовсе.
- L547 `ONE_PER_NAME = True` — --- биржевое правило, которое реплей D-серии НЕ соблюдал --------------- На одном счёте в одностороннем режим…
- L556 `AHEAD_H = HOLD_H + 48` — --- «записано вперёд» --------------------------------------------------- Решение считается записанным ВПЕРЁД…
- L558 `HERE = os.path.dirname(os.path.abspath(__file_…`
- L559 `OUT = os.path.join(HERE, 'out')`
- L560 `JOURNAL = os.path.join(OUT, 'journal.jsonl')`
- L561 `ARTIFACT = os.path.join(OUT, 'DCA-paper.json')`
- L565 `H24_JOURNAL = os.path.join(OUT, 'short.jsonl')` — Журнал и артефакт семейства `h24` — СВОИ. Имя не «journal-…»: ротация длинной книги глобит `journal-*.jsonl`,…
- L566 `H24_ARTIFACT = os.path.join(OUT, 'DCA-short.json')`
- L569 `floor_of(ruler)` — Пол билета РЕЖИМА: биржевой минимум, переведённый в маржу.
- L581 `peak_of(ruler)` — Пик РЕЖИМА, измеренный по журналу. Неизвестный режим — пик пула.
- L591 `ticket(deposit, ruler)` — Билет книги: не меньше пола режима и не больше доли на все места.
- L604 `slots(deposit, ruler)` — Сколько мест помещается в депозит при билете этой книги.
- L609 `share(deposit, ruler)` — Доля счёта на позицию — ровно билет, выраженный долей.
- L614 `ahead(decided_at, written_at, hours=AHEAD_H, since=None)` — Записано ли решение вперёд, а не восстановлено пересчётом.
- L632 `shard_of(path, at=None)` — Файл журнала, в который идёт решение: СУТКИ по метке решения.
- L650 `shard_day(at=None)` — Дата суток решения строкой — ключ ротации, один на всех.
- L665 `SHARD_CAP = 4 * 1024 * 1024` — Порог ЧАСТИ суточного файла. Сутки оказались единицей недостаточной: одно решение живёт во всех книгах разом…
- L668 `shard_parts(path, at=None, day=None)` — Части суток по порядку: `journal-<дата>.jsonl`, затем `.01`, `.02`…
- L682 `shard_place(path, day, lines, cap=SHARD_CAP)` — Разложить строки суток по частям, не переступая порог.
- L707 `journal_parts(path=JOURNAL)` — Все куски журнала: старый цельный файл и суточные, по порядку.
- L719 `journal_key(r)` — Ключ решения: тем же составом, каким дедуплицирует запись.
- L730 `read_journal(path=JOURNAL, stats=None)` — Строки журнала как есть — из ВСЕХ его кусков, БЕЗ повторов.
- L767 `split_rows(rows, hours=AHEAD_H)` — Наблюдение и пересчёт — ДВА списка, и складывать их нельзя.

## research/dca_paper/run_paper.py · 1141 строк
