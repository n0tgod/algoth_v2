...
## research/dca_paper/rules.py · 953 строк
...
- L387 `PAIR_SHORT_GATE = {'pair_safe': True, 'pair_optimal': Tru…` — Гейт по ставке funding на входе КОРОТКОЙ стороны общего счёта — решение владельца 2026-09-08, объявлено ДО пр…
- L391 `short_gate_on(pair_key)` — Стоит ли гейт по ставке на короткой стороне этого общего счёта.
- L394 `_PAIR_NOTE = 'Общий счёт: длинная книга режима и кор…`
- L424 `PAIR_SHORT_SHARE = {'pair_safe': 1.0, 'pair_optimal': 0.25…` — Доля билета СТОРОНЫ в общем счёте — решение владельца 2026-09-07, объявленное ДО прогона по этому правилу. Ос…
- L432 `FAMILY_RULES = {'pair': 3}` — Версия правил СЕМЕЙСТВА. Общая версия `RULES` тут не годится: её смена обнулила бы запись длинных и коротких…
- L435 `family_rules(key)` — Версия правил семейства книги. Нет версии — нет и поля в строке.
- L440 `pair_share_mult(pair_key, book_key)` — Во сколько раз билет СТОРОНЫ меньше её собственного в общем счёте.
- L451 `ticket_in(pair_key, book_key, deposit)` — Билет стороны в общем счёте: свой билет × доля, но НЕ НИЖЕ пола.
- L464 `share_in(pair_key, book_key, deposit)` — Доля счёта на позицию стороны в общем счёте.
- L469 `parts_of(key)` — Из каких книг собран общий счёт. Обычная книга — источник один.
- L482 `RETIRED_AT = {'safe_s': '2026-09-07', 'optimal_s': '…` — --- снятые книги (решение владельца 2026-09-07) ------------------------ Зеркала `*_s` на ситуационном листе…
- L486 `retired(key)` — Снята ли книга. Дата снятия — в `RETIRED_AT`, одним местом.
- L495 `RULER_ORDER = [k for k in RULER_ORDER_ALL if not reti…` — Действующий порядок: снятая книга выпадает отсюда, и ВСЕ читатели — прогон, свод, отчёт, страница — перестают…
- L498 `family_of(key)` — Семейство книги: `sit` — по листу ситуационной, `h24` — по `h24`.
- L503 `order_of(family='sit', with_retired=False)` — Порядок книг семейства — один список на всех читателей.
- L515 `hold_of(key)` — Срок удержания книги, ч. Не объявлен — общий срок книг.
- L520 `DEFAULT_RULER = 'optimal'` — Запись БЕЗ поля `ruler` писана этой линейкой: до 2026-09-04 книга была одна и считалась глубиной. Умолчание д…
- L523 `ruler_of(row)` — Линейка строки журнала; у строк прежнего образца поля нет.
- L529 `notional_of(row)` — Нотионал позиции = маржа × плечо; None, если чего-то из двух нет.
- L545 `avg_walk(fills, entry=None, notional=None, take_frac=None, side…` — Плавающая ТВХ: средняя цена входа ПОСЛЕ каждого рунга.
- L640 `open_stats(positions)` — ХУДШАЯ из открытых позиций — та, что просела глубже всех СЕЙЧАС.
- L667 `ruler_title(key)`
- L671 `side_of(key)` — Сторона книги. Ключ без поля — длинная: до 2026-09-05 других нет.
- L682 `row_side(row, ruler=None)` — Сторона ЗАПИСИ позиции — одно правило на всех читателей.
- L702 `min_lev_of(key)` — Порог входа по плечу у режима; НЕТ поля — гейта нет вовсе.
- L720 `ONE_PER_NAME = True` — --- биржевое правило, которое реплей D-серии НЕ соблюдал --------------- На одном счёте в одностороннем режим…
- L729 `AHEAD_H = HOLD_H + 48` — --- «записано вперёд» --------------------------------------------------- Решение считается записанным ВПЕРЁД…
- L731 `HERE = os.path.dirname(os.path.abspath(__file_…`
- L732 `OUT = os.path.join(HERE, 'out')`
- L733 `JOURNAL = os.path.join(OUT, 'journal.jsonl')`
- L734 `ARTIFACT = os.path.join(OUT, 'DCA-paper.json')`
- L738 `H24_JOURNAL = os.path.join(OUT, 'short.jsonl')` — Журнал и артефакт семейства `h24` — СВОИ. Имя не «journal-…»: ротация длинной книги глобит `journal-*.jsonl`,…
- L739 `H24_ARTIFACT = os.path.join(OUT, 'DCA-short.json')`
- L744 `PAIR_JOURNAL = os.path.join(OUT, 'pair.jsonl')` — Общий счёт — тоже своё семейство со своим журналом: строки его книг ДРУГИЕ (та же сделка получает другую марж…
- L745 `PAIR_ARTIFACT = os.path.join(OUT, 'DCA-pair.json')`
- L748 `floor_of(ruler)` — Пол билета РЕЖИМА: биржевой минимум, переведённый в маржу.
- L760 `peak_of(ruler)` — Пик РЕЖИМА, измеренный по журналу. Неизвестный режим — пик пула.
- L770 `ticket(deposit, ruler)` — Билет книги: не меньше пола режима и не больше доли на все места.
- L783 `slots(deposit, ruler)` — Сколько мест помещается в депозит при билете этой книги.
- L788 `share(deposit, ruler)` — Доля счёта на позицию — ровно билет, выраженный долей.
- L793 `ahead(decided_at, written_at, hours=AHEAD_H, since=None)` — Записано ли решение вперёд, а не восстановлено пересчётом.
- L811 `shard_of(path, at=None)` — Файл журнала, в который идёт решение: СУТКИ по метке решения.
- L829 `shard_day(at=None)` — Дата суток решения строкой — ключ ротации, один на всех.
- L844 `SHARD_CAP = 4 * 1024 * 1024` — Порог ЧАСТИ суточного файла. Сутки оказались единицей недостаточной: одно решение живёт во всех книгах разом…
- L847 `shard_parts(path, at=None, day=None)` — Части суток по порядку: `journal-<дата>.jsonl`, затем `.01`, `.02`…
- L861 `shard_place(path, day, lines, cap=SHARD_CAP)` — Разложить строки суток по частям, не переступая порог.
- L886 `journal_parts(path=JOURNAL)` — Все куски журнала: старый цельный файл и суточные, по порядку.
- L898 `journal_key(r)` — Ключ решения: тем же составом, каким дедуплицирует запись.
- L910 `read_journal(path=JOURNAL, stats=None)` — Строки журнала как есть — из ВСЕХ его кусков, БЕЗ повторов.
- L947 `split_rows(rows, hours=AHEAD_H)` — Наблюдение и пересчёт — ДВА списка, и складывать их нельзя.

## research/dca_paper/run_pair.py · 505 строк

Общий счёт: длинная книга и короткая на ОДНОМ депозите.

- L47 `HERE = os.path.dirname(os.path.abspath(__file_…`
- L48 `ROOT = os.path.dirname(os.path.dirname(HERE))`
- L61 `MEM_LIMIT_MB = 1200` — Предел памяти прогона. На VPS 7.7 ГБ без свопа, и тяжёлый прогон рядом с часовым циклом убивает НЕ СЕБЯ (OOM…
- L64 `long_recs(cache=None, log=print)` — Позиции длинных книг из кэша реплея `run_paper`, по книгам.
- L93 `short_recs(cache=None, log=print)` — Позиции коротких книг `h24` из их кэша, по книгам семейства.
- L111 `pack(longs, shorts, keys=None)` — Решения общей книги: обе стороны в одном списке, с меткой источника.
- L127 `gate_shorts(shorts, pk, ctx, log=print)` — Короткие решения под гейтом по ставке funding — правило книги.
- L175 `collisions(rows, ruler)` — Имена, которые общий счёт держит РАЗОМ длинной и короткой.
- L192 `_side_series(rows, ruler, side)`
- L196 `link(rows, ruler)` — Связь дневных денег сторон ВНУТРИ общего счёта.
- L212 `separate(rows_by_book, dep)` — Те же книги на РАЗДЕЛЬНЫХ счетах — для сравнения с общим.
- L227 `one_sided(book, lk, sk)` — Какой стороны в общем счёте НЕТ. Пусто — обе на месте.
- L241 `run(log=print, now=None, journal=None, long_cache=None, short_c…`
- L329 `_p(x, d=2)`
- L333 `_u(x)`
- L337 `report(s)`
- L476 `publish(name)`
...
## tools/watchdog_book.sh · 364 строк
