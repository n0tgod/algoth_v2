...
## research/dca_paper/rules.py · 691 строк
...
- L38 `DEPOSITS = [1000.0, 10000.0, 100000.0]` — --- депозиты (решение владельца) ---------------------------------------
- L52 `MIN_NOTIONAL = 5.0` — --- размер билета ВЫВЕДЕН из пола биржи, а не назначен ------------------ Минимальный ордер площадки ровно $5…
- L53 `RUNG_SHARE = 0.25`
- L54 `HEADROOM = 1.25`
- L55 `TICKET_MIN = MIN_NOTIONAL / RUNG_SHARE * HEADROOM`
- L83 `PEAK_SEEN = 457` — --- потолок билета: столько, чтобы хватило на ВСЕ места ---------------- Пол задаёт биржа, потолок — сама кни…
- L84 `PEAK_MARGIN = 1.5`
- L85 `PEAKS = {'safe': 457, 'optimal': 457, 'aggr': 2…`
- L86 `TICKET = TICKET_MIN`
- L89 `MIN_EDGE_BP = 33.0` — --- гейты входа: те же, которыми входит живая ситуационная книга -------
- L90 `MIN_RR = 2.0`
- L95 `SIDE = 'long'` — Сторона книги стала осью реестра (`RULERS[...]["side"]`, решение владельца 2026-09-05: зеркальные короткие кн…
- L98 `SURVIVE_MULT = 2.0` — --- ограда и выход: база D3/D4 (забор 2.0, пол капитуляции 0.10) -------
- L99 `FLOOR_FRAC = 0.1`
- L100 `N_RUNGS = 4`
- L101 `HOLD_H = 72`
- L129 `TAKE_ANCHOR = 'avg'` — --- тейк позиции (решение владельца 2026-09-05, замер D8) -------------- Было: `вход · (1 + mfe)` — НЕПОДВИЖН…
- L130 `TAKE_MULT = 2.0`
- L133 `take_rule(fav_bp, side='long')` — Правило тейка позиции: якорь и доля цены. None — цели нет.
- L176 `SIGMA_MULT = 6.0` — --- две линейки плеча (решение владельца 2026-09-04) -------------------- Плечо не настройка агрессивности: о…
- L191 `AGGR_MIN_LEV = MIN_NOTIONAL / RUNG_SHARE / 5.0` — --- порог третьего режима (решение владельца 2026-09-04) -------------- «Агрессивный» режим есть та же линейк…
- L192 `RULERS = {'safe': {'rule': 'sigma', 'param': SIG…`
- L248 `_SHORT_NOTE = 'Зеркало длинной книги той же линейки: …` — --- зеркальные КОРОТКИЕ книги (решение владельца 2026-09-05) ----------- «Такая же логика, только шорт»: те ж…
- L267 `RULER_ORDER = ['safe', 'optimal', 'aggr', 'safe_s', '…`
- L270 `DEFAULT_RULER = 'optimal'` — Запись БЕЗ поля `ruler` писана этой линейкой: до 2026-09-04 книга была одна и считалась глубиной. Умолчание д…
- L273 `ruler_of(row)` — Линейка строки журнала; у строк прежнего образца поля нет.
- L279 `notional_of(row)` — Нотионал позиции = маржа × плечо; None, если чего-то из двух нет.
- L295 `avg_walk(fills, entry=None, notional=None, take_frac=None, side…` — Плавающая ТВХ: средняя цена входа ПОСЛЕ каждого рунга.
- L390 `open_stats(positions)` — ХУДШАЯ из открытых позиций — та, что просела глубже всех СЕЙЧАС.
- L417 `ruler_title(key)`
- L421 `side_of(key)` — Сторона книги. Ключ без поля — длинная: до 2026-09-05 других нет.
- L432 `row_side(row, ruler=None)` — Сторона ЗАПИСИ позиции — одно правило на всех читателей.
- L452 `min_lev_of(key)` — Порог входа по плечу у режима; НЕТ поля — гейта нет вовсе.
- L470 `ONE_PER_NAME = True` — --- биржевое правило, которое реплей D-серии НЕ соблюдал --------------- На одном счёте в одностороннем режим…
- L479 `AHEAD_H = HOLD_H + 48` — --- «записано вперёд» --------------------------------------------------- Решение считается записанным ВПЕРЁД…
- L481 `HERE = os.path.dirname(os.path.abspath(__file_…`
- L482 `OUT = os.path.join(HERE, 'out')`
- L483 `JOURNAL = os.path.join(OUT, 'journal.jsonl')`
- L484 `ARTIFACT = os.path.join(OUT, 'DCA-paper.json')`
- L487 `floor_of(ruler)` — Пол билета РЕЖИМА: биржевой минимум, переведённый в маржу.
- L499 `peak_of(ruler)` — Пик РЕЖИМА, измеренный по журналу. Неизвестный режим — пик пула.
- L509 `ticket(deposit, ruler)` — Билет книги: не меньше пола режима и не больше доли на все места.
- L522 `slots(deposit, ruler)` — Сколько мест помещается в депозит при билете этой книги.
- L527 `share(deposit, ruler)` — Доля счёта на позицию — ровно билет, выраженный долей.
- L532 `ahead(decided_at, written_at, hours=AHEAD_H, since=None)` — Записано ли решение вперёд, а не восстановлено пересчётом.
- L550 `shard_of(path, at=None)` — Файл журнала, в который идёт решение: СУТКИ по метке решения.
- L568 `shard_day(at=None)` — Дата суток решения строкой — ключ ротации, один на всех.
- L583 `SHARD_CAP = 4 * 1024 * 1024` — Порог ЧАСТИ суточного файла. Сутки оказались единицей недостаточной: одно решение живёт во всех книгах разом…
- L586 `shard_parts(path, at=None, day=None)` — Части суток по порядку: `journal-<дата>.jsonl`, затем `.01`, `.02`…
- L600 `shard_place(path, day, lines, cap=SHARD_CAP)` — Разложить строки суток по частям, не переступая порог.
- L625 `journal_parts(path=JOURNAL)` — Все куски журнала: старый цельный файл и суточные, по порядку.
- L637 `journal_key(r)` — Ключ решения: тем же составом, каким дедуплицирует запись.
- L648 `read_journal(path=JOURNAL, stats=None)` — Строки журнала как есть — из ВСЕХ его кусков, БЕЗ повторов.
- L685 `split_rows(rows, hours=AHEAD_H)` — Наблюдение и пересчёт — ДВА списка, и складывать их нельзя.
