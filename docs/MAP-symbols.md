...
## research/b1_book/web.py · 12049 строк
...
- L7707 `PAPERPAGE = '<!doctype html><meta charset="utf-8">\…`
- L8038 `LIVEPAGE = '<!doctype html><meta charset="utf-8">\…`
- L8477 `VOLPAGE = '<!doctype html><meta charset="utf-8">\…`
- L8722 `GLOSSARY_PAGE = '<!doctype html><meta charset="utf-8">\…`
- L8976 `TREEPAGE = '<!doctype html><meta charset="utf-8">\…` — Страница дерева моделей — просьба владельца: разветвление от основных ML и AI, и по каждой ветке простыми сло…
- L9473 `TOURPAGE = '<!doctype html><meta charset="utf-8">\…` — Страница турнира политик — просьба владельца: весь лист веток и подветок отдельной страницей. Данные — артефа…
- L9878 `LEAGUE = '<!doctype html><meta charset="utf-8">\…` — Страница лиги — просьба владельца: наблюдение за каждой стратегией и моделью отдельно (что ведёт себя лучше)…
- L10208 `BUILTPAGE = '<!doctype html><meta charset="utf-8">\…` — Страница автономной системы: конвейер ролей и механических шагов, границы и то, что уже построено. Тексты — и…
- L10564 `STRATPAGE = '<!doctype html><meta charset="utf-8">\…`
- L10991 `ASKSPAGE = '<!doctype html><meta charset="utf-8">\…`
- L11159 `AGENTSPAGE = '<!doctype html><meta charset="utf-8">\…`
- L11689 `serve(collector, port, token, log)` — Поднять сервер наблюдения в отдельном потоке.
...
## research/dca_paper/rules.py · 1003 строк
...
- L433 `PAIR_SHORT_SHARE = {'pair_safe': 1.0, 'pair_optimal': 0.25…` — Доля билета СТОРОНЫ в общем счёте — решение владельца 2026-09-07, объявленное ДО прогона по этому правилу. Ос…
- L456 `PAIR_MIN_AGE_DAYS = {'pair_safe': 7.0, 'pair_optimal': 7.0,…` — ВОЗРАСТ ИМЕНИ на входе короткой стороны — решение владельца 2026-09-08 («объявляй»), объявленное ДО следующег…
- L460 `min_age_days(pair_key)` — Порог возраста имени для короткой стороны. Ноль — фильтра нет.
- L482 `FAMILY_RULES = {'pair': 5}` — Версия правил СЕМЕЙСТВА. Общая версия `RULES` тут не годится: её смена обнулила бы запись длинных и коротких…
- L485 `family_rules(key)` — Версия правил семейства книги. Нет версии — нет и поля в строке.
- L490 `pair_share_mult(pair_key, book_key)` — Во сколько раз билет СТОРОНЫ меньше её собственного в общем счёте.
- L501 `ticket_in(pair_key, book_key, deposit)` — Билет стороны в общем счёте: свой билет × доля, но НЕ НИЖЕ пола.
- L514 `share_in(pair_key, book_key, deposit)` — Доля счёта на позицию стороны в общем счёте.
- L519 `parts_of(key)` — Из каких книг собран общий счёт. Обычная книга — источник один.
- L532 `RETIRED_AT = {'safe_s': '2026-09-07', 'optimal_s': '…` — --- снятые книги (решение владельца 2026-09-07) ------------------------ Зеркала `*_s` на ситуационном листе…
- L536 `retired(key)` — Снята ли книга. Дата снятия — в `RETIRED_AT`, одним местом.
- L545 `RULER_ORDER = [k for k in RULER_ORDER_ALL if not reti…` — Действующий порядок: снятая книга выпадает отсюда, и ВСЕ читатели — прогон, свод, отчёт, страница — перестают…
- L548 `family_of(key)` — Семейство книги: `sit` — по листу ситуационной, `h24` — по `h24`.
- L553 `order_of(family='sit', with_retired=False)` — Порядок книг семейства — один список на всех читателей.
- L565 `hold_of(key)` — Срок удержания книги, ч. Не объявлен — общий срок книг.
- L570 `DEFAULT_RULER = 'optimal'` — Запись БЕЗ поля `ruler` писана этой линейкой: до 2026-09-04 книга была одна и считалась глубиной. Умолчание д…
- L573 `ruler_of(row)` — Линейка строки журнала; у строк прежнего образца поля нет.
- L579 `notional_of(row)` — Нотионал позиции = маржа × плечо; None, если чего-то из двух нет.
- L595 `avg_walk(fills, entry=None, notional=None, take_frac=None, side…` — Плавающая ТВХ: средняя цена входа ПОСЛЕ каждого рунга.
- L690 `open_stats(positions)` — ХУДШАЯ из открытых позиций — та, что просела глубже всех СЕЙЧАС.
- L717 `ruler_title(key)`
- L721 `side_of(key)` — Сторона книги. Ключ без поля — длинная: до 2026-09-05 других нет.
- L732 `row_side(row, ruler=None)` — Сторона ЗАПИСИ позиции — одно правило на всех читателей.
- L752 `min_lev_of(key)` — Порог входа по плечу у режима; НЕТ поля — гейта нет вовсе.
- L770 `ONE_PER_NAME = True` — --- биржевое правило, которое реплей D-серии НЕ соблюдал --------------- На одном счёте в одностороннем режим…
- L779 `AHEAD_H = HOLD_H + 48` — --- «записано вперёд» --------------------------------------------------- Решение считается записанным ВПЕРЁД…
- L781 `HERE = os.path.dirname(os.path.abspath(__file_…`
- L782 `OUT = os.path.join(HERE, 'out')`
- L783 `JOURNAL = os.path.join(OUT, 'journal.jsonl')`
- L784 `ARTIFACT = os.path.join(OUT, 'DCA-paper.json')`
- L788 `H24_JOURNAL = os.path.join(OUT, 'short.jsonl')` — Журнал и артефакт семейства `h24` — СВОИ. Имя не «journal-…»: ротация длинной книги глобит `journal-*.jsonl`,…
- L789 `H24_ARTIFACT = os.path.join(OUT, 'DCA-short.json')`
- L794 `PAIR_JOURNAL = os.path.join(OUT, 'pair.jsonl')` — Общий счёт — тоже своё семейство со своим журналом: строки его книг ДРУГИЕ (та же сделка получает другую марж…
- L795 `PAIR_ARTIFACT = os.path.join(OUT, 'DCA-pair.json')`
- L798 `floor_of(ruler)` — Пол билета РЕЖИМА: биржевой минимум, переведённый в маржу.
- L810 `peak_of(ruler)` — Пик РЕЖИМА, измеренный по журналу. Неизвестный режим — пик пула.
- L820 `ticket(deposit, ruler)` — Билет книги: не меньше пола режима и не больше доли на все места.
- L833 `slots(deposit, ruler)` — Сколько мест помещается в депозит при билете этой книги.
- L838 `share(deposit, ruler)` — Доля счёта на позицию — ровно билет, выраженный долей.
- L843 `ahead(decided_at, written_at, hours=AHEAD_H, since=None)` — Записано ли решение вперёд, а не восстановлено пересчётом.
- L861 `shard_of(path, at=None)` — Файл журнала, в который идёт решение: СУТКИ по метке решения.
- L879 `shard_day(at=None)` — Дата суток решения строкой — ключ ротации, один на всех.
- L894 `SHARD_CAP = 4 * 1024 * 1024` — Порог ЧАСТИ суточного файла. Сутки оказались единицей недостаточной: одно решение живёт во всех книгах разом…
- L897 `shard_parts(path, at=None, day=None)` — Части суток по порядку: `journal-<дата>.jsonl`, затем `.01`, `.02`…
- L911 `shard_place(path, day, lines, cap=SHARD_CAP)` — Разложить строки суток по частям, не переступая порог.
- L936 `journal_parts(path=JOURNAL)` — Все куски журнала: старый цельный файл и суточные, по порядку.
- L948 `journal_key(r)` — Ключ решения: тем же составом, каким дедуплицирует запись.
- L960 `read_journal(path=JOURNAL, stats=None)` — Строки журнала как есть — из ВСЕХ его кусков, БЕЗ повторов.
- L997 `split_rows(rows, hours=AHEAD_H)` — Наблюдение и пересчёт — ДВА списка, и складывать их нельзя.

## research/dca_paper/run_pair.py · 577 строк
...
- L177 `age_shorts(shorts, pk, launch=None, log=print, now=None)` — Короткие решения под фильтром возраста имени — правило книги.
- L229 `collisions(rows, ruler)` — Имена, которые общий счёт держит РАЗОМ длинной и короткой.
- L246 `_side_series(rows, ruler, side)`
- L250 `link(rows, ruler)` — Связь дневных денег сторон ВНУТРИ общего счёта.
- L266 `separate(rows_by_book, dep)` — Те же книги на РАЗДЕЛЬНЫХ счетах — для сравнения с общим.
- L281 `one_sided(book, lk, sk)` — Какой стороны в общем счёте НЕТ. Пусто — обе на месте.
- L295 `run(log=print, now=None, journal=None, long_cache=None, short_c…`
- L388 `_p(x, d=2)`
- L392 `_u(x)`
- L396 `report(s)`
- L548 `publish(name)`
...
## research/dca_paper/run_paper.py · 1373 строк
...
- L105 `read_cache(path=None, sig=None)` — Кэш реплея: (пара, символ, момент) → запись позиции.
- L138 `write_cache(cache, path=None, sig=None)` — Кэш пишется ЦЕЛИКОМ и атомарно: дозапись оставила бы в файле записи двух подписей разом, а различить их потом…
- L152 `needs_replay(cache, legs, pairs)` — Какие решения обязан пересчитать этот прогон.
- L182 `_key(r)` — Ключ решения: имя плюс секунда входа. Ими и дедуплицируется.
- L187 `_cell(ruler, dep)` — Ключ книги: линейка и депозит. Одно решение живёт в обеих книгах, и склеив их одним ключом, мы потеряли бы вт…
- L193 `build_rows(by_ruler, now=None, log=print, keys=None)` — Решения, взятые каждой книгой, с деньгами в долларах.
- L372 `append_journal(rows, path=None, log=print)` — Дописывает только НОВЫЕ решения. Запись write-ahead: строка, однажды попавшая в журнал, не переписывается — и…
- L425 `dups(rows)` — Правило владельца числом: одно имя — одна позиция ОДНОВРЕМЕННО.
- L464 `_stats(rows, deposit)` — Итог, просадка и форма по дням — на ЭТОМ подмножестве строк.
- L540 `_book_costs(rows)` — Что вычтено у книги: суммы издержек и число неизмеренных сделок.
- L585 `summarize(path=None, live=None, keys=None, ctx=None)` — Свод по книгам: ОДНА кривая, и в ней помечено, что бэктест.
- L682 `_pct(x, d=2)`
- L686 `_tail_words(s)` — Числа хвоста словами. Нет чисел — так и сказано, а не ноль.
- L721 `costs_block(s)` — Раздел «издержки» — ОДИН на весь отчёт: деньги ниже уже нетто.
- L807 `report(s)`
- L1165 `rules_snapshot(keys=None)` — Правила прогона в свод: страница описывает ИМИ то, что видит.
- L1194 `publish(name)`
- L1200 `main()`

## research/dca_paper/run_short.py · 310 строк
...
- L55 `CELL = ('fence:none:t2', 'fence', 'none', 't2')` — Ячейка сетки D10, которой торгует семейство: плечо забора, без доливов, цель ×2. Объявлена здесь одним местом…
- L59 `BOOKS = {'safe_h': 'safe_s', 'optimal_h': 'opti…` — Книга → линейка забора, на которой считается её позиция. «Агрессивная» считается на той же линейке, что «опти…
- L60 `ARMS = ('gbm', 'nn')`
- L67 `CACHE_DIR = os.path.join(HERE, 'cache')` — Кэш реплея лежит ВНЕ публикуемого каталога. `publish.sh` кладёт в историю весь `research/*/out`, и семимегаба…
- L68 `CACHE = os.path.join(CACHE_DIR, 'recs-short.jso…`
- L71 `cache_sig()` — Подпись настроек, от которых зависит исход позиции.
- L86 `legs(arms=ARMS, limit=None, log=print, path=None)` — Короткие выборы `h24` ОБЕИХ рук: решение владельца «оставить обе».
- L101 `needs_replay(cache, legs_)` — Какие решения считать заново: новых нет в кэше, открытые стареют.
- L114 `read_cache(path=None, log=print)` — Кэш реплея семейства. Непригодный не чинится молча.
- L128 `write_cache(cache, path=None)`
- L135 `replay(need, src=None, log=print)` — Досчёт недостающих решений: одна ячейка, отметки и заполнения.
- L168 `run(limit=None, src=None, log=print, legs_=None, journal=None, …`
- L219 `_p(x, d=2)`
- L223 `_u(x)`
- L227 `report(s)`
- L276 `publish(name)`
