...
# Пять правил: удаления в защищённых путях, маркеры конфликта, ключи,
# крупные файлы, разросшееся ядро памяти CLAUDE.md.
#
...
# --- 5. Ядро памяти не разрастается ------------------------------------
# CLAUDE.md загружается КАЖДОЙ сессии и каждому вызову агента целиком.
# К 2026-09-06 он дорос до 1.08 МБ (~200 тысяч токенов на вызов) и был
# разделён: ядро здесь, история — docs/memory/, хронология — журнал.
# Правило «сюда не дописывают ход работы» само по себе не держит —
# держит эта проверка. Осознанный рост — ALLOW_BIG_MEMORY=1.
MEMORY_CAP=49152
if git diff --cached --name-only 2>/dev/null | grep -qx 'CLAUDE.md'; then
    msz=$(git cat-file -s :CLAUDE.md 2>/dev/null || echo 0)
    if [ "$msz" -gt "$MEMORY_CAP" ] && [ "${ALLOW_BIG_MEMORY:-0}" != "1" ]; then
        say "ОТКАЗ: CLAUDE.md больше $((MEMORY_CAP / 1024)) КБ ($((msz / 1024)) КБ)."
        say "  Ядро памяти обязано быть коротким: история — docs/memory/,"
        say "  хронология — docs/journal/. Осознанный рост — ALLOW_BIG_MEMORY=1."
        fail=1
    fi
fi
