...
# Маркер `.done` пишется ПОСЛЕ исполнения, поэтому без замка сторож
# каждые 5 минут запускал бы ИДУЩЕЕ задание второй раз — ровно так
# зонд каскада вверх был размножен и убит по памяти рядом со сборщиком
# (2026-08-30; длинные прогоны выживали только случайно: их
# промежуточные публикации грязнили дерево и валили merge --ff-only).
# Занятый замок — штатное состояние при долгом задании, не тревога:
# в queue-state он не пишется, иначе кричал бы всю длину прогона.
exec 9>"$JOBS/.lock"
if ! flock -n 9; then
