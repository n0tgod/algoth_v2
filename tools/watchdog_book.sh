...
# --- догон рядов funding площадки (решение владельца 2026-09-08) -------
# Сборщик A1 возобновляется по СУЩЕСТВОВАНИЮ файла: ряд кончается днём
# своего прогона и не растёт никогда. Один раз это уже стоило замера —
# покрытие позиций 0 %, ряды кончились за месяц до журнала книг. Теперь
# у рядов появился второй читатель, которому свежесть нужна каждый час:
# гейт по ставке funding на входе короткой стороны общего счёта. Ставка
# старше суток для него «неизвестна», то есть устаревший ряд закрывает
# книгу молча — ровно тот отказ, что неотличим от исправности.
# Каденция суточная (ряды растут тремя–двадцатью четырьмя точками в
# сутки), час выбран тихий: обучение в 02 и 06 UTC, сборщик и цикл
# заняты своим.
FUND=research/a1_universe/out/funding-refresh.json
FUND_LOG=research/a1_universe/out/refresh.log
if ! pgrep -f "a1_universe/funding_refresh.py" >/dev/null; then
    f_age=999999999
    if [ -f "$FUND" ]; then
        f_ts=$(date -u -r "$FUND" +%s 2>/dev/null || echo 0)
        [ "$f_ts" -gt 0 ] && f_age=$(( $(date -u +%s) - f_ts ))
    fi
    f_hh=$(date -u +%H)
    if [ "$f_hh" = "04" ] && [ "$f_age" -gt 72000 ]; then
        echo "[$(now)] ряды funding: последний догон ${f_age} с назад — прогон"
        setsid nohup bash -c "
            nice -n 15 .venv/bin/python \
                research/a1_universe/funding_refresh.py \
                >> $FUND_LOG 2>&1" &
    fi
fi
...
# не спадёт. Не блокирует ничего: тревога — не действие.
if [ -x .venv/bin/python ] && [ -f tools/disk_alarm.py ]; then
    .venv/bin/python tools/disk_alarm.py || true
fi
