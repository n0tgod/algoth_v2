  ...
  for i in 1 2 3 4 5; do
    ...
    if [ -n "$unmerged" ]; then
      ...
      while IFS= read -r p; do
        [ -z "$p" ] && continue
        case "$p" in
          # Логи очереди и её состояние — тоже запись ПРОГОНА, а не
          # чья-то работа: обе стороны дописывают их сами. Конфликт
          # здесь механический, и утром он уже держал канал закрытым
          # (`jobs/done/diag-q-short.log`). Разрешается так же —
          # в пользу того, что записал прогон.
          research/*/out/*|jobs/done/*|jobs/queue-state.md)
