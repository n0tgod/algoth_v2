...
# record=True: журналы машины пишет ТОТ, КТО СУДИТ НАЧИСТО, то есть
# запускалка. Роль может позвать ту же проверку, чтобы проверить себя,
# и тогда она не пишет ничего — иначе запись двоится (03.09 «ждёт
# владельца» легло в очередь дважды).
ok, bad = R.check_role(sys.argv[1], sys.argv[2], since=float(sys.argv[3]),
                       out=sys.argv[4], record=True)
