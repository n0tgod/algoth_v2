...
/// Книга без манифеста не даёт исполнителю стартовать: версию правил
/// узнать не у кого, журнал остался бы без маркера, и следующий
/// запуск отставил бы его с открытыми позициями (инцидент
/// 2026-08-23 — исполнитель поднялся раньше, чем цикл создал
/// манифест новой книги).
#[test]
fn без_манифеста_книги_исполнитель_не_стартует() {
    let fx = Fx::new("no-manifest");
    let m = Mock::new().with_sym("ARBUSDT", 0.9999, 1.0001, 0.0001, 0.1, 0.1, 5.0);
    std::fs::remove_file(fx.s8.join("manifest.json")).unwrap();
    let r = Executor::open(fx.cfg(false), m, false);
    let e = r.err().expect("старт без манифеста обязан отказать");
    assert!(e.contains("нет манифеста"), "{e}");
}

/// Живой журнал с несовпадающим (или потерянным) маркером версии НЕ
/// отставляется сам — только отказ словами с названным лечением:
/// само-архив бросил бы позиции на бирже без управления и записи.
/// Обе стороны: отказ при потерянном маркере, старт после его
/// восстановления рукой.
#[test]
fn журнал_без_маркера_не_отставляется_а_останавливает() {
    let fx = Fx::new("rules-marker");
    let m = Mock::new().with_sym("ARBUSDT", 0.9999, 1.0001, 0.0001, 0.1, 0.1, 5.0);
    fx.entry("gbm", "2026-08-20-15", "ARBUSDT", "long", 1.0, -50.0, 120.0, 1755699950.5);
    {
        let mut ex = Executor::open(fx.cfg(false), m.clone(), false).unwrap();
    ...
    // Маркер потерян (класс инцидента: журнал начат до манифеста).
    std::fs::remove_file(fx.jr.join("rules_version.txt")).unwrap();
    let r = Executor::open(fx.cfg(false), m.clone(), false);
    let e = r.err().expect("несовпадение версии обязано отказать");
    assert!(e.contains("сам не отставляется"), "{e}");
    assert!(e.contains("echo 13"), "лечение не названо: {e}");
    // Журнал на месте: не переименован и не опустошён.
    assert!(fx.jr.join("book.txt").exists() || true);
    assert!(std::fs::read_dir(&fx.jr).unwrap().flatten().any(|f| f
        .file_name()
        .to_string_lossy()
        .starts_with("journal-")),
        "журнал пропал из каталога");
    assert!(!fx.root.join("bot_live.rules-v0").exists(),
            "журнал отставлен само-архивом");
    // Восстановленный рукой маркер — исполнитель поднимается, позиция
    // читается из того же журнала.
    std::fs::write(fx.jr.join("rules_version.txt"), "13\n").unwrap();
    m.push_position("ARBUSDT", Side::Long, 30.0);
    let ex = Executor::open(fx.cfg(false), m, false).unwrap();
    assert_eq!(ex.pos.len(), 1, "позиция потеряна при подъёме");
}
...
/// Файл NO_ENTRIES — штатная пауза входов на время перевода на другую
/// книгу (решение владельца 2026-08-22): свежее событие не входит, не
/// шлёт заявку и не оставляет строки в журнале, а ВЫХОД при паузе
/// работает как обязанность. Статус называет паузу словами. Снятый
/// флаг возвращает входы — встроенный отрицательный контроль: без
/// него «пауза» была бы неотличима от любой другой причины тишины.
#[test]
fn пауза_входов_файлом_не_трогает_выходы() {
    let fx = Fx::new("no-entries");
    let m = Mock::new()
        .with_sym("ARBUSDT", 0.9999, 1.0001, 0.0001, 0.1, 0.1, 5.0)
        .with_sym("OPUSDT", 0.9999, 1.0001, 0.0001, 0.1, 0.1, 5.0);
    // Позиция открыта ДО паузы — ей предстоит закрыться ПРИ паузе.
    fx.entry("gbm", "2026-08-20-15", "ARBUSDT", "long", 1.0, -50.0, 120.0,
             1755699950.5);
    let mut ex = Executor::open(fx.cfg(false), m.clone(), false).unwrap();
    ex.tick(NOW);
    assert_eq!(ex.pos.len(), 1);

    std::fs::write(fx.jr.join("NO_ENTRIES"), b"").unwrap();
    fx.entry("gbm", "2026-08-20-15", "OPUSDT", "long", 1.0, -50.0, 120.0,
             1755700005.0);
    let rep = ex.tick(NOW + 10_000);
    assert_eq!(rep.opened, 0, "вход открылся при паузе");
    let placed = m.placed();
    assert!(placed.iter().all(|p| p.sym != "OPUSDT"), "{placed:?}");
    let recs = fx.records();
    assert!(recs.iter().all(|r| !matches!(&r.event,
        Event::Decision { sym, .. } if sym == "OPUSDT")),
        "пауза оставила след в журнале");
    let st = ex.status_json(NOW + 10_000, None);
    assert_eq!(st["entries_paused"], true, "пауза не названа в статусе");

    // Выход — обязанность: пауза его не касается.
    fx.exit("gbm", "2026-08-20-15", "ARBUSDT", "long",
            "цена прошла обещанный ход против", 1755700012.0);
    let rep2 = ex.tick(NOW + 15_000);
    assert_eq!(rep2.closed, 1, "выход не исполнился при паузе");
    assert!(ex.pos.is_empty());

    // Отрицательный контроль: флаг снят — то же событие (ещё свежее
    // по порогу) обязано войти, и статус обязан замолчать.
    std::fs::remove_file(fx.jr.join("NO_ENTRIES")).unwrap();
    let rep3 = ex.tick(NOW + 20_000);
    assert_eq!(rep3.opened, 1, "снятый флаг не вернул входы");
    let st = ex.status_json(NOW + 20_000, None);
    assert_eq!(st["entries_paused"], false);
}
