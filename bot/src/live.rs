...
/// больше объявленного.
/// Базисные пункты → процент, беззнаковой величиной.
///
/// Наружу проект показывает только доллары и проценты (решение
/// владельца); базисный пункт остаётся единицей ХРАНЕНИЯ. Потолок
/// цены — настройка, а не движение цены, поэтому знака у него нет:
/// «+0.30 %» читалось бы как прибыль. Правило то же, что у `lvl` на
/// страницах: два знака, три у мелких величин.
pub fn lvl_bp(v: f64) -> String {
    let d = if v.abs() >= 10.0 { 2 } else { 3 };
    format!("{:.*} %", d, v / 100.0)
}
        ...
        match self.ex.order_status(&p.sym, &oid) {
            ...
            Ok(_) => {
                self.reject_exit(
                    key,
                    &p,
                    &format!("{reason} — IOC не исполнилась в потолке {}", lvl_bp(cap_bp)),
                    now_ms,
                    rep,
                );
            }
    ...
    fn try_enter(
        ...
        if st.filled_qty <= 0.0 {
            self.reject_entry(
                ...
                side,
                format!(
                    "IOC не исполнилась в потолке {} (запрошено {})",
                    lvl_bp(self.cfg.entry_cap_bp),
                    fmt_step(qty, ins.step)
                ),
                now_ms,
                rep,
