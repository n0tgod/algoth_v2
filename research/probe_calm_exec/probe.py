...
def main():
    ...
    if not events:
        # Причины обязаны попасть В ЛОГ: отсылка к счётчикам, которых
        # никто не напечатал, — отказ, неотличимый от тишины. Первый же
        # смоук на сервере упал ровно так: --days 2 не оставляет σ ни
        # одних прошлых суток, и снаружи это было не прочитать.
        for k, v in sorted(counters.items()):
            print(f"  пропуск — {k}: {v}")
        raise SystemExit(
            "записей нет — причины выше; σ требует "
            f"~{MIN_SIGMA_OBS // 23 + 1} суток записи ДО меряных")
