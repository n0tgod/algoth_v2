...
def main():
    ...
    # 7
    add("## 7. Свечи Binance\n")
    add("Загрузчик — `binance_klines.py`, таймфрейм задаётся аргументом.")
    add("Он сверяет контрольные суммы архива и считает пропуски баров")
    add("**внутри наблюдаемого диапазона**: месяц листинга и месяц делистинга")
    add("неполны по построению, и считать их недобором — значит объявлять")
    add("дефектом нормальные данные.\n")
    add("Пилот на шести активах с самой длинной историей, 15m, 468")
    add("символо-месяцев: все контрольные суммы сошлись, 1 360 896 баров,")
    add("**пропусков ноль**. То есть архив Binance по мажорам за 6.5 лет")
    add("сплошной, и гигиена раздела 2.4 упирается не в него.\n")
    add("Объём по всему универсуму — 20 198 символо-месяцев:\n")
    add("| Таймфрейм | Сжатый архив |")
    add("|---|---:|")
    add("| 1m | ~31 ГБ |")
    add("| 5m | ~6.5 ГБ |")
    add("| 15m | ~2.3 ГБ |")
    ...
    add("")
    no_overlap = sorted(
        r["base"] for r in A.values()
        if r["binance_symbol"]
        and (not r["binance_first_month"]
             or min(r["last_trading_day"][:7],
                    r["binance_last_month"] or r["last_trading_day"][:7])
             < r["binance_first_month"])
    )
    add(f"Из {with_bnc} активов с историей Binance в план загрузки входят")
    add(f"{with_bnc - len(no_overlap)}. Исключены {', '.join(no_overlap)}:")
    add("Bybit делистнул их раньше, чем Binance листинговал, — пересечения")
    add("по времени нет вовсе.\n")

    add("## 8. Чего в этом этапе нет\n")
