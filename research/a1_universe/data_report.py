...
def main():
    ...
    if not klines:
        ...
        add("")
        # Загрузка идёт шире отбора, и об этом надо сказать прямо: иначе
        # «720 активов» здесь и 632 крипто-актива в универсуме выглядят
        # противоречием, хотя противоречия нет.
        nc = [b for b in A if universe["assets"].get(b, {}).get("asset_class")
              == "non_crypto"]
        if nc:
            nc_months = sum(A[b]["months_ok"] for b in nc)
            add(f"Из загруженных активов {len(nc)} помечены `non_crypto` "
                f"({nc_months} символо-месяцев, {pct(nc_months, months_ok)} "
                "от объёма).")
            add("В отбор они не идут: `universe_at` их отсеивает, и ни в одном")
            add("историческом окне года истории у них нет. Загрузка намеренно")
            add("шире отбора — метка `asset_class` проставляется по ставке")
            add("комиссии и решение обратимо, а перекачивать 23 ГБ ради смены")
            add("одного признака незачем.\n")
