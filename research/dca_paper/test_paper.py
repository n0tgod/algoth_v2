...
def test_watchdog_runs_short_books_by_the_same_rule():
    ...
    # именно тот прогон, суточной каденцией и в тихий час.
    d = src.index("# --- очередь заданий")
    blk = src[c:d]
    for need in ("a1_universe/funding_refresh.py", "funding-refresh.json",
                 "pgrep", "72000", 'f_hh" = "04"'):
        if need not in blk:
            print(f"  ПАДЕНИЕ сторож/funding: нет «{need}»")
            ok = False
    print("ok  сторож поднимает короткие книги и общий счёт тем же "
          "правилом, а ряды funding догоняет суточно в тихий час: "
          "свежий счёт молчит, идущий прогон не дублируется")
    return ok
