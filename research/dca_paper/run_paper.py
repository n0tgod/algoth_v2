...
отличаются РОВНО им — но билет НЕ один и тот же: он есть максимум из
пола биржи ($25) и доли депозита, которой хватает на все места
(`rules.ticket`). У $1k и $10k связывает пол, у $100k — пик книги: 40 /
400 / 689 мест при билете $25 / $25 / $145. По линейке
...
def report(s):
    ...
    for name, key in (("Наблюдение (записано вперёд)", "forward"),
        ...
        L += [f"## {name}", "",
              "| линейка | депозит | билет | мест | сделок | имён | дней | "
