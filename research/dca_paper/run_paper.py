...
РОВНО им — но билет НЕ один и тот же: он есть максимум из пола биржи
($25) и доли депозита, которой хватает на все места (`rules.ticket`). У
$1k и $10k связывает пол, у $100k — пик книги: 40 / 400 / 689 мест при
билете $25 / $25 / $145. По режиму отличается то, из чего выводится
...
def build_rows(by_ruler, now=None, log=print):
    ...
    for rk in R.RULER_ORDER:
        ...
        # Пик СВОЕЙ книги: билет считается от пика ВСЕГО пула, а у режима
        # с гейтом позиций меньше, то есть его собственный пик ниже и
        # деньги простаивают. Печатается числом, чтобы решение о билете
        # опиралось на замер, а не на догадку.
    ...
    for rk in R.RULER_ORDER:
        for dep in R.DEPOSITS:
            ...
            out["books"][_cell(rk, dep)] = {
                ...
                "slots": R.slots(dep), "ticket": R.ticket(dep),
...
def report(s):
    tk = " / ".join(f"${R.ticket(d):g}" for d in R.DEPOSITS)
    sl = " / ".join(str(R.slots(d)) for d in R.DEPOSITS)
    L = ["# DCA — бумажные книги на трёх депозитах", "",
         ...
         "выходы у них одни. Билет при этом НЕ один и тот же — "
         f"**{tk}**, мест **{sl}**, — и это не вторая ось, а следствие "
         "первой: билет зажат между полом биржи и пиком самой книги.", "",
         "**Пол задаёт биржа.** Минимальный "
         ...
         "поднят так, чтобы хватало на ВСЕ места.** Больше "
         f"{R.PEAK_SEEN} позиций разом книга не держала ни разу за "
         "запись — пик ИЗМЕРЕН по журналу, обе линейки дали одно число, — "
         f"поэтому билет равен `депозит / ({R.PEAK_SEEN} × "
         f"{R.PEAK_MARGIN:g})`, где запас нужен потому, что пик есть "
         "максимум выборки, а сечение растёт. Правило одно на все "
         "депозиты, и оно само говорит главное: **мелкий депозит "
         "наполнить нельзя** — у $1k и $10k связывает пол биржи, и "
         ...
         "**Билет у режима с гейтом НЕ понижен, и это решение, а не "
         "недосмотр.** По собственной арифметике агрессивного режима пол "
         f"был бы вчетверо ниже: он не берёт плеча слабее "
         f"{R.AGGR_MIN_LEV:g}×, значит худший его случай требует "
         f"${R.MIN_NOTIONAL / R.RUNG_SHARE / R.AGGR_MIN_LEV:g} маржи, а "
         "не $20. Понизив билет, мы дали бы книге БОЛЬШЕ мест — то есть "
         "она отличалась бы от «оптимальной» двумя правилами разом, и "
         "разницу результатов нельзя было бы приписать гейту. Плата "
         "названа: позиций у режима меньше, а билет тот же, поэтому часть "
         "денег простаивает, и его процент к депозиту занижен не правилом "
         "рынка, а нашим выбором. Насколько — видно по числу отказов "
         "кассы и по собственному пику режима ниже.", ""]
    ...
    if one:
        L += ["## Гейт плеча и собственный пик режима", "",
              ...
              "что режим взял, и сравнивается с пиком всего пула "
              f"({R.PEAK_SEEN}), из которого выведен билет. Билет общий "
              "на все режимы, а собственный пик режима с гейтом заведомо "
              "ниже — значит часть его денег простаивает, и это цена "
              "нашего выбора, а не свойство правила.", "",
              "| режим | решений | отсеял гейт | после одной на имя | "
              "медиана плеча | свой пик позиций |",
              "|---|--:|--:|--:|--:|--:|"]
        for rk in R.RULER_ORDER:
            o = one.get(rk)
            if not o:
