...
def report(s):
    L = ["# DCA-книги в разрезе руки модели (деревья / сеть)", "",
         ...
         "разрешает КЭШ РЕПЛЕЯ — в нём остаётся одна запись из двух, и "
         "чья, видно по её собственным числам (прогноз и обещание). "
         "Догадка «побеждает больший прогноз» была неверна: замер "
         "`arm_book.py` показал, что все спорные записи в кэше "
         "принадлежат сети.", ""]
    w = s.get("window") or {}
