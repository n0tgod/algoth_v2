...
def _row(rk, dep, sym, at, exit_ts, px, lev, margin, usd):
...
def test_candidates_and_money():
    ...
    rows = [_row("optimal_h", 1000, "AAAUSDT", T0, b - 1, 100.0, 25.0, 40.0, 5.0),
            _row("optimal_h", 10000, "AAAUSDT", T0, b - 1, 100.0, 25.0, 400.0, 50.0),
    ...
    assert abs(o2["usd"] - 100.0) < 1e-9 and o2["usd_trades"] == 50.0, o2
    ...
    # контроль: записанная цена НЕ закрытие часа — контроль это видит числом
