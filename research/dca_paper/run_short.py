...
def report(s):
    ...
    L += ["",
         ...
         "медиана дня | плюсов | " + RP.CONC_HEAD + " вперёд / пересчёт |",
         "|---|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|"]
    for rk in R.H24_ORDER:
        for dep in R.DEPOSITS:
            ...
            L.append(
                ...
                + f" {b.get('n_forward', 0)} / {b.get('n_restored', 0)} |")
    L += ["", "**Колонки концентрации отвечают на разные вопросы.** «Без "
          "лучшего имени» ловит одну разогнанную монету; «без 3 лучших "
          "дней» ловит рыночный эпизод, который раздаёт деньги десяткам "
          "имён разом, и первой колонке он невидим; «просадка без худшего "
          "дня» говорит, сделана ли просадка ОДНИМ днём или месяцем "
          "сползания. Если книга без трёх дней уходит в минус, она "
