...
def enrich(rows, funding, to_asset, taker, log=print):
    ...
    out, miss = [], {"taker_fallback": 0, "no_commission": 0,
    ...
    log(f"строк с исходом {len(out)}; тейкер по умолчанию у "
        f"{miss['taker_fallback']}, без комиссии {miss['no_commission']}, "
...
def run(rows=None, funding=None, assets=None, log=print):
    ...
    n_rows = len(rich)
    cover = ((n_rows - miss["no_funding_series"] - miss["funding_uncovered"])
             / n_rows if n_rows else 0.0)
    fr = sorted(set(float(r["taker_bp"]) for r in rich))
    return {"cells": cells, "arms": arms, "sides": sides, "miss": miss,
            "rows": n_rows, "funding_cover": round(cover, 3),
...
def verdict(s):
    ...
    for k, a in s["arms"].items():
        if not a or a["n_fav"] < 30 or a["median_fav"] is None:
            continue
...
def report(s):
    ...
    P = ["# Издержки бумажных DCA-книг: комиссия, funding, гейт по ставке", "",
         ...
         f"Строк с исходом {s['rows']}; рядов funding {s['funding_assets']}, "
         f"покрытие позиций {s['funding_cover'] * 100:.1f} % при пороге "
         f"вердикта {s['min_cover'] * 100:.0f} %"
         + ("" if v["measurable"] else " — **funding НЕ измерен, колонки нетто "
            "читать нельзя**") + f". Версия правил книг {s['rules_version']}.",
         ""]
    ...
    P += ["## По сторонам (депозит $10 000)", "",
          "| сторона | n | брутто $ | комиссия $ | funding $ | нетто $ | "
    ...
    P += ["", f"Гейт помогает по медиане позиции у {len(v['gate_helps'])} книг"
          ...
          + "; книги с меньше чем 30 прошедшими не судятся.", "",
