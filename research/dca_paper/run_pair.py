...
def short_recs(cache=None, log=print):
    ...
    log("короткие книги: " + ", ".join(f"{k} {len(v)}"
                                       for k, v in sorted(out.items())))
    # Записи без обещания модели — ЧИСЛОМ: у них не будет линии цели на
    # графике, а молчаливый null читался как «у книги цели нет» — так
    # стояли ВСЕ короткие строки общего счёта (2026-09-22: кэш коротких
    # писался до добора обещания). Считаются ЗАПИСИ кэша, а не строки по
    # книгам: одна линейка кормит две книги, и по книгам число удвоилось бы.
    bases = set(S.BOOKS.values())
    no_fav = sum(1 for (rk, _sym, _at), r in cache.items()
                 if rk in bases and r.get("fav_bp") is None)
    if no_fav:
        log(f"короткие записи без обещания модели: {no_fav} — у них не "
            "будет линии цели на графике")
    return out, None
...
def report(s):
    L = ["# Общий счёт: длинная книга и короткая на одном депозите", "",
         ...
         "получили тот же фильтр 2026-09-08 по решению владельца, и "
         "живого контроля у правила с этого дня нет: контроль "
         "расчётный, обе ветки на одном кэше. Гейт по "
    ...
    L += ["## Книги", "",
          ...
          "просадка | медиана дня | плюсов | " + RP.CONC_HEAD
          + " связь сторон | совпадений имён |",
          "|---|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|"]
    for pk in (s.get("rulers") or R.PAIR_ORDER):
        for dep in R.DEPOSITS:
            ...
            L.append(
                ...
                f"{st.get('win', '—')} % |" + RP.conc_cells(st)
                + f" {'—' if lk_.get('corr') is None else lk_['corr']} | "
                f"{col.get('n', 0)}"
                + (f" ({100 * col['share']:.1f} %)" if col.get("share")
                   else "") + " |")
    L += ["", "**Колонки концентрации.** «Без лучшего имени» вычитает одну "
          "разогнанную монету, «без 3 лучших дней» — рыночный эпизод, "
          "раздавший деньги десяткам имён разом (первой колонке он "
          "невидим), «просадка без худшего дня» говорит, сделана ли "
          "просадка одним днём. У общего счёта это вопрос ОБЕИХ сторон "
          "сразу: эпизод, кормящий короткую сторону, обычно тот же, что "
          "бьёт длинную, и наоборот — потому лучший день счёта может "
          "оказаться худшим днём одной из его книг.", ""]
    bad = []
    for pk in (s.get("rulers") or R.PAIR_ORDER):
        for dep in R.DEPOSITS:
            b = (s.get("books") or {}).get(RP._cell(pk, dep)) or {}
