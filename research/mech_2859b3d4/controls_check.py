...
Каждая подделка из `research/factory/out/build.json` применяется к КОПИИ
файла, прогоняется `test_drift.py`, файл восстанавливается и сверяется по
sha256. Требуется не просто падение, а падение ИМЕННО названной проверки:
контроль, роняющий что-то другое, ничего не доказывает о том правиле,
ради которого написан.

Машина та же, что у `research/probe_fshift/controls_check.py`, и это её
второй экземпляр намеренно: свой каталог механики строитель правит, а
чужой — нет. Расчётного ядра здесь нет вовсе, только запуск сюиты.
...
потерянного шага.
    ...
    cd ~/algoth_v2 && .venv/bin/python \\
        research/mech_2859b3d4/controls_check.py
"""
...
SUITE = "research/mech_2859b3d4/test_drift.py"
    ...
    for i, c in enumerate(controls, 1):
        ...
        if orig.count(c["old"]) != 1:
            print(f"  #{i} НЕПРИМЕНИМ: строка встречается "
                  f"{orig.count(c['old'])} раз  [{c['expect']}]")
            bad.append(i)
            continue
