...
def lvl(bp):
    ...
    Порог, издержка, граница — это настройка, а не движение цены, и
    «+0.30 %» у порога читается как прибыль. Пара к `pct`: одно
    правило знаков после запятой, разные решения о знаке. На страницах
    ровно та же пара (`pct` / `lvl`), и расходиться им нельзя.
    """
    if bp is None:
        return "—"
    d = 2 if abs(bp) >= 10 else 3
...
# Решение владельца 2026-08-13: капитал каждой руки каждой книги —
# 3000 $. Счёт — чистая функция от файлов, поэтому вся денежная
# история пересчитывается от нового старта сама; размеры лотов и pnl
# масштабируются, исполнение крупного лота по лесенке честно дороже.
START_BALANCE = 3000.0
# Депозит СТРАТЕГИИ автономной системы (решение владельца 2026-09-02):
# каждой книге-кандидату по умолчанию 1000 $ на руку, и проценты с
# долларами считаются от него. Книги ядра остаются на 3000 — их
# капитал владелец назначил отдельным решением, и подтягивать одно за
# другим значило бы менять забор молча.
CAND_START_BALANCE = 1000.0


def start_of(man):
    """Депозит книги: из её МАНИФЕСТА, а не из константы читателя.

    Манифест есть запись о том, чем книга торговала, и капитал —
    такая же её часть, как слоты и правило размера. Книга без поля —
    книга ядра: умолчание сохраняет прежние числа бит в бит.
    """
    try:
        v = float((man or {}).get("start_balance") or 0.0)
    except (TypeError, ValueError):
        v = 0.0
    return v if v > 0 else START_BALANCE
# Риск на сделку книги равного риска (доля капитала): стоп всегда
# стоит −R, тейк при RR r — +r·R, и математика ожидания в деньгах
# сходится (наблюдение владельца: при одном RR тейк приносил то 20 $,
# то 5 $, а стоп забирал 15). Величина 0.1 % выведена из забора, а не
# из воздуха: потолок на имя 10 % × типичный исполняемый стоп ~1 % —
# это риск, который книга фиксированного размера и так несёт на
# медианной сделке; выравнивается РАСПРЕДЕЛЕНИЕ риска, не его
# уровень. У стопа теснее ~1 % размер упирается в потолок и риск
# выходит меньше целевого — предел забора, помечено `name_capped`.
FIXED_RISK_SHARE = 0.001
...
# v5 — капитал 1000 → 3000 (решение владельца 2026-08-13). Это
# правка правила кассы: журнал тени хранит размеры, посчитанные
# прежним капиталом, и после смены несравним навсегда — сверка
# краснела бы вечно. Смена версии архивирует его при деплое.
CASH_RULES_VERSION = 5
...
# Дневной тормоз — второй ЗАБОР (решение владельца 2026-08-29 по
# реплею `probe_drain/DRAIN-brake-0824.md`): когда реализованный
# результат ВСЕХ торгуемых не-эхо книг за календарные сутки UTC
# опустился до −1 % их суммарного капитала, новые входы до конца суток
# не открываются. Выходы не гасятся никогда: вход — возможность,
# выход — обязанность (урок X2). На записанном эпизоде слива порог
# срезал бы +572 $ из −2061 при цене +7 $ в прибыльной базе; в базе
# правило связывало 65 входов из 8813 (0.7 %) — по той же мерке
# редкости, по которой смена приоритета per σ не отставляла книгу,
# книги при внедрении не архивируются, а заблокированные часы видны
# журналом тормоза и страницей.
#
# Это забор, а не настройка: он не учится и не подбирается по
# результатам. Оговорка записана вместе с числом: порог выведен из
# ОДНОГО эпизода, и его пересмотр — решение владельца, а не подгонка.
DAY_BRAKE_SHARE = 0.01
DAY_BRAKE_STALE_SEC = 900     # состояние старше — тормоз неизвестен
DAY_BRAKE_FILE = "day_brake.json"


def money_ts(t):
    """Момент, когда деньги сделки стали известны.

    То же выражение, что у лиги и страницы learning: живой выход,
    иначе конец часа разбора, иначе открытие. Второй расчёт того же
    момента в тормозе разошёлся бы со страницами молча.
    """
    return t.get("exit_ts") or t.get("closes_at") or t.get("opened_at")


def day_realized(pairs, now_ts):
    """Реализованный результат текущих суток UTC: Σ pnl по моментам
    денег. Пара без момента не входит — неизвестное время не есть
    сегодняшнее."""
...
def day_brake_limit(n_books, arms=2):
    """Порог тормоза в долларах: доля от суммарного капитала торгуемых
    не-эхо книг. Выводится, а не повторяется числом: смена капитала
    или состава книг двигает порог сама (две записи одного числа
    разошлись бы — правило run_bot.sh про версию кассы)."""
    return round(DAY_BRAKE_SHARE * START_BALANCE * arms * n_books, 2)


def read_day_brake(path, now_ts, stale_sec=DAY_BRAKE_STALE_SEC):
    """Состояние тормоза из файла единственного писателя (сборщик).

    Возвращает (state, braked): устаревшее или нечитаемое состояние —
    тормоз НЕИЗВЕСТЕН и не тормозит (fail-open), но состояние несёт
    `stale`, и страница обязана это кричать: защита, которой молча
    нет, — тот же класс отказа, что ionice без BFQ.
    """
    try:
        with open(path, encoding="utf-8") as f:
            st = json.load(f)
        ...
        return None, False
    st = dict(st)
    st["stale"] = (now_ts - float(st.get("at") or 0)) > stale_sec
    return st, day_brake_active(st, now_ts, stale_sec)


def day_brake_active(st, now_ts, stale_sec=DAY_BRAKE_STALE_SEC):
    """Действует ли тормоз СЕЙЧАС. Одно правило на файл и на память
    сборщика — две реализации однажды разошлись бы. Устаревшее
    состояние не тормозит (fail-open, но с криком на странице), и
    тормоз, взведённый вчера, сегодня не действует: правило живёт
    внутри календарных суток UTC."""
    if not st or not st.get("on"):
        return False
    at = float(st.get("at") or 0)
    if now_ts - at > stale_sec:
        return False
    return int(at // 86400) == int(now_ts // 86400)
    ...
    for _, kind, t in ev:
        if kind == 1:
            ...
            if sizing == "fixed_risk":
                # Равный доллар риска: размер обратен исполняемому
                # стопу. Стоп — та же величина, которой гейт считает
                # RR (`mae_bp`, переякоренный уровень): рисковать по
                # одной мерке, а мерить по другой — ошибка единиц.
                # Сделка без стопа размера не получает: неизмеримый
                # риск не есть нулевой (урок замороженных рядов).
                r = abs(t.get("mae_bp") or 0.0)
                want = ((cash + busy) * FIXED_RISK_SHARE
                        / (r / 1e4) if r > 0 else 0.0)
            else:
