...
# Списка «законных ключей адреса» здесь нет намеренно. Он был
# (`addressable`) и уехал на страницу константой, собранной на
# импорте; с появлением книг фабрики это стало ВТОРЫМ списком книг:
# кандидат объявляется каждый час, страница о нём не знает до
# перезапуска, ключ отбрасывается как чужой — и ссылка МОЛЧА
# открывает главную книгу. Существование книги решает резолвер
# сервера (ядро из реестра, кандидаты с диска), а страница проверяет
# только ФОРМУ ключа: он уезжает и в адрес, и в путь на диске.
