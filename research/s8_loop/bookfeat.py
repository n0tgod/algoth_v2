...
def target_pack(s, r, elig, beta):
    ...
    for h in HORIZONS:
        ...
        out[f"mfe_{h}h"] = mfe
        out[f"mae_{h}h"] = mae
        # Цель в единицах СОБСТВЕННОЙ волатильности монеты — теперь на
        # ВСЕХ горизонтах, а не только на сигнальном. Раньше её хватало
        # одному сечению, потому что книга в σ была одна; решением
        # владельца на этот порядок переводятся книги 1 ч и 24 ч, а
        # порядок сечения нельзя задать целью, которой не существует.
        # Делитель уже несёт √h, поэтому обобщение честное: у каждого
        # горизонта своя мера разброса, а не пересчитанная с чужого.
        #
