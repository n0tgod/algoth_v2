...
def write(d, out_path):
    ...
    demo = "model_demo" in os.path.basename(os.path.normpath(d))
    L = [f"# S8 — {'показ вывода на синтетике' if demo else 'пробный прогон конвейера'}",
         ""]
    if demo:
        L += ["> **Данные здесь выдуманы целиком, сигнал заложен руками.** "
              "Отчёт отвечает на вопрос «что я буду видеть», когда "
              "накопится история: какие строки появятся, что напишут "
              "мысли, как выглядят выбор, разбор фактом и счёт. Числа "
              "не значат ничего о рынке.", ""]
    ...
    if man and not man.get("probe") and not demo:
    ...
    if rd:
        ...
        if bmin is not None and hps is not None:
            # Оговорку печатать только когда она верна: «300 при 96, а
            # без беты нет цели» читалось бы как поломка на данных, где
            # всё в порядке.
            L += [f"- часов годной истории на монету: **{hps}** при "
                  f"{bmin}, нужных бете"
                  + (" — а без беты нет цели `fwd_4h`, то есть ни "
                     "выбора монет, ни живого IC" if hps < bmin
                     else " — хватает")]
        L += ["",
    ...
    L += ["## Числа прогона", "",
          ("Числа получены на выдуманных данных с заложенным сигналом: "
           "они показывают, КАК выглядит вывод, и ничего не говорят о "
           "рынке." if demo else
