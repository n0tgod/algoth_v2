...
class NN:
    ...
    def contrib(self, x):
        """Вклад признака: прогноз минус прогноз «будь признак обычным».

        Каждый признак по очереди заменяется медианой обучения (флаг
        пропуска при этом гаснет — «данные были и были обычными»), и
        разница предсказаний приписывается ему. Мера честно ГРУБЕЕ,
        чем разложение у деревьев: из-за взаимодействий сумма вкладов
        не обязана сходиться к предсказанию, поэтому тождества здесь
        нет и тест его не требует — только что заложенный в синтетику
        драйвер получает наибольший вклад. На странице эти числа
        подписываются как вклад В ЭТОТ прогноз, единица та же — б.п.
        """
        base = self.predict(x)
