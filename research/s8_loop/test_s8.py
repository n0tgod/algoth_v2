...
def test_merge_adds_shows_one_position():
    ...
    # Деньги головной строки описывают ВСЮ позицию. Головная сделка —
    # копия первого лота, и без этого показ печатал бы размер позиции
    # рядом с прибылью одного лота.
    def lot2(px, size, opened, closes, net, state="закрыта", ex=None,
             live=None):
        q = lot(px, size, opened, closes, state, net)
        q["got_bp"] = net
        if state == "закрыта":
            q["exit_px"] = ex
            q["exit_ts"] = closes
            q["exit_reason"] = f"выход {opened}"
        else:
            q["unreal_net_bp"] = live
        return q

    # 100 $ дали +20 б.п., 100 $ дали −40; обе разгрузки закрыты.
    full = TR.merge_adds([lot2(100.0, 100.0, 0, 3600, 20.0, ex=100.2),
                          lot2(100.0, 100.0, 600, 7200, -40.0, ex=99.6)])
    fh = full[0]
    check("прибыль позиции — сумма лотов, а не первого",
          abs(fh["pnl"] - (0.2 - 0.4)) < 1e-9, str(fh["pnl"]))
    check("ход позиции взвешен размером",
          fh["net_bp"] == -10.0 and fh["got_bp"] == -10.0,
          f"{fh['net_bp']} / {fh['got_bp']}")
    check("конец позиции — ПОСЛЕДНЯЯ разгрузка, а не первый выход",
          fh["state"] == "закрыта" and fh["exit_px"] == 99.6
          and fh["exit_ts"] == 7200,
          f"{fh['state']} / {fh['exit_px']} / {fh['exit_ts']}")
    # Первый лот закрыт, второй жив: позиция НЕ закрыта, и открытая
    # часть идёт отдельной отметкой, а не складывается с закрытой.
    part = TR.merge_adds(
        [lot2(100.0, 100.0, 0, 3600, 20.0, ex=100.2),
         lot2(100.0, 100.0, 600, 7200, 0.0, state="открыта", live=55.0)])
    ph = part[0]
    check("позиция с живым лотом закрытой не называется",
          ph["state"] == "открыта" and ph["lots_closed"] == 1,
          f"{ph['state']} / {ph['lots_closed']}")
    check("у незакрытой позиции цены выхода нет вовсе",
          ph.get("exit_px") is None, str(ph.get("exit_px")))
    check("закрытая часть и открытая идут РАЗНЫМИ числами",
          ph["pnl"] == 0.2 and ph["net_bp"] == 20.0
          and ph["unreal_net_bp"] == 55.0,
          f"{ph['pnl']} / {ph['net_bp']} / {ph['unreal_net_bp']}")
...
def test_backfill_recovers_the_book_for_old_trades():
    ...
    try:
        ...
        # Подарок входа у пересчитанной сделки: своего поля у выбора нет
        # (история не правится), но середина в момент решения лежит в
        # дописанной книге. Не взять её оттуда значило бы потерять
        # величину, которая в данных есть.
        check("подарок считается и у пересчитанной сделки",
              by["AUSDT"]["gift_bp"] == 0.0
              and by["AUSDT"]["px_live"] == 100.0,
              str({k: by["AUSDT"].get(k)
                   for k in ("gift_bp", "px_live", "entry_px")}))
...
def test_train_cycle_end_to_end():
    ...
    try:
        ...
        # неотличима от сломанной. Часовой книги нет: удалена решением
        # владельца, её каталог не пишется вовсе.
        check("удалённая часовая книга не ведётся",
              not os.path.exists(T.MODEL_DIR + "_h1"),
              "каталог model_h1 создан")
        for hh in (24,):
