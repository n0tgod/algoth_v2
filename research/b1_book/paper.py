...
def current(trades, ver):
    """Сделки текущей версии правил.

    После правки правила подъём истории поднимает и старые сделки. Если
    считать их вместе, сводка смешивает две геометрии: числа выходят
    бессмысленными, а на вид остаются осмысленными — худший вид отказа.
    """
    return [t for t in trades if int(t.get("ver") or 1) == ver]
