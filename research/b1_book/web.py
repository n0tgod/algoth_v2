...
function intro(d){
  const r = d.rules || {};
  const tks = r.TICKETS || {}, dd = (r.DEPOSITS || []).map(
    x => "$" + x.toLocaleString("en-US")).join(" / ");
  const t = (r.DEPOSITS || []).map(x => {
    const v = tks[String(Math.trunc(x))];
    return v == null ? "&mdash;" : "$" + v;
  }).join(" / ") || (r.TICKET == null ? "&mdash;" : "$" + r.TICKET);
  ...
  h += "<p><b>Книги отличаются двумя вещами, и только ими: РЕЖИМОМ и " +
    ...
    "Билет при этом НЕ один — <b>" + t + "</b> — и " +
    "это не третья ось, а следствие депозита: он зажат полом биржи " +
    "снизу и пиком самой книги сверху.</p>";
  h += "<p><b>Пол задаёт биржа, потолок — книга.</b> Минимальный ордер " +
    "$5, мельчайший рунг лестницы 25 % нотионала, значит маржа не " +
    "бывает меньше $20/плечо, а забор выдаёт от 1&times; &mdash; отсюда " +
    "пол $25 с четвертью запаса. Сверху билет ограничен тем, чтобы " +
    "хватало на ВСЕ места: больше " +
    (r.PEAK_SEEN == null ? "&mdash;" : r.PEAK_SEEN) +
    " позиций разом книга не держала ни разу за запись (пик измерен по " +
    "журналу), и билет равен доле депозита на этот пик с запасом " +
    (r.PEAK_MARGIN == null ? "&mdash;" : r.PEAK_MARGIN + "&times;") +
    ". Отсюда главное: <b>мелкий депозит наполнить нельзя</b> &mdash; " +
    "у него связывает пол биржи, и только у крупного потолок оказался " +
    "выше пола.</p>";
  const rl = d.rulers || [];
  if (rl.length){
    ...
    // сказать, чем за него плачено: билет ему НЕ понижен, значит часть
    // денег простаивает, и процент к депозиту занижен нашим выбором, а
    // не рынком. Понизив билет, режим отличался бы от соседней книги
    // двумя правилами разом, и разницу нельзя было бы приписать гейту.
    for (const r of rl.filter(x => x.min_lev != null))
      h += "<p><b>У режима «" + esc(r.title || r.key) + "» есть гейт " +
        "входа: плечо не ниже " + r.min_lev + "&times;.</b> Порог не " +
        ...
        "запись вперёд. <span class=dim>Билет ему НЕ понижен намеренно: " +
        "по своей арифметике пол был бы вчетверо ниже, но другой билет " +
        "дал бы другое число мест, и разницу с соседней книгой нельзя " +
        "было бы приписать гейту. Плата за это — часть денег " +
        "простаивает.</span></p>";
  }
