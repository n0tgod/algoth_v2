...
function draw() {
  ...
  for (const t of MT) {
    ...
    // Средняя цена позиции входит в диапазон ВСЕГДА: по ней проходит
    // граница зон, и зона, не достающая до собственной границы,
    // прижала бы её к краю окна — то есть нарисовала бы не там, где
    // она есть.
    const wk = t.walk || [];
    const wkv = wk.map(s => s.avg);
    for (const v of (actsOnLevels() ? [pAdv, pFav, ex].concat(wkv)
                                    : [ex].concat(wkv))) {
    ...
    // Плавающая ТВХ — ступени средней цены позиции после каждого рунга.
    ...
    //
    // Граница зон идёт по СРЕДНЕЙ цене позиции, а не по первому рунгу
    // (просьба владельца): в момент времени позиция в плюсе или в
    // минусе относительно СВОЕЙ средней, и долив её опускает. Пока
    // граница стояла на первом входе, зелёное начиналось выше цены,
    // которую позиция уже отбила. Ступени те же, что у линии ТВХ, —
    // иначе картинка спорила бы сама с собой.
    if (xb > xa + 1 && yBot > yTop) {
      const zone = (x0, x1, y0, y1, colr) => {
        if (y1 - y0 < 1 || x1 - x0 < 1) return;
        g.fillStyle = colr; g.globalAlpha = base * (me ? 0.16 : 0.09);
        g.fillRect(x0, y0, x1 - x0, y1 - y0);
      };
      const above = up ? css("--bid") : css("--ask");
      const below = up ? css("--ask") : css("--bid");
      // Ступень выше или ниже окна цены зоны не рисует, а прижимается
      // к его краю: полоса за краем неотличима от полосы у края, но
      // «нет заливки» читалось бы как «зоны у сделки нет».
      const parts = segs.length
        ? segs.map(([x0, x1, yv]) => [Math.max(x0, xa), Math.min(x1, xb),
                                      Math.min(Math.max(yv, yTop), yBot)])
        : [[xa, xb, ye]];
      for (const [x0, x1, yv] of parts) {
        zone(x0, x1, yTop, yv, above);
        zone(x0, x1, yv, yBot, below);
      }
    }
    g.globalAlpha = base;
    ...
    // сказать, от какой цены считается результат.
    //
    // Ступени взяты те же, что разделили зоны выше (`segs`): два счёта
    // одной величины однажды разошлись бы, и заливка спорила бы с
    // линией. Список решает и то, была ли линия нарисована: подпись
    // рядом с ненарисованной линией печатается из данных, а не из
...
// Полная дата, а не «ММ-ДД»: владелец прочёл «08-09 23:09» как 8 сентября
// (день-месяц по-русски) и увидел сделку из будущего. Год стоит ради
// однозначности — как всюду на страницах, а не ради экономии знаков.
...
function intro(d){
  ...
  // Тейк — правило, решающее ИСХОД сделки, и молчать о нём на странице
  // книги нельзя: владелец видит кривую, а не константы прогона. Числа
  // приходят из артефакта, а не записаны здесь: правило меняется, и
  // вторая его запись однажды разошлась бы с той, которой торгуют.
  if (r.TAKE_ANCHOR || r.TAKE_MULT != null) {
    h += "<p><b>Тейк: цель " +
      (r.TAKE_ANCHOR === "avg"
        ? "от ПЛАВАЮЩЕЙ ТВХ &mdash; она едет вниз с каждым доливом, и " +
          "цель едет вместе с ней"
        : "от цены входа &mdash; неподвижная") +
      ", расстояние " +
      (r.TAKE_MULT == null ? "&mdash;" : "&times;" + r.TAKE_MULT) +
      " обещания модели.</b> " +
      "<span class=dim>Оба множителя выведены сеткой из 17 объявленных " +
      "до прогона ячеек (замер D8, 27.7 суток): якорь взят за " +
      "согласованность &mdash; тейк по ТВХ платит тождественно " +
      "«заполненный нотионал &times; доля», &mdash; а расстояние за " +
      "деньги и форму: обещание стояло на 2.56 % при суточной σ имени " +
      "5.55 %, то есть ниже половины дневного размаха монеты.</span></p>";
  }
  h += "<p><b>Записанное вперёд и пересчёт по прошлому не складываются " +
    ...
    " плюс двое суток на прогон)" +
    (r.RULES_SINCE
      ? " И принято НЕ РАНЬШЕ момента, с которого действует нынешняя " +
        "версия правил: решение старше него есть бэктест по построению " +
        "&mdash; правило выбрано после того, как эти часы стали видны"
      : "") +
    ". Первый прогон восстанавливает " +
...
function shortBlock(sh){
  ...
  h += "<p class=dim>Каждая из трёх книг стоит и отдельной вкладкой " +
    "выше (с её сделками и графиком); здесь они собраны разом. " +
    "Вход &mdash; короткие выборы книги со сроком " +
...
function pairBlock(b){
  ...
  // «у нас минимализм в интерфейсе должен быть». Стороны, связь их
  // дневных денег, совпадения имён и сравнение с раздельными счетами
  // живут в отчёте `DCA-pair.md`, где их читают числами.
  //
...
function dupLine(dd){
  ...
  // одному имени; повтор после закрытия им не считается (биржа его
  // допускает) и стоит рядом своим числом. Вердикт выводится ИЗ ЧИСЛА:
  // фраза рядом с числом стареет молча, а поля нет — так и сказано.
  if (!dd) return "<div class=panel><p class=dim>Правило «одно имя &mdash; " +
    "одна позиция» на этой книге НЕ ПРОВЕРЯЛОСЬ: свод прежнего образца " +
    "поля не несёт. Это не то же самое, что «дублей нет».</p></div>";
  const bad = (dd.overlaps || 0) > 0;
  const pause = (dd.pause_median_h === null || dd.pause_median_h === undefined)
    ? "&mdash;" : (dd.pause_median_h + " ч");
  return "<div class=panel>" + (bad
    ? "<p class=bad><b>Дубли: " + dd.overlaps + ".</b> Две позиции книги по " +
      "одному имени пересекаются во времени &mdash; это дефект правила " +
      "«одна на имя», а не свойство рынка.</p>"
    : "<p><b>Дублей нет.</b> Ни одна пара позиций книги по одному имени не " +
      "пересекается во времени: " + dd.positions + " позиций, " + dd.names +
      " имён.</p>") +
    "<p class=dim>Повторных входов ПОСЛЕ закрытия " + dd.repeats +
    ", медиана паузы " + pause + ", больше всего входов в одно имя " +
    dd.max_per_name + ". Повтор после закрытия дублем не считается: " +
    "биржа его допускает, и книга им живёт.</p></div>";
}

function costsWarn(ct, meta){
  // Издержки УЧТЕНЫ В КАЖДОЙ СДЕЛКЕ, и деньги книги выше — уже нетто.
  // Своих плиток у них здесь нет: владелец 2026-09-07 попросил ничего
  // не добавлять на страницу, издержки должны просто сидеть в
  // результате. Молчать совсем нельзя ровно в двух случаях, и оба —
  // про «не измерено», а не про величину: если издержки НЕ вычтены,
  // деньги выше брутто и выглядят как нетто; если у части сделок
  // что-то не измеримо, они брутто внутри общего числа. Всё
  // измерено — эта функция не рисует НИЧЕГО.
  if (ct === undefined || ct === null)
    return "<p class=bad>Издержки в этих деньгах НЕ учтены: свод " +
      "прежнего образца их не считал. Числа выше &mdash; БРУТТО.</p>";
  if (!ct.applied)
    return "<p class=bad>Издержки не вычтены" +
      ((meta && meta.error) ? ": " + esc(meta.error) : "") +
      ". Числа выше &mdash; БРУТТО.</p>";
  let w = "";
  if (ct.not_measured) w += "у " + ct.not_measured + " сделок из " + ct.n +
    " нет записи рунгов &mdash; издержки им не вычтены вовсе";
  if (ct.no_funding) w += (w ? "; " : "") + "у " + ct.no_funding +
    " сделок ряд funding не покрывает позицию &mdash; funding им не " +
    "начислен (прочерк, не ноль)";
  return w ? "<p class=dim>" + w + ".</p>" : "";
}
...
function statBlock(st, dep, title, op, grp, nt, meta){
  ...
  const cells = [
    ...
    // Сколько сделок закрыто по котировке: у тонкого имени хвост окна
    // пуст не потому, что цены не было, а потому, что по ней не
    // торговали. Прочерк означает «прогон этого не считал» (свод
    // прежнего образца), а не ноль.
...
function posBlock(b, grp){
  ...
  h += "<p class=k>Одна строка &mdash; ОДНА ПОЗИЦИЯ по паре: доливы " +
    ...
    (grp === "fwd"
      ...
      : "Пометка <span class=tag>бэктест</span> означает пересчёт по "
        ...
        + "отдельно.") +
    " Пометка <span class=tag>по котировке</span> означает, что исход " +
    "посчитан по СЕРЕДИНЕ СТАКАНА: у тонкого имени хвост окна пуст не " +
    "потому, что цены не было, а потому, что по ней не торговали, и " +
    "лента в хвосте продолжается котировкой. Цена наблюдена, а не " +
    "оценена; лимитка (рунг и тейк) на такой минуте не заполняется " +
    "вовсе &mdash; её исполняет чужой принт." +
  ...
  for (const r of win){
    ...
    h += "<tr class=pos onclick=\"dcaToggle('" + esc(key) + "')\">" +
      ...
      // У оборванной причина стоит РЯДОМ с состоянием: правило хвоста
...
function render(){
  ...
  h += statBlock(st, b.deposit || DEP,
                 ...
                 op, GRP, nt, d.costs);
  // Смена правил семейства обнуляет «записанное вперёд»: решения,
  // писанные другими правилами, в счёт этих правил не идут. Без этой
  // строки короткая запись читается как «книга перестала торговать» —
  // владелец так её и прочитал 08.09. Число приходит с сервера
  // (`rules.FAMILY_SINCE`), страница своей даты не выдумывает.
  const fsince = ((d.rules || {}).FAMILY_SINCE || {})[rmeta.family];
  if (GRP === "fwd" && fsince) h += "<div class=panel><p class=dim>" +
    "Запись вперёд у этой книги идёт с <b>" + esc(fsince) + "</b>: в " +
    "этот день сменились правила её семейства, и решения, писанные " +
    "прежними правилами, в счёт нынешних не идут. Это не остановка " +
    "книги &mdash; сделки в группе «с бэктестом» считаются как прежде." +
    "</p></div>";
  // Общая статистика стоит ВЫСОКО, сразу под плитками книги: она и есть
  // ответ на вопрос «что даёт короткая книга рядом с длинной», и внизу
  // страницы её никто не находит (проверено владельцем 07.09).
  h += dupLine(b.dups);
  h += dayTable(st, b.deposit || DEP, "по суткам");
  // Сводка трёх коротких книг — ниже: сами книги теперь стоят вкладками
  // рядом с длинными, и таблица нужна как «все три разом», а не как
