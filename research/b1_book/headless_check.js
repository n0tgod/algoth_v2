...
// Контракты рунга в подставном ответе: доля нотионала, делённая на цену
// рунга, — ровно то, что кладёт в ответ сервер (`rules.avg_walk`). Это
// ДАННЫЕ заглушки, а не второе правило: страница их только печатает, а
// тождество с симуляцией закреплено серверным тестом
...
const dcaStub = (url) => {
  ...
  const mk = (n, usd, dd, wd, top, wo, wo3, bt) => ({
    ...
    // Доля прибыльных ПОЗИЦИЙ и среднее время в сделке — их считает
    // прогон, и заглушка обязана выглядеть как живой артефакт. Числа
    // выводятся из денег группы: совпади они у групп, проверка
    // «переключение меняет числа» прошла бы на сломанном ключе.
    // сколько сделок закрыто по КОТИРОВКЕ (хвост ленты продолжен
    ...
    // Семь суток, а не три: разбивка сворачивается до пяти последних,
    // и на трёх правило не сработало бы вовсе — проверка прошла бы на
    // фикстуре, которой нечего сворачивать. Доли добавленных дней
    // гасят друг друга, поэтому сумма по дням по-прежнему равна итогу
    // книги: расходись они, таблица спорила бы со сводкой.
    days_rows: [{d: "2026-08-30", usd: usd * 0.1, n: 4, bt: 4},
                {d: "2026-08-31", usd: usd * -0.1, n: 2, bt: 2},
                {d: "2026-09-01", usd: usd * 0.5, n: Math.ceil(n / 2),
                ...
                {d: "2026-09-04", usd: usd * 0.15, n: 6, bt: 0},
  ...
  const tr = (sym, usd, lev, bt, tail, side) => ({
    ...
    // Контракты дописывает СЕРВЕР той же функцией, что ТВХ
    // (`rules.avg_walk` с нотионалом = маржа × плечо), — заглушка
    // обязана выглядеть как живой ответ, иначе проверка «контракты
    // показаны» шла бы на данных, которых у живого ответа не бывает.
    walk: dcaq(25.0, lev, [{at: T0 - 7200, px: 2.0, w: 0.25, avg: 2.0},
                           {at: T0 - 5400, px: 1.6, w: 0.25, avg: 1.8}]),
    // исход посчитан по котировке — назначается СТРОКЕ явно, чтобы
    // пометка отличала строку, а не красила все
    tail: tail === true});
  ...
  const D = {
    ...
    rules: {RULES: 2, TICKET: 25.0, TICKET_MIN: 25.0,
            // билеты ПО РЕЖИМАМ: у режима с гейтом свой пол и свой пик
            TICKETS: {safe: {"1000": 25.0, "10000": 25.0, "100000": 145.0},
                      optimal: {"1000": 25.0, "10000": 25.0,
                                "100000": 145.0},
                      aggr: {"1000": 6.25, "10000": 28.0, "100000": 281.0}},
            FLOORS: {safe: 25.0, optimal: 25.0, aggr: 6.25},
            PEAKS: {safe: 457, optimal: 457, aggr: 237},
            PEAK_SEEN: 457, PEAK_MARGIN: 1.5,
            DEPOSITS: [1000, 10000, 100000],
            AHEAD_H: 120, HOLD_H: 72, ONE_PER_NAME: true,
            // правило тейка: якорь и расстояние приходят ОТВЕТОМ, а не
            // записаны на странице — правило меняется, и вторая его
            // запись однажды разошлась бы с той, которой торгуют
            TAKE_ANCHOR: "avg", TAKE_MULT: 2.0,
            RULES_SINCE: 1788615790.0,
    ...
    if (!nolive) b.open = {
      positions: [{sym: "SUIUSDT", at: T0 - 1800, lev: 3.0, margin: 25.0,
                  ...
                  {sym: "TIAUSDT", at: T0 - 5400, lev: 2.0, margin: 25.0,
                   ...
                   walk: dcaq(25.0, 2.0,
                     [{at: T0 - 5400, px: 4.0, w: 0.25, avg: 4.0}])}],
      // Худшую открытую дописывает СЕРВЕР (`rules.open_stats`), значит
      // живой ответ её несёт — заглушка обязана нести тоже.
      // Причину обрыва дописывает СЕРВЕР (`tail.cut_reason`, а записи
      // прежнего прогона добирает `collect`) — заглушка обязана нести
      ...
      cut: [{sym: "CUTUSDT", at: T0 - 9000, lev: 1.0, margin: 25.0,
             ...
             walk: dcaq(25.0, 1.0,
               [{at: T0 - 9000, px: 10.0, w: 0.25, avg: 10.0}])}],
  ...
  const body = url.startsWith("/model_marks")
             ...
             ? {present: true, book: "safe:10000", ruler: "safe",
                ...
                rows: [{sym: "BTCUSDT", arm: "dca",
                        ...
                        // до правки она уезжала полем `size`, и подсказка
                        // печатала «0.25 $» вместо «15.00 $».
...
new Function(js + "\nglobal.__step = typeof tick !== 'undefined' "
                ...
                + "? dcaFull : null;"
                // Разбивка по суткам: свёрнута до последних пяти и
                // разворачивается НАСТОЯЩЕЙ функцией страницы —
                // своя копия поведения в проверке гоняла бы не ту
                // дорогу.
                + "\nglobal.__dcaDays = typeof dcaDays === 'function' "
                + "? dcaDays : null;"
...
(async () => {
  ...
  if (isDca) {
    ...
    // билеты РАЗНЫЕ У РЕЖИМОВ, и это видно числами, а не словом:
    // у режима с гейтом свой пол ($6.25) и свой пик (237)
    if (!/\$25 \/ \$25 \/ \$145/.test(intro))
      bad.push("DCA: билеты книг без гейта не названы числами");
    if (!/\$6\.25 \/ \$28 \/ \$281/.test(intro))
      bad.push("DCA: билеты книг с гейтом не названы числами");
    if (!/457/.test(intro) || !/237/.test(intro) || !/1\.5/.test(intro))
      bad.push("DCA: пики режимов и запас не названы числом");
    if (!/одной формулой|ОДНОЙ формулой/.test(intro))
      bad.push("DCA: разные билеты выданы за отдельную ось");
    if (!/У имени позиция одна/.test(intro))
      bad.push("DCA: биржевое правило одной позиции не названо");
    // Тейк решает ИСХОД сделки, и его правило обязано стоять на странице
    // ЧИСЛАМИ из ответа: страница, молчащая о правиле, по которому книга
    // закрывает позиции, выглядит исправной и не отвечает за кривую.
    if (!/от ПЛАВАЮЩЕЙ ТВХ/.test(intro))
      bad.push("DCA: якорь тейка не назван");
    if (!/&times;2 обещания|×2 обещания/.test(intro))
      bad.push("DCA: расстояние тейка не названо числом");
    if (!/бэктест по построению/.test(intro))
      bad.push("DCA: граница версии правил не названа");
    ...
    if (!/Билет у него свой/.test(intro))
      bad.push("DCA: собственный билет режима с гейтом не объяснён");
    // средняя загрузка у режима с гейтом остаётся ниже, и это свойство
    // ПРАВИЛА, а не наш недогруз — молча это читалось бы наоборот
    if (!/средняя загрузка/.test(intro))
      bad.push("DCA: остаточная недогрузка режима с гейтом не названа");
    ...
    // сложить.
    const gtabs = String(global.__el ? global.__el("gtabs").innerHTML : "");
    const nG = (gtabs.match(/class='tab/g) || []).length;
    if (nG !== 2)
      bad.push(`DCA: переключателей группы ${nG}, а их должно быть два`);
    if (!/с бэктестом/.test(gtabs) || !/без бэктеста/.test(gtabs))
      bad.push("DCA: группы не подписаны «с бэктестом» и «без бэктеста»");
    if (!/class='tab on' data-grp='all'/.test(gtabs))
      bad.push("DCA: по умолчанию открыта не общая кривая");
    // $1000: у безопасной книги наблюдения ещё нет вовсе
    const b1 = flat();
    if (!/40/.test(b1)) bad.push("DCA: число мест книги $1000 не показано");
    // открыта БЕЗОПАСНАЯ книга $1000 — её числа, а не соседней линейки
    if (!/\+19\.80 \$/.test(b1))
      bad.push("DCA: деньги безопасной книги $1000 не показаны");
    if (/\+41\.20 \$/.test(b1))
    ...
    // Пометка «по котировке» стоит У СВОЕЙ строки: в книге две сделки,
    // помечена одна. Считается ЧИСЛОМ — «пометка есть где-то» прошло бы
    // и на списке, покрашенном целиком.
    {
      // считается САМА ПОМЕТКА (разметка чипа), а не слова: подпись
      // плитки «из них по котировке» содержит те же слова, и счёт по
      // тексту прошёл бы на неотмеченной строке
      const nt = (b1.match(/<span class=tag>по котировке<\/span>/g)
                  || []).length;
      // одна у строки из двух плюс одна в легенде списка
      if (nt !== 2)
        bad.push(`DCA: пометок «по котировке» ${nt}, ждали 2 `
                 + "(одна строка из двух плюс легенда)");
    ...
    // ОБЩИЙ счёт: бэктест и записанное вперёд ведутся одной кривой
    if (!/\+412\.30 \$/.test(b2))
      bad.push("DCA: общий счёт книги $10000 не показан");
    if (!/общий счёт/.test(b2))
      bad.push("DCA: группа «с бэктестом» не подписана общим счётом");
    ...
    if (!/из них бэктест/.test(b2))
      bad.push("DCA: доля бэктеста в общем счёте не показана");
    // Сколько сделок посчитано по КОТИРОВКЕ: хвост ленты продолжен
    // серединой стакана, и деньги по котировке обязаны быть отличимы от
    // денег по сделкам. Число берётся у заглушки, а не «слово есть».
    if (b2.indexOf("из них по котировке") < 0)
      bad.push("DCA: число сделок по котировке не показано");
    else if (b2.indexOf(String(Math.max(1, Math.round(2840 * 0.1)))) < 0)
      bad.push("DCA: число сделок по котировке напечатано не из ответа");
    // ЕДИНИЦА: свод хранит доли, а общий формат говорит на базисных
    // пунктах — «к депозиту», просадка, медиана и худший день печатались
    // нулями. Ожидание берётся у САМОЙ страницы (`__fpct`), иначе оно
    // проверяет память о правиле формата, а не правило.
    if (global.__fpct){
      const wDep = global.__fpct(412.3 / 10000);   // как считает заглушка
      if (b2.indexOf(wDep) < 0)
        bad.push("DCA: доля к депозиту напечатана не в процентах "
                 + `(ждали ${wDep})`);
      const wDD = global.__fpct(-0.031);
      if (b2.indexOf(wDD) < 0)
        bad.push(`DCA: просадка депозита напечатана не в процентах (${wDD})`);
      if (/к депозиту<\/div><div class='v mono [a-z]*'>[+-]0\.00/.test(b2))
        bad.push("DCA: доля к депозиту схлопнута в ноль форматом");
    } else bad.push("DCA: формат долей не выведен наружу");
    // Новые меры формы: просадка депозита, доля прибыльных СДЕЛОК (не
    // дней), среднее время в сделке
    for (const t of ["просадка депозита", "прибыльных сделок",
                     "среднее время в сделке"])
      if (b2.indexOf(t) < 0) bad.push(`DCA: нет меры «${t}»`);
    if (!/52\.3 %/.test(b2))
      bad.push("DCA: доля прибыльных сделок не показана числом");
    if (!/24\.3 ч/.test(b2))
      bad.push("DCA: среднее время в сделке не показано числом");
    // Кривая: точек СТОЛЬКО ЖЕ, сколько суток в группе — «блок есть»
    // прошло бы и на пустой ломаной
    const pl = /<polyline points='([^']+)'/.exec(b2);
    if (!pl) bad.push("DCA: кривой нет вовсе");
    else {
      const np = pl[1].trim().split(/\s+/).length;
      if (np !== 7) bad.push(`DCA: точек кривой ${np}, а суток в группе 7`);
    }
    ...
    if (!/dcanolive=1/.test(SEARCH)) {
      ...
      // Контракты позиции (просьба владельца 2026-09-05): маржа одна и
      // та же у всех строк, а МОНЕТ в позиции она не называет вовсе —
      // у лестницы их набирают рунгами. Число приходит с сервера
      // (`rules.avg_walk` с нотионалом), страница печатает пришедшее.
      // У TIAUSDT это ровно 0.25 × 25 $ × 2 / 4.00 = 3.125 монеты.
      if (!/контрактов/.test(pb))
        bad.push("DCA: колонки контрактов нет в списке позиций");
      if (rTia && rTia.indexOf("3.13") < 0)
        bad.push("DCA: контракты открытой позиции не показаны в строке: "
                 + rTia.slice(-260));
      // Разворот показывает КАЖДЫЙ рунг: сколько куплено им и сколько
      // стало в позиции после него. Без этого «общее количество» не
      // проверить составом, а «купили/продали» не видно вовсе.
      if (!/куплено/.test(pb) || !/стало/.test(pb))
        bad.push("DCA: в развороте позиции нет контрактов рунга");
      ...
      if (global.__dcaState){
        ...
        // Причина обрыва стоит РЯДОМ с состоянием: правило хвоста
        // снимает не всякий обрыв, и «оборвана записью» без причины
        // читается как «правило не работает».
        if (!/книги в хвосте нет вовсе/.test(pc))
          bad.push("DCA: у оборванной позиции не названа причина");
    ...
    // Разбивка по суткам. Свежий день СВЕРХУ (просьба владельца), и
    // видны последние пять — остальное по кнопке: двадцать восемь
    // строк вытесняли последнюю, ради которой страницу и открывают.
    // Даты ищутся В САМОЙ ТАБЛИЦЕ суток: на странице есть и другие
    // (окно решений, позиции), и по всей разметке проверка считала бы
    // не то, что проверяет.
    const dayCells = () => {
      const all = global.__el("box").innerHTML;
      const i = all.indexOf("по суткам");
      const j = i < 0 ? -1 : all.indexOf("</table>", i);
      if (i < 0 || j < 0) return [];
      return (all.slice(i, j).match(/\d{4}-\d{2}-\d{2}/g) || []);
    };
    const d5 = dayCells();
    if (d5.length !== 5)
      bad.push(`DCA: по умолчанию показано не пять суток, а ${d5.length}`);
    if (d5[0] !== "2026-09-05")
      bad.push(`DCA: свежий день не первый в разбивке: ${d5[0]}`);
    if (d5[0] < d5[d5.length - 1])
      bad.push("DCA: порядок суток не перевёрнут");
    if (d5.includes("2026-08-30"))
      bad.push("DCA: свёрнутый хвост суток всё равно показан");
    if (!/Показаны последние 5 из 7 суток/.test(b2))
      bad.push("DCA: свёрнутое не названо числом");
    // Кнопка обязана быть НАРИСОВАНА и ПРИВЯЗАНА. Заглушка DOM узлов
    // по селектору не отдаёт, поэтому нажатие тут не наблюдаемо:
    // рисунок проверяется разметкой, дорога — источником. Без второй
    // половины снятие привязки проходило бы зелёным — проверки ниже
    // дёргают функцию страницы напрямую и кнопки не касаются.
    if (!/<button class=btn data-days>/.test(b2))
      bad.push("DCA: кнопки разворота суток нет в разметке");
    if (!/\[data-days\]"\)\)\s*\n\s*el\.onclick = \(\) => dcaDays\(\)/
        .test(src))
      bad.push("DCA: кнопка разворота суток ни к чему не привязана");
    // Экспорт обязателен: пропустив проверки при его отсутствии, мы
    // получили бы «зелено» ровно там, где кнопку перестали подключать.
    if (!global.__dcaDays)
      bad.push("DCA: страница не отдала переключатель разбивки суток");
    else {
      global.__dcaDays();
      const all = dayCells();
      if (all.length !== 7)
        bad.push(`DCA: кнопка не развернула сутки (${all.length})`);
      if (all[0] !== "2026-09-05")
        bad.push("DCA: развёрнутая разбивка сменила порядок");
      // Накопленный итог считается ПО ВРЕМЕНИ, а не по строкам: у
      // САМОГО СТАРОГО дня (он теперь внизу) итог обязан равняться
      // деньгам этого же дня. Пересчёт сверху вниз на перевёрнутом
      // ряде дал бы другую величину под тем же именем.
      const tbl = global.__el("box").innerHTML;
      const from = tbl.indexOf("по суткам");
      const row = (tbl.slice(from).split("<tr")
        .find(x => x.includes("2026-08-30")) || "");
      const cs = row.split("<td").slice(1)
        .map(x => x.replace(/^[^>]*>/, "").replace(/<[^>]*>/g, "").trim());
      // Из ячейки берётся ПЕРВОЕ денежное число: закрывающих тегов
    ...
    if (!/USDT <span class=tag>бэктест<\/span>/.test(b2))
      bad.push("DCA: сделка бэктеста не помечена в общем списке");
    // Третьего блока быть не должно: он приглашал бы сложить группы
    if (/из него пересчёт по прошлому|из него записано вперёд/.test(b2))
      bad.push("DCA: третий блок группы остался на странице");
    // ГРУППА «без бэктеста»: числа обязаны СМЕНИТЬСЯ, а список позиций
    // — потерять строки бэктеста. Иначе таблица описывает не тот счёт,
    // что стоит над ней.
    if (global.__dcaSetGrp){
      global.__dcaSetGrp("fwd");
      const g2 = flat();
      if (g2 === b2) bad.push("DCA: переключение группы ничего не меняет");
      if (!/\+9\.70 \$/.test(g2))
        bad.push("DCA: деньги наблюдения книги $10000 не показаны");
      if (/\+412\.30 \$/.test(g2))
        bad.push("DCA: под группой «без бэктеста» показан общий счёт");
      if (!/наблюдение/.test(g2))
        bad.push("DCA: группа «без бэктеста» не подписана наблюдением");
      if (/USDT <span class=tag>бэктест<\/span>/.test(g2))
        bad.push("DCA: в списке «без бэктеста» осталась сделка бэктеста");
      // у наблюдения книги $10000 трёх дней ещё нет — прочерк с причиной
      if (!/вычитать нечего/.test(g2))
        bad.push("DCA: прочерк концентрации по дням не объяснён");
      // у безопасной книги $1000 наблюдения нет вовсе — причина словом
      if (global.__dcaSetDep) global.__dcaSetDep("1000");
      const g1 = flat();
      if (!/только после того, как его позиция/.test(g1))
        bad.push("DCA: пустое наблюдение не объяснено причиной");
      if (global.__dcaSetDep) global.__dcaSetDep("10000");
      global.__dcaSetGrp("all");
    } else bad.push("DCA: переключатель группы не выведен наружу");
    // Разворот позиции: рунги и ТВХ появляются, а свёрнутая строка их
    // не показывает
    const b3 = flat();
    if (!/ddet-/.test(b3))
      bad.push("DCA: у позиции нет разворота вовсе");
    if (!/ТВХ после/.test(b3))
      bad.push("DCA: плавающая ТВХ в развороте не показана");
    if (!/display:none/.test(b3))
  ...
  // Дизайн страницы по макетам владельца. Проверяется ЧИСЛОМ и
  // разметкой: «блок есть где-то» прошло бы и на прежнем виде.
  if (isDca && !/dcaabsent=1/.test(SEARCH)) {
    const bx = flatBox();
    // 1. Тон плитки взят из ЗНАКА ЕЁ ЖЕ ЧИСЛА. Фикстура даёт плюс
    // «стратегии» и минус «просадке депозита»: зелёная плитка с
    // красным числом обязана быть невозможна по построению.
    const tint = (label) => {
      const i = bx.indexOf("<div class=k>" + label + "</div>");
      if (i < 0) return null;
      const j = bx.lastIndexOf("<div class='st", i);
      const m = /<div class='st( pos| neg)?'/.exec(bx.slice(j, i));
      const v = /<div class='v mono (good|bad)?/.exec(bx.slice(i, i + 220));
      return {tint: m ? (m[1] || "").trim() : null,
              sign: v ? (v[1] || "") : null};
    };
    for (const [lab, want] of [["стратегия заработала", "pos"],
                               ["просадка депозита", "neg"],
                               ["открытых позиций", ""]]) {
      const t = tint(lab);
      if (!t) { bad.push("DCA: плитки «" + lab + "» нет"); continue; }
      if (t.tint !== want)
        bad.push("DCA: тон плитки «" + lab + "» не по знаку: "
                 + JSON.stringify(t) + ", ждали «" + want + "»");
      const bySign = t.sign === "good" ? "pos"
                   : (t.sign === "bad" ? "neg" : "");
      if (t.tint !== bySign)
        bad.push("DCA: тон плитки «" + lab + "» разошёлся с цветом её "
                 + "числа: " + JSON.stringify(t));
    }
    // 2. Подпись метрик считает плитки ЧИСЛОМ, а не литералом:
    // literal устарел бы при первой же добавленной плитке.
    const mcap = /метрики и статистика книги \((\d+)\)/.exec(bx);
    if (!mcap) bad.push("DCA: подписи метрик числом нет");
    else {
      const after = bx.slice(bx.indexOf(mcap[0]));
      const grid = after.slice(after.indexOf("<div class=stats>"),
                               after.indexOf("</div><div class=chart"));
      const n = (grid.match(/<div class=st>/g) || []).length;
      if (+mcap[1] !== n)
        bad.push("DCA: подпись метрик обещает " + mcap[1] + ", плиток "
                 + n);
    }
    // 3. Итог на кривой — ТО ЖЕ поле, что в главной плитке, а не
    // пересчёт по точкам: числа обязаны совпасть дословно.
    if (global.__usd) {
      const want = global.__usd(19.8);
      if (bx.indexOf("<span class='badge mono good'>" + want
                     + " итогом</span>") < 0)
        bad.push("DCA: итог кривой не совпал с плиткой: " + want);
    }
    // 4. Строка заголовка таблицы НЕ несёт класс `hd`: он занят шапкой
    // раздела и делает строку флекс-контейнером — заголовки вставали
    // столбиком (измерено в браузере). Регрессия молчаливая: страница
    // рисуется, но читать её нельзя.
    if (/<tr class=hd>/.test(bx))
      bad.push("DCA: строка заголовка таблицы взяла класс шапки `hd`");
    const thr = (bx.match(/<tr class=thr>/g) || []).length;
    if (thr !== 2)
      bad.push("DCA: строк заголовка не две (сутки и позиции): " + thr);
    // 5. Подписи ячеек для карточек телефона: без них число в карточке
    // остаётся без имени. Считаются числом — «подпись есть» прошло бы
    // и на одной.
    const dl = (bx.match(/data-l='/g) || []).length;
    if (dl < 30)
      bad.push("DCA: подписей ячеек мало для карточек телефона: " + dl);
    // 6. Нижняя панель телефона печатает ПЕРЕДАННЫЕ числа: накопленный
    // счёт совпадает с плиткой, открытое с закрытым не сложено.
    const bar = String(global.__el ? global.__el("dbar").innerHTML : "")
      .replace(/\s+/g, " ");
    if (!/накопл\. счёт/.test(bar))
      bad.push("DCA: нижней панели телефона нет");
    // Панель фильтров существует и на месте. Обратная сторона —
    // «сборщик не ответил, панель ушла вместе с содержимым» — здесь
    // НЕ проверяется: прогон страницы в этом состоянии роняет все
    // прежние проверки DCA разом, они писаны в расчёте на содержимое.
    // Гасить их ради одной новой значило бы обменять сто проверок на
    // одну, поэтому ветка названа непокрытой, а не выдана за
    // проверенную.
    const flt = global.__el ? global.__el("filters") : null;
    if (!flt || (flt.style && flt.style.display === "none"))
      bad.push("DCA: панель фильтров спрятана при живом ответе");
    if (global.__usd && bar.indexOf(global.__usd(19.8)) < 0)
      bad.push("DCA: счёт нижней панели разошёлся с плиткой: " + bar);
    // Открытых «не считали» — ПРОЧЕРК, а не ноль: свод пересобран из
    // журнала, куда открытые не идут. Проверка двусторонняя, иначе
    // починив одну сторону, потеряли бы другую.
    if (/dcanolive=1/.test(SEARCH)) {
      if (!/&mdash; \/ 40/.test(bar))
        bad.push("DCA: неизвестное число открытых в нижней панели не "
                 + "прочерком: " + bar);
      if (/>0 \/ 40/.test(bar))
        bad.push("DCA: нижняя панель показала «не считали» нулём: " + bar);
    } else if (!/2 \/ 40/.test(bar))
      bad.push("DCA: открыто/мест в нижней панели не числом: " + bar);
    if (/15\.80|23\.80/.test(bar))
      bad.push("DCA: нижняя панель сложила открытое с закрытым: " + bar);
  }
  ...
  if (isChart && /dca=/.test(SEARCH) && !/dcalong=1/.test(SEARCH)
    ...
    // Подсказка долива: ДЕНЬГИ и КОНТРАКТЫ. До правки 2026-09-05 в
    // поле `size`, подписанном долларами, ехала ДОЛЯ нотионала (0.25),
    // и точка честно печатала «0.25 $» вместо «15.00 $» — та же ошибка
    // единиц, что спред «на ногу» против цикла «на пару ног» в R4.
    const tipD = global.__el ? global.__el("tip") : null;
    const hAddD = (global.__hit ? global.__hit() : []).find(h => h.add);
    if (!global.__hover) {
      bad.push("график: функции наведения не существует");
    } else if (!hAddD) {
      bad.push("график: точка долива не попала в карту наведения");
    } else if (tipD) {
      global.__hover({clientX: (hAddD.x0 + hAddD.x1) / 2,
                      clientY: (hAddD.y0 + hAddD.y1) / 2});
      const tpd = String(tipD.innerHTML || "").replace(/\s+/g, " ");
      if (!/15\.00 \$/.test(tpd))
        bad.push("график: размер долива показан не деньгами: "
                 + tpd.slice(0, 160));
      if (!/contracts/.test(tpd) || !/0\.000232/.test(tpd))
        bad.push("график: в подсказке долива нет контрактов: "
                 + tpd.slice(0, 160));
    }
