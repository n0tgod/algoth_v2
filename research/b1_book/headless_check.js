...
(async () => {
  ...
  if (isTrades) {
    ...
    if (nb !== 8)
      bad.push(`страница сделок: кнопок книг ${nb}, а книг восемь`);
  ...
  if (isLive) {
    ...
    if (!/href="\/chart\?k=xxx&sym=ARBUSDT&arm=gbm&hour=2026-08-21-19&hz=sit"/.test(bx))
      bad.push("живое исполнение: сделка не открывается на графике");
