...
class Collector:
    ...
    def dca_trades(self, sym, book):
        ...
        jp = DR.journal_of(rk)
        # «Журнал есть» спрашивается ТЕМ ЖЕ кодом, что его читает
        # (`journal_parts`): журнал живёт суточными кусками, цельного
        # файла у семейств `h24` и `pair` нет вовсе, и проверка
        # существования одного имени объявляла бы их журнал отсутствующим.
        if not DR.journal_parts(jp):
          ...
          for rk in [x["key"] for x in fam_rulers]:
            for d in out["deposits"]:
                ...
                b["trades"] = [{
                    ...
                    # едет со сделкой — деньги по котировке обязаны быть
                    # отличимы от денег по сделкам.
                    "tail": bool(r.get("tail")),
                } for r in merged[:(None if full == k else DCA_TRADES)]]
                # Сколько закрытых ВСЕГО и сколько отдано: список есть
                # ХВОСТ журнала, и плитка «закрытых позиций» считает по
                # всему журналу. Без этих двух чисел страница печатала
                # бы «позиций 315» рядом с «закрытых 2095» и молчала о
                # том, что это разные множества (владелец заметил ровно
                # это). Урезать то, что ОТДАЁШЬ, законно; урезать молча
                # — нет.
                b["trades_shown"] = len(b["trades"])
                b["trades_total"] = len(merged)
    ...
    def factory_built(self):
        ...
        for cid, rec in sorted(state.items()):
            ...
            # Есть ли у кандидата ЖИВАЯ книга, решает ПРАВИЛО, а не
            # диск, и порядок здесь принципиален. Каталог книги
            # переживает её отставку (в нём остаётся копия манифеста),
            # и пока ответ брался с диска, страница читала такой
            # каталог как «книга заведена этим часом, чисел ещё нет» —
            # то есть обещала числа, которых не будет никогда. Это
            # ровно то, против чего заведена `live_why`: «книги ещё
            # нет» на месте «книги не положено».
            #
            # Причину называет `live_books`, а не эта страница: два
            # места, решающих одно, разошлись бы, и страница
