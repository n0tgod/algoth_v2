...
def test_league_ranks_by_realised_money():
    ...
    try:
        ...
        check("сделок в периоде две — без исхода и без книги-дубля",
              p30["n"] == 2, str(p30["n"]))
        # Деньги — те, что штампует касса: у ситуационной книги шесть
        # слотов (1000/6 = 166.67 в позиции, 49 б.п. нетто → 0.82 $),
        # у часовой один слот на одно имя × один час (1000 в позиции,
        # −51 б.п. → −5.10 $). Числа выведены из размера, а не взяты
        # из разбора — в разборе денег нет.
        arm = {g["key"]: g for g in p30["groups"]["arm"]}
        check("деньги по рукам — из кассы, по размеру позиции",
              arm["gbm"]["pnl"] == 0.82 and arm["nn"]["pnl"] == -5.1,
              str(arm))
