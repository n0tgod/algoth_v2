    ...
    try:
        for name, src, search in (
                ...
                ("график сделки ниже гейта", web.CHART,
                 "?k=xxx&sym=BTCUSDT&arm=nn&hour=2026-08-03-14"
                 "&hz=sit&rr=1.5"),
                # Книга равного риска — ситуационная: у её сделок ЕСТЬ
                # стоп и цель, их ведёт тот же сторож. Зашитый список
                # ключей на графике этого не знал, и сделки рисовались
                # голым спаном без уровней — владелец: «очень странно
                # отображаются».
                ...
                ("график длинной позиции DCA", web.CHART,
                 "?k=xxx&sym=BTCUSDT&dca=safe:10000&hour=2026-08-03-14"
                 "&dcalong=1"),
                # ОТКРЫТАЯ позиция (30 ч без выхода): у неё нет конца,
                # и окно графика схлопывалось в сутки — вход уезжал за
                # левый край, а страница говорила «запись началась
                # позже». Проверяются и свечи, и вид: подгонка
                # срабатывала только при метке конца, которой у
                # открытой нет.
                ("график открытой позиции DCA", web.CHART,
                 "?k=xxx&sym=BTCUSDT&dca=safe:10000&hour=2026-08-03-14"
                 "&dcalong=1&dcaopen=1"),
                ...
                # «общих часов 0» обязано читаться состоянием, а не
                # поломкой — ровно так пара выглядит на сервере после
                # переезда на 24 ч.
                ("лига с пустой парой", web.LEAGUE, "?k=xxx&leaguethin=1"),
                ...
                # Живое исполнение (пункт меню bot live): живые сделки
                ...
                # Бумажные DCA-книги: три депозита переключателем, одни
                ...
                ("DCA-книги с короткой книгой", web.DCAPAGE,
                 "?k=xxx&dcashort=1"),
                # Общий счёт: книга того же формата, но билет и число
                # мест — свойства СТОРОНЫ. Без этой фикстуры плитки
                # общего счёта не исполнялись headless ни разу, и
                # прочерк на них владелец увидел раньше проверки.
                ("DCA-книги с общим счётом", web.DCAPAGE,
                 "?k=xxx&dcapair=1"),
                # Суточный прогон не пришёл — страница обязана кричать.
                ("DCA-книги с устаревшим прогоном", web.DCAPAGE,
                 "?k=xxx&dcastale=1"),
                # Машина без журнала: «сделок нет» и «мы не на той
                ...
                # Расписание есть, а шаг круга молчит: с этого момента
                # тишина — отказ, и она обязана кричать, называя шаг и
                # возраст последнего успеха.
                # Сводки нет: панель обязана назвать это состоянием,
                # а не стоять пустой.
                ("автономная система без сводки владельцу",
                 web.AGENTSPAGE, "?k=xxx&agentsnosumry=1"),
                ("автономная система: круг молчит",
                 web.AGENTSPAGE, "?k=xxx&agentssched=1"),
                # Круг отработал: та же тревога обязана МОЛЧАТЬ —
                ...
                ("разбор сделки", web.TRADEINFO,
                 "?k=xxx&sym=BTCUSDT&arm=nn&hour=2026-08-03-14"
                 "&side=long&hz=sit"),
                # Решение, схлопнувшее позицию: сделки не было, и
                # карточка обязана сказать это, а не «Still open»
                # (найдено владельцем на FARTCOIN).
                ("разбор схлопнувшего решения", web.TRADEINFO,
                 "?k=xxx&sym=BTCUSDT&arm=nn&hour=2026-08-03-15"
                 "&side=short&hz=h24&infonet=1"),
                # Тот же разбор из книги равного риска: свой перечень
                # ключей на странице не знал sit_r, и значок «i» её
                # сделки открывал разбор главной книги.
                ("разбор сделки книги равного риска", web.TRADEINFO,
                 "?k=xxx&sym=BTCUSDT&arm=nn&hour=2026-08-03-14"
                 "&side=long&hz=sit_r"),
...
def test_sit_scan_anchors_forecast_to_live_price():
    ...
    row = {"sym": "AUSDT", "fwd": 30.0, "mae": -20.0, "mfe": 25.0,
           "beta": 1.0, "px": 100.0}
    # Правило v10: прогноз мельче гейта — кандидата не существует,
    # каким бы ни был ход цены. Разгон +2 % при прогнозе −1 б.п.
    # раздувал остаток и входил «ситуацией», которой модель не видела.
    # Обещания выживают переякорение (иначе кандидата отсёк бы знак,
    # а не проверяемое правило): сырые концы пути — mae вниз −418,
...
def test_sit_noise_is_median_minute_range():
    """Мера шума v12: максимум медианы целых минут и текущей минуты.

    Первая версия исключала текущую минуту как заниженную — и у
    CATSTOCK видела «шум 0» при входе внутрь минуты с фитилём в
    225 б.п.: у тонкого инструмента медиана целых минут — ноль.
    Теперь размах текущей минуты — нижняя граница меры, нулевой итог
    и нехватка минут — отсутствие меры, а не ноль.
    """
    ...
    quiet = list(pts) + [(600.0 + 5, 100.0), (600.0 + 10, 100.01)]
    c.mid = {"AUSDT": deque(quiet)}
    v = c.sit_noise("AUSDT", now + 20)
    check("тихая текущая минута — решает медиана целых",
          v is not None and abs(v - 4.0) < 0.5, str(v))
    # Взрыв в текущей минуте: её размах перекрывает медиану — мера
    # обязана вырасти до него (случай CATSTOCK).
    c3 = C.Collector.__new__(C.Collector)
    c3._noise_cache = {}
    c3.mid = {"AUSDT": deque(list(pts)
                             + [(600.0 + 5, 90.0), (600.0 + 10, 110.0)])}
    v3 = c3.sit_noise("AUSDT", now + 20)
    check("взрыв текущей минуты входит в меру",
          v3 is not None and v3 > 1500, str(v3))
    # Размах текущей минуты растёт — мера растёт в ту же минуту,
    # кеш держит только медиану целых.
    c3.mid["AUSDT"].append((600.0 + 25, 120.0))
    v3b = c3.sit_noise("AUSDT", now + 30)
    check("мера следит за текущей минутой, а не кешем",
          v3b is not None and v3b > v3, f"{v3} -> {v3b}")
    # Меньше пяти целых минут — меры нет, даже при взрыве сейчас.
    c2 = C.Collector.__new__(C.Collector)
    c2._noise_cache = {}
    c2.mid = {"AUSDT": deque(pts[:8]
                             + [(600.0 + 5, 90.0), (600.0 + 10, 110.0)])}
    check("меньше пяти минут — меры нет, а не ноль",
          c2.sit_noise("AUSDT", now + 20) is None)
    # Замороженная котировка: все точки одной ценой — меры нет.
    c4 = C.Collector.__new__(C.Collector)
    c4._noise_cache = {}
    c4.mid = {"AUSDT": deque([(k * 60.0 + 1, 100.0) for k in range(10)]
                             + [(k * 60.0 + 30, 100.0)
                                for k in range(10)])}
    check("котировка не шевелилась — это замороженный ряд, не ноль",
          c4.sit_noise("AUSDT", now + 20) is None)
...
def test_sit_scan_enters_only_on_a_crossing_it_saw():
    ...
    try:
        ...
        # Свежая позиция в памяти сторожа несёт момент открытия: по
        # нему страж свежести отличает её от старых, и путь,
        # накопленный ДО входа, к ней не применяется. Без поля
        # CATSTOCK 2026-08-13 был убит фитилём, случившимся до входа,
        # той же секундой, что открылся.
        fresh = [p for stt in books.values() for p in stt["pos"]
                 if p["sym"] == "S0USDT"]
        check("свежая позиция несёт момент открытия",
              fresh and all(p.get("at_ts") == 1010.0 for p in fresh),
              str(fresh and fresh[0]))
...
def test_league_ranks_by_realised_money():
    ...
    try:
        ...
        check("сделок в периоде две — без исхода и без книги-дубля",
              p30["n"] == 2, str(p30["n"]))
        # Деньги — те, что штампует касса ПОД ЗАБОРОМ v4 при капитале
        # 3000: потолок на имя 10 % режет слот ситуационной книги
        # (500 → 300, 49 б.п. нетто → 1.47 $); слот суточной книги —
        # 3000/24 = 125 (−51 б.п. → −0.64 $). Числа выведены из
        ...
        check("деньги по рукам — из кассы, по размеру позиции",
              arm["gbm"]["pnl"] == 1.47 and arm["nn"]["pnl"] == -0.64,
              str(arm))
...
def test_model_tree_names_every_book():
    ...
    try:
        ...
        # Деньги — те же, что у лиги: касса под забором v4 при капитале
        # 3000 даёт слоту ситуационной книги 300 $, 49 б.п. → 1.47 $.
        check("деньги ветки — из кассы",
              sit["stats"]["gbm"]["closed"] == 1
              and sit["stats"]["gbm"]["pnl"] == 1.47,
              str(sit["stats"]))
...
def test_tournament_page_reads_artifact():
    ...
    try:
        ...
        # Гейт книги поднят до 33 (зонд крайности) — референс турнира
        # переехал на ячейку e33, она в объявленной сетке с первого дня.
        check("пол измеримости и текущие правила — из турнира",
              tr["min_cell"] == 30
              and tr["current"] == "e33_rr2.0_sq_t1_a24",
...
def test_tree_scrolls_to_its_left_edge():
    ...
    Дефект, найденный владельцем: до первой книги нельзя доскроллить,
    карточка обрезана краем. Причина — `justify-content:center` на ряде
    веток, который шире контейнера: переполнение разливается В ОБЕ
    стороны, а прокручиваемой считается только правая. Померено в
    браузере на настоящем CSS страницы (окно 1280): `scrollWidth` 1209
    при нужных 1825 — 616 px дерева для полосы прокрутки не
    существовало вовсе; первая карточка при нулевой прокрутке занимала
    [-86, 32], то есть стояла левее начала; целиком показывались 8
    карточек из 10, и две не показывались ни при какой прокрутке.
    После правки: `scrollWidth` 1825, первая карточка [5, 177], левее
    начала нет ни одной.

    Раскладку харнесс не считает, поэтому здесь — правило в источнике,
    как у медиазапроса выше. Правил ДВА, и проверять их надо парой:
    ширина по содержимому чинит десктоп и одна сломала бы телефон
    (там `.node` идёт на `width:100%`, и узлы растянулись бы вслед за
    деревом). Потерять любое из двух — дефект.
    """
    import web
    check("ширина дерева — по содержимому",
          ".tree{width:max-content;" in web.TREEPAGE,
          "снято правило ширины: ряд веток снова центрируется "
          "переполнением, левый край недостижим")
    check("дерево центрируется, пока помещается",
          "margin-left:auto;margin-right:auto}" in web.TREEPAGE,
          "снято центрирование дерева")
    check("на телефоне ширина по содержимому снята",
          ".tree{width:auto;margin:0}" in web.TREEPAGE,
          "без сброса вертикальное дерево растянется по max-content")
...
def test_book_registry_is_one_list():
    ...
    # Пара в единицах σ переехала с 4 ч на 24 ч: главная книга сама
    # перешла на этот порядок, и пара на её горизонте стала бы
    # дубликатом. Каталог тот, что ПИШЕТСЯ сейчас, — иначе страница
    # показывала бы замерший архив как живую книгу.
    check("книга в единицах σ есть и там, и там",
          dirs.get("z") == "model_h24z" and "z" in books, str(dirs))
...
def test_sit_watch_levels_and_crossing():
    ...
    # Какие книги сторож вообще сторожит. Книга «выход по времени»
    # (ось `geom: timer` фабрики) уровней не имеет вовсе, и сторож,
    # закрывающий её по стопу, торговал бы не то правило, которое
    # судит её реплей. Умолчание обязано остаться прежним: флага нет
    # — уровни в силе, иначе правка молча сняла бы сторожа с книг
    # владельца.
    w = C.sit_watched([{"dir": "model_sit"},
                       {"dir": "model_c_x", "no_levels": True},
                       {"dir": "model_c_y", "no_levels": False}],
                      "/s8")
    check("книга без уровней не сторожится",
...
def test_dca_serves_ruler_and_deposit_as_one_book():
    ...
    try:
        ...
        check("DCA: общий счёт показан книгой, а не блоком в конце",
              ...
              and "показан КНИГОЙ" in page,
              "нет разметки общего счёта")
        # Минимализм (владелец 07.09): у общего счёта своей панели нет —
        # он показан КНИГОЙ, а стороны, связь и сравнение с раздельными
        # счетами читаются в отчёте. Молчание кончается на дефекте.
        check("DCA: у общего счёта нет своей панели на странице",
              "одна касса" not in page and "x.ticket_own" not in page
              and "Связь дневных денег сторон" not in page
              and "РАЗДЕЛЬНЫХ счетах" not in page,
              "панель общего счёта осталась на странице")
        check("DCA: односторонняя общая книга всё равно кричит",
              "В общем счёте нет одной" in page and "b.one_sided" in page
              and "pairBlock(b)" in page,
...
def test_dca_trades_speak_the_language_of_the_chart():
    ...
    try:
        ...
        check("DCA-график: долив отдан точкой, а не вторым спаном",
              ...
              str(t.get("adds")))
        # Долив едет в ДЕНЬГАХ и КОНТРАКТАХ, а не долей нотионала. До
        # правки 2026-09-05 полем `size`, подписанным на графике
        # долларами, уезжала доля (0.25): подсказка честно печатала
        # «0.25 $» вместо «12.50 $» и попадала под порог «денег в этот
        # час не было». Считалось верно — врал показ, ошибка единиц.
        ad = (t.get("adds") or [{}])[0]
        check("DCA-график: размер долива отдан деньгами",
              abs(float(ad.get("size") or 0) - 12.5) < 1e-9, str(ad))
        check("DCA-график: контракты долива посчитаны по его цене",
              abs(float(ad.get("qty") or 0) - 12.5 / 90.0) < 1e-12
              and abs(float(ad.get("share") or 0) - 0.25) < 1e-12, str(ad))
        # Контракты позиции целиком — последний шаг лестницы; без маржи
        # или плеча их не бывает вовсе (неизмеримое не есть ноль).
        check("DCA-график: контракты позиции равны сумме рунгов",
              abs(float(walk[-1].get("qty") or 0)
                  - (0.25 * 50 / 100.0 + 0.25 * 50 / 90.0)) < 1e-12,
              str(walk))
        nolev = C.Collector._dca_adds([[t0, 90.0, 0.25]], None)
        check("DCA-график: без нотионала деньги долива не выдуманы",
              nolev and nolev[0]["size"] is None
              and nolev[0]["qty"] is None, str(nolev))
