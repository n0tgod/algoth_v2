...
def liq_line(line):
    ...
    У Bybit `side` — сторона ИСПОЛНЕНИЯ принудительной заявки:
    ликвидация лонга продаёт (Sell), шорта — покупает (Buy)."""
    r = json.loads(line)
...
def report(art, path):
    ...
    if g["_sell_share"] is not None:
        L.append(f"\nДоля Sell среди принтов окна (ликвидации лонгов): "
                 f"{g['_sell_share']:.2f} — падение обязано сносить "
                 "лонгов, заметно меньшая доля означала бы, что окно "
                 "ловит чужие события.\n")
    L.append("\n" + art["reading"] + "\n")
