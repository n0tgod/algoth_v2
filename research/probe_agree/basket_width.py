...
# Часов сечения на руку, ниже которых мерить нечего: корзина живёт до
# 24 ч, и на истории короче трёх её жизней не закрывается ни одна —
# таблица вышла бы из нулей, а ноль читается как «денег нет», хотя
# верное чтение «не измерено». `preds.jsonl` при этом ОЧЕРЕДЬ, а не
# архив: `score_preds` убирает оценённые записи, поэтому в файле
# лежат только векторы с незакрытым форвардом.
MIN_HOURS = 3 * HOLD_H
...
def cell_row(name, c, extra=""):
    ...
    Корзина, не закрывшаяся НИ РАЗУ, не имеет реализованного вовсе:
    печатать у неё `+0.00` значило бы выдать «не измерено» за «денег
    нет» — молчаливый ноль, стоивший проекту нескольких дефектов.
    Такая строка идёт отметкой хвоста и подписана словом.
    """
    if not c:
        return f"| {name} | — | — | — | — | — |{extra}"
    if not c["baskets"]:
        return (f"| {name} | не закрылась ни разу (отметка "
                f"{fmt(c['open_mark'])}) | 0 | — | — |{extra}")
...
def write_short(path, per_arm, hours, target):
    """Отчёт-диагноз: почему замер не состоялся.

    Пустой прогон без артефакта снаружи неотличим от «не запускали»,
    а таблица нулей — от измеренного нуля. Здесь ни того, ни другого.
    """
    L = ["# Ширина согласной корзины — замер НЕ состоялся\n"]
    L.append(
        f"Прогон {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}"
        f" · цель `{target}` · сечений в `preds.jsonl` {hours} "
        f"({per_arm} часов на руку) при нужных {MIN_HOURS}\n")
    L.append(
        "**Причина не в данных рынка, а в устройстве файла: "
        "`preds.jsonl` — ОЧЕРЕДЬ на оценку живого IC, а не архив.** "
        "`score_preds` убирает записи, чей форвард уже закрылся, "
        "поэтому в файле лежат только векторы последних часов "
        "(у 24-часовой цели — примерно сутки). Истории полного "
        "сечения задним числом не существует, и состав шире "
        "записанных шести ног на прошлом построить нечем.\n")
    L.append(
        "**Чего этот отказ НЕ означает.** Он не говорит, что широкая "
        "согласная корзина плоха или хороша: она не измерена вовсе. "
        "Записанные выборы несут ровно шесть ног в час — это правило "
        "живой книги, а не ограничение замера.\n")
    L.append(
        "Купить измеримость можно только календарём: писать полное "
        "сечение 24-часовой цели в собственный журнал (как "
        "`sheets.jsonl` пишет лист сканера), и через месяц ось ширины "
        "станет считаться этим же кодом без правок.\n")
    with open(path, "w", encoding="utf-8") as f:
        f.write("\n".join(L) + "\n")
    return path
...
def main(argv=None):
    ...
    per_arm = min(len(v) for v in sections.values())
    log_(f"сечений {hours} по {len(sections)} рукам "
         f"({per_arm} на руку), пол {floor_bp:g} б.п.")
    if per_arm < MIN_HOURS:
        # Не измерено ≠ ноль: на такой истории не закроется ни одна
        # корзина, и таблица вышла бы из нулей, читаемых как «денег
        # нет». Диагноз пишется отчётом — молчаливый отказ прогона
        # неотличим от «не запускали».
        write_short(os.path.join(OUT, f"AGREE-width-{a.tag}.md"),
                    per_arm, hours, a.target)
        log_(f"истории сечения {per_arm} ч на руку при нужных "
             f"{MIN_HOURS} — preds.jsonl это ОЧЕРЕДЬ на оценку, а не "
             f"архив (score_preds убирает оценённые). Ширину на "
             f"истории мерить нечем")
        if not a.no_publish:
            PT.publish()
        return 1
