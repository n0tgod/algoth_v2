...
DIR_PREFIX = "research/mech_"


def dir_of(mid):
    """Каталог механики. Выводится МАШИНОЙ из ключа, и это не вкус.

    Контракт строителя разрешает портить только СВОИ файлы, а «свои»
    надо чем-то определить. Взять каталог из отчёта нельзя: отчёт
    пишет судимая роль, и она назвала бы своим что угодно. Взять из
    прозы задания тоже нельзя: проза приходит из заявки, то есть опять
    от модели. Ключ механики есть хеш ЗАГОЛОВКА заявки — его пишет
    предлагающий, а судят строителя, и подделать имя каталога изнутри
    постройки нечем.

    Отсюда же неудобство, названное честно: имя нечитаемое. Читаемость
    даёт очередь (`mech_queue.py` печатает ключ рядом с заголовком) и
    страница агентов, а не сам путь.
    """
    return DIR_PREFIX + str(mid or "").strip() + "/"
...
def queue(out, prop, src="propose"):
    ...
    rec = {"ev": "queued", "at": round(time.time(), 3), "id": k,
           "from": src, "title": title, "dir": dir_of(k)}
...
def state(out):
    ...
    for r in rows:
        ...
        if r.get("ev") == "queued":
            ...
            # `code` — не состояние, а отметка: код механики уехал в
            # git. Механика при этом остаётся там, где была (закрытая
            # закрытой, ждущая владельца — ждущей); затри её состояние
            # отметкой, и «ждёт владельца» исчезло бы со страницы,
            # оставив просьбу без причины.
            if r["ev"] == "code":
                seen[mid]["code"] = r.get("note") or ""
                continue
...
def task_text(rec):
    ...
    d = rec.get("dir") or dir_of(rec["id"])
    L += [
        "## Где строить",
        "",
        "Каталог твоей механики — `%s`, и он назначен МАШИНОЙ" % d,
