...
# Предел прогонов РОЛЕЙ за сутки. Ролей в круге стало четыре
# (разведчик, брифер, предлагающий, строитель); запас оставлен на
# повтор после отказа, но не на бесконечность.
MAX_ROLE_RUNS_PER_DAY = 12
...
CIRCLE = [
    ...
    ("declare", "mech", ["research/factory/declare.py"],
     "declare.json"),
    # Механика, которой движок не умеет, ждала строителя в прозе
    # `proposal.json` — а тот перезаписывается следующим прогоном.
    # Теперь она стоит в очереди, шаг `task` выдаёт задание, и
    # строитель зовётся ТОЛЬКО когда задание есть: строитель без
    # задания стоит столько же, сколько строитель с заданием.
    ("task", "mech", ["research/factory/mech_queue.py", "--next"],
     "mech_task.json"),
    ("build", "role", None, None),
]

# Шаг, у которого есть условие: без него шаг пропускается молча, но
# строкой в выводе, а не тишиной.
GATES = {"build": lambda out: _build_ready(out)}


def _build_ready(out):
    try:
        sys.path.insert(0, HERE)
        import mech_queue as MQ
        return MQ.build_ready(out)
    except Exception as e:                                # noqa: BLE001
        print("гейт строителя не сработал: %s" % e)
        return False
...
AFTER = {
    ...
    # Вход шага задания — сама очередь механик. Без этой строки шаг,
    # сказавший утром «нечего», считался бы сделанным на весь день, и
    # механика, поставленная в очередь ПОСЛЕ него (заявка приходит от
    # предлагающего, а он идёт раньше), ждала бы до завтра молча.
    # Случилось 2026-09-02: «нечего» в 20:30, механика в 22:36.
    "task": "mechanisms.jsonl",
}
...
def main(argv=None):
    ...
    for key, kind, argvv, proof in CIRCLE:
        if done_today(key, kind, proof, rows, now):
            continue
        gate = GATES.get(key)
        if gate and not gate(OUT):
            # Условие шага не выполнено — например, строителю не
            # выдано задания. Пропуск НАЗЫВАЕТСЯ: молчаливый пропуск
            # неотличим от сделанного шага.
            print(f"шаг {key}: условия нет — пропускаю, иду дальше")
            continue
