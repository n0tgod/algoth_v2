...
# Заголовок раздела, который бриф обязан нести. Строкой, а не
# структурой: бриф пишет модель человеческим текстом, и проверять в нём
# можно ровно то, что видно глазом. Берётся ИМЕНЕМ УЖЕ СУЩЕСТВУЮЩЕГО
# раздела промпта, а не новым: завести второе имя для одного и того же
# значило бы развести проверку с тем, что роль пишет.
BRIEF_OPEN_MARK = "Что открыто"
...
def check_brief(text, root, budget=BRIEF_BUDGET_CHARS,
    ...
    # Раздел открытых углов обязателен: предлагающий читает ТОЛЬКО
    # бриф, и придумать своё он может лишь из того, что в брифе видно.
    # Память полна утверждений «не измерено», «не проверялось», «вопрос
    # открыт» — без этого раздела они до него не доходят вовсе, и
    # заявка неизбежно оказывается пересказом чужого меню.
    if BRIEF_OPEN_MARK.lower() not in text.lower():
        bad.append("нет раздела «%s»: без него открытые углы проекта "
                   "до предлагающего не доходят вовсе, и заявка "
                   "неизбежно окажется пересказом чужого меню"
                   % BRIEF_OPEN_MARK)
    got = cites(text)
...
# Откуда заявка взялась. Поле обязательное и с закрытым набором
# значений: «придумал сам» и «пересказал чужое меню» — разные работы, и
# без этого различия в журнале их не отличить никогда. Проверяется
# форма, а не искренность: у собственной заявки обязано быть названо
# НАШЕ измерение, на котором она стоит, и ссылка на него вне каталога
# фабрики — журналы фабрики измерением рынка не являются.
PROPOSAL_ORIGIN = ("own", "scout", "space")
PROPOSAL_MIN_SEED = 120
FACTORY_PREFIX = "research/factory/"
...
def check_proposal(text, root, ledger_ids=(), space=None,
    ...
    origin = d.get("origin")
    if origin not in PROPOSAL_ORIGIN:
        bad.append("origin обязан быть одним из %s: «придумал сам» и "
                   "«взял из меню разведчика» — разные работы, и не "
                   "различив их, мы не узнаем, придумывает ли роль "
                   "вообще" % ", ".join(PROPOSAL_ORIGIN))
    elif origin == "own":
        seed = (d.get("seed") or "").strip()
        if len(seed) < PROPOSAL_MIN_SEED:
            bad.append("origin=own, но не названо НАШЕ измерение, из "
                       "которого заявка выросла: %d символов при "
                       "минимуме %d" % (len(seed), PROPOSAL_MIN_SEED))
    ...
    # У собственной заявки обязано быть НАШЕ измерение указателем, а не
    # только прозой: журналы фабрики (реестр, потолок, бриф) говорят о
    # ходе испытаний, а не о рынке, и заявка, стоящая на них одних,
    # выросла из процесса, а не из данных.
    if origin == "own" and not [c for c in got
                                if not c.startswith(FACTORY_PREFIX)]:
        bad.append("origin=own, а все указатели ведут в каталог "
                   "фабрики: собственная заявка обязана стоять на "
                   "нашем замере, а не на журналах процесса")
