...
Модуль называется `candidate`, а не `book`: имён `book.py` в проекте
уже два (сборщик стакана и месячная бумажная книга), и мой перекрыл
чужой на пути импорта — загрузчик баров упал на ВСЕХ символах разом
первым же живым прогоном. Тот же класс, что `nulls.py` в F3; урок был
записан, применён к `run_d1.py` и всё равно нарушен здесь.
