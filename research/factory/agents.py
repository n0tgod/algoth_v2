...
PIPELINE = [
    ...
    },
    {
        ...
        # Решение владельца: предлагающему — самая способная модель.
...
def fallback_of(key):
    """Запасная модель роли. Пусто — отката нет.

    Откат делается ТОЛЬКО на отказ по лимиту и ровно один раз: молчаливый
    перебор моделей превратил бы «роль отработала» в «отработала неизвестно
    чем», а строка прогона обязана называть, чем произведён артефакт.
    """
    st = by_key(key) or {}
    return st.get("fallback") or ""
