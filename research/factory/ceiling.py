...
* **Независимость.** Насколько дневные деньги повторяют деньги уже
  ...
  Ключ здесь несущий, а не оформление, и это показано числом, а не
  объявлено: на пуле, где четыре естественных способа написать ключ
  выбирают РАЗНЫЕ книги, взвешенный и модульный дают `pass` там, где
  знаковый даёт `closed`, — то есть объявилась бы почти точная копия уже
  живой книги. Второе условие независимости от этой ошибки НЕ спасает:
  на том же пуле эффективное `N` от копии растёт 3.00 → 4.00. Закреплено
  в `test_the_closest_link_is_chosen_by_the_signed_correlation_itself`.
