...
* **Независимость.** Насколько дневные деньги повторяют деньги уже
  ...
  выбирается по величине связи, а не по порядку в пуле и не по числу
  общих суток, и читается при этом весь пул: потолок, смотрящий на одну
  книгу из пяти, пропустил бы точную копию четвёртой.
