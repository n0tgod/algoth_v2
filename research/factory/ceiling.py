...
Потолок судит заявку СТРУКТУРНО и ровно по трём величинам.

* **Измеримость формы (с 2026-09-02).** В скольких СУТКАХ записи
  кандидат закрывал сделки. Главный критерий владельца — устойчивость,
  и правило вылета судит её медианой дня и укусом
  (`pool.shape_why` → `stability.stats`), то есть по суткам, а не по
  сделкам: залп в редкие дни даёт сколько угодно сделок и слишком мало
  суток, чтобы форму вообще посчитать. Порог выведен из двух уже
  объявленных чисел — за `pool.IDLE_D` суток (после которых молчащий
  слот и так освобождается) кандидат обязан набрать `stability.MIN_DAYS`
  суток со сделками. Судится ИЗМЕРИМОСТЬ, а не сама форма: форму на
  записи потолок не смотрит вовсе — это была бы ошибка R5.
...
* **Независимость.** Насколько дневные деньги повторяют деньги уже
  ...
  номинально. Условий здесь ДВА, и второе не порог.

  Первое — предел связи с ТЕСНЕЙШЕЙ живой книгой пула. Теснейшая
  выбирается по ЗНАКОВОЙ величине связи — не по порядку в пуле, не по
  числу общих суток, не по модулю и не со взвешиванием на длину пары, —
  и читается при этом весь пул: потолок, смотрящий на одну книгу из
  пяти, пропустил бы точную копию четвёртой.

  Ключ здесь несущий, а не оформление, и цена ошибки показана числом:
  на пуле, где естественные способы написать ключ выбирают РАЗНЫЕ книги,
  взвешенный и модульный дают `pass` там, где знаковый даёт `closed`, —
  то есть объявилась бы почти точная копия уже живой книги.

  Закреплён при этом не перечень подделок, а САМО ПРАВИЛО, и разница не
  словесная: перечнем это доказательство не сходится в принципе —
  пространство переписей ключа бесконечно, список конечен, и каждый
  прошлый заход ронялся ключом, которого в списке не было (`r·days**0.25`
  и `r·min(days,5)` проходили сюиту зелёной). Поэтому правило записано
  оракулом отдельной строкой и сверяется с реализацией на четырёхстах
  СЛУЧАЙНЫХ пулах при зерне, закреплённом числом: сутки не вложены, связи
  всех знаков, короткие книги рядом с длинными. Сверяются три величины —
  какая книга теснейшая, сколько у ЭТОЙ пары общих суток и какое `N` она
  несёт. Неэквивалентная запись ключа расходится с оракулом на каком-то
  из пулов и падает, не будучи названной заранее.

  Докуда это достаёт — ЗАМЕРЕНО подделками файла, а не объявлено, и
  универсальным свойство называть нельзя: взвешивание на длину пары
  падает вплоть до `days**0.02` (прежняя фикстура ловила только от
  `days**0.32`), а `days**0.01`, `round(r,2)` и однопроцентный нудж
  коротких пар проходят зелёной — на четырёхстах пулах у сдвига такой
  силы не находится ни одного свидетеля. Ниже 1e-4 не докажет ничего
  никакая сверка: выбор идёт по связи, округлённой до четырёх знаков
  (`links` хранит `round(r, 4)`), и переписи мельче этого сам потолок не
  различает. Закреплено в
  `test_the_key_is_fixed_by_an_oracle_on_random_pools`, где стоят и
  числа, и ворота на вырождение генератора.

  Второе условие независимости от ошибки ключа НЕ спасает, и это
  измерено там, где второе условие ЖИВОЕ: на пуле со средней связью
  +0.59 и эффективным `N` 1.38 при трёх номинальных копия узкой книги
  всё равно растит `N` 1.38 → 1.44. Что условие там не выключено,
  показано обратной стороной — другая заявка на ТОМ ЖЕ пуле роняет `N`
  1.38 → 1.31, держась при этом ниже предела связи, и закрывается именно
  ростом. Прежде это утверждение стояло здесь общим свойством, а
  показано было на пуле с зеркалом, где средняя связь подрезается нулём
  и `N_eff` тождественно равно номинальному числу книг: `after > before`
  выполнялось при любой мыслимой заявке, то есть утверждение не могло
  быть ложным. Теперь оно в
  `test_the_key_error_is_not_rescued_by_the_growth_condition`.

  Второе — **знак, а не порог: заявка, от которой эффективное `N` пула
  НЕ РАСТЁТ, закрывается.** Это не подгонка: эффективное `N` и есть
  число независимых наблюдений, и новичок, от которого оно не растёт,
  наблюдений не добавляет по определению самой величины — подкручивать
  тут нечего, сравнение идёт с нулём. Второе условие ловит то, чего
  первое не видит: заявка, смешанная из двух живых книг, держит связь с
  каждой ниже предела и при этом роняет `N` пула, потому что первое
  условие берёт максимум по парам ТОЛЬКО с заявкой, а пул усредняет
  связь по ВСЕМ парам.

  Условие роста осмысленно лишь при двух и более живых книгах, чей
  дневной ряд пул вообще считает: на пустом и на единичном пуле `N_eff`
  вырожден, и вердикт по этому условию там не выносится вовсе. Сказано
  это СЛОВАМИ — прочерк в таблице читался бы как «посчитали и ничего не
  вышло».
...
# --- измеримость ФОРМЫ (решение владельца 2026-09-02) -----------------
#
# Главный критерий владельца — устойчивость: «приносит немного, но
# стабильно, и не забирает за один день прибыль недели». Правило вылета
# судит её по СУТКАМ (`pool.shape_why` → `stability.stats`), и суток
# этих нужно не меньше `stability.MIN_DAYS`: меньше — вердикта нет
# вовсе. Значит кандидат, закрывающий сделки в редкие сутки, по форме
# НЕИЗМЕРИМ, сколько бы сделок он ни делал в те редкие дни, — а слот
# при этом занимает.
#
# Порог не назначен, а выведен из двух уже объявленных чисел, как и
# скорость сделок выше: за `pool.IDLE_D` суток простоя (после которых
# молчащий слот и так освобождается) кандидат обязан набрать
# `stability.MIN_DAYS` суток со сделками. Отсюда доля.
#
# Потолок при этом НЕ судит форму по существу — только её измеримость.
# Судить форму на записи значило бы отбирать по прошлому, которое
# ассистент видел, когда предлагал: ошибка R5, стоившая проекту месяца.
# Считается здесь ЧИСЛО СУТОК, а не деньги: величины дневного нетто в
# эту ветку не входят вовсе, и это закреплено теми же двумя тестами,
# что стерегут остальной потолок (переворот знака и умножение).
MIN_ACTIVE_SHARE = SB.MIN_DAYS / float(PL.IDLE_D)
...
def counted_by_pool(daily):
    """Считает ли пул этот дневной ряд вовсе.

    Спрошено у САМОГО `ledger.effective_n`, а не повторением его условия
    на длину ряда: копия порога разошлась бы с оригиналом молча, и
    потолок выносил бы вердикт о величине, в которую заявка не входит.
    Ряд, который пул не берёт, даёт `N = 0`; взятый — единицу.
    """
    n_eff, _r = LG.effective_n({"x": daily})
    return n_eff >= 1.0


def pool_eff_n(live, pending=None, key="pending"):
    """Эффективное `N` ПУЛА — с заявкой или без неё.

    Та же функция, которой фабрика печатает своё `N` в суточном отчёте.
    Величина здесь другая, чем у `pair_eff_n`: пул усредняет связь по
    ВСЕМ парам, а не по парам с заявкой, и ровно поэтому заявка-смесь
    двух живых книг проходит порог связи и всё-таки роняет `N`.
    """
    series = dict(live)
    if pending is not None:
        series[key] = pending
    n_eff, _r = LG.effective_n(series)
    return n_eff
...
def judge(run, min_tpd=MIN_TRADES_PER_DAY, max_corr=MAX_CORR,
          min_pair_days=MIN_PAIR_DAYS, min_active=MIN_ACTIVE_SHARE):
    ...
    base = {"id": key, "rule": rule, "days": n_days,
            ...
            "min_active_share": round(min_active, 3),
            "shape_days": SB.MIN_DAYS,
            "max_corr": max_corr}
    if n_days is None:
        return _res(UNDET,
    ...
    # Форму судят СУТКИ, а не сделки, и это отдельные ворота: книга,
    # торгующая залпом в редкие дни, даёт правилу вылета слишком мало
    # суток, чтобы медиану дня и укус вообще посчитать.
    active = len(p_daily) / float(n_days)
    base["active_share"] = round(active, 3)
    if active < min_active:
        return _res(CLOSED,
                    f"формы не измерить: сделки закрывались в "
                    f"{len(p_daily)} сутках из {n_days} записи "
                    f"({active:.2f} при требуемых {min_active:.2f}) — "
                    f"правилу вылета нужно {SB.MIN_DAYS} суток со "
                    f"сделками, чтобы посчитать медиану дня и укус, а "
                    f"при такой доле их набирается {active * PL.IDLE_D:.1f} "
                    f"за {PL.IDLE_D} суток, после которых молчащий слот и "
                    f"так освобождается", **base)
    # Измеримо ли ВТОРОЕ условие независимости — рост эффективного `N`
    # пула. Считается по книгам, чей ряд пул берёт в свой знаменатель, а
    # не по номинальному числу живых: пул, где виден один ряд, для
    # `N_eff` вырожден ровно так же, как пул из одной книги, и «рост с
    # 1.00 до 2.00» на нём означал бы только то, что заявка существует.
    pool_seen = [cid for cid in live if counted_by_pool(live[cid])]
    if len(pool_seen) < 2:
        base["pool_eff_n_why"] = (
            f"живых книг, чей дневной ряд пул вообще считает, {len(pool_seen)}"
            f" — при меньше чем двух эффективное N вырождено, и вердикт по "
            f"условию его роста не выносится вовсе")
    elif not counted_by_pool(p_daily):
        base["pool_eff_n_why"] = (
            f"пул не берёт дневной ряд самой заявки: суток в нём "
            f"{len(p_daily)}, и `ledger.effective_n` такой ряд в свой "
            f"знаменатель не считает — вердикт по условию роста N не "
            f"выносится вовсе")
    # Измеримость выводится из ОДНОГО места — того, где названа причина.
    # Держи её отдельным условием, и правка одного из двух молча оставила
    # бы вердикт без причины либо причину без вердикта.
    n_measurable = "pool_eff_n_why" not in base
    links = []
    ...
    if not links:
        return _res(PASS, f"объявлять можно: сделок {p_tr} за {n_days} "
                    ...
                    f"{min_tpd:.2f}), закрывались они в {len(p_daily)} "
                    f"сутках из {n_days} ({active:.2f} при требуемых "
                    f"{min_active:.2f}), то есть форму будет чем "
                    f"измерить; живых книг в пуле нет вовсе, "
    ...
    # же величину, которой мерит пул.
    #
    # Знак закреплён в ДВУХ местах, и каждое пришлось закреплять
    # отдельно: в СРАВНЕНИИ с порогом — проверкой на зеркале, в ВЫБОРЕ
    # книги строкой выше — проверкой на пуле с расходящимися ключами.
    # Одной проверки на зеркале не хватало: она ловит `abs()` здесь и не
    # ловит его в `max(...)`, хотя правило одно и то же.
    if best["r"] >= max_corr:
        return _res(CLOSED,
                    ...
                    **base)
    # Второе условие независимости — ЗНАК, а не порог. Подкрутить его
    # нечем: сравнение идёт с нулём, и число, с которым сравниваем, —
    # то самое, которым фабрика мерит свой знаменатель доказательства.
    grew = ""
    if n_measurable:
        before = pool_eff_n(live)
        after = pool_eff_n(live, p_daily, key)
        base["pool_eff_n"] = round(before, 3)
        base["pool_eff_n_with"] = round(after, 3)
        if after <= before:
            return _res(CLOSED,
                        f"независимости нет: эффективное N пула не растёт — "
                        f"{before:.2f} без заявки и {after:.2f} с ней при "
                        f"{len(pool_seen) + 1} номинальных испытаниях. "
                        f"Связь с теснейшей книгой при этом всего "
                        f"{best['r']:+.3f}, ниже предела {max_corr:.2f}: "
                        f"порог берёт максимум по парам только с заявкой, а "
                        f"пул усредняет связь по всем парам. Новичок, от "
                        f"которого число независимых наблюдений не растёт, "
                        f"наблюдений не добавляет по определению этой "
                        f"величины", **base)
        grew = (f", эффективное N пула растёт {before:.2f} → {after:.2f} "
                f"при {len(pool_seen) + 1} номинальных испытаниях")
    else:
        grew = f"; {base.get('pool_eff_n_why')}"
    return _res(PASS,
                f"объявлять можно: {p_tr} сделок за {n_days} суток записи "
                f"({rate:.2f} в сутки при требуемых {min_tpd:.2f}) в "
                f"{len(p_daily)} сутках из {n_days} ({active:.2f} при "
                f"требуемых {min_active:.2f} — форму будет чем измерить), "
                f"теснейшая связь {best['r']:+.3f} с `{best['id']}` при "
                f"пределе {max_corr:.2f} — пара несёт эффективное N "
                f"{base['pair_eff_n']:.2f}{grew}", **base)
...
def write_report(path, res, log=print):
    ...
    L.append(f"| доля суток со сделками | "
             f"{_num(res.get('active_share'), '{:.2f}')} |")
    L.append(f"| требуется доля суток | "
             f"{_num(res.get('min_active_share'), '{:.2f}')} |")
    L.append(f"| суток нужно правилу формы | "
             f"{_num(res.get('shape_days'), '{:d}')} |")
    ...
    L.append(f"| эффективное N пула без заявки | "
             f"{_num(res.get('pool_eff_n'), '{:.2f}')} |")
    L.append(f"| эффективное N пула с заявкой | "
             f"{_num(res.get('pool_eff_n_with'), '{:.2f}')} |")
    L.append("")
    ...
    L.append("## Растёт ли эффективное N пула\n")
    if res.get("pool_eff_n_why"):
        # Именно СЛОВАМИ, а не прочерком: вырожденное `N_eff` означает,
        # что условия не существует, а прочерк в таблице читался бы как
        # «посчитали и ничего не вышло».
        L.append("По этому условию вердикт НЕ выносился: "
                 + res["pool_eff_n_why"] + ".\n")
    elif res.get("pool_eff_n") is None:
        L.append("Расчёт до этого условия не дошёл: вердикт вынесен "
                 "раньше. Это не «N не растёт» — величина не считалась "
                 "вовсе.\n")
    else:
        L.append(f"Пул несёт эффективное N "
                 f"{_num(res.get('pool_eff_n'), '{:.2f}')} без заявки и "
                 f"{_num(res.get('pool_eff_n_with'), '{:.2f}')} с ней. "
                 f"Заявка, от которой это число не растёт, закрывается — "
                 f"и это ЗНАК, а не порог: эффективное N и есть число "
                 f"независимых наблюдений, подкручивать сравнение с нулём "
                 f"нечем. Величина не совпадает с порогом связи выше: "
                 f"порог берёт максимум по парам только с заявкой, а пул "
                 f"усредняет связь по всем парам, поэтому заявка-смесь "
                 f"двух живых книг держится ниже порога и всё-таки роняет "
                 f"N.\n")
    if res.get("links"):
    ...
    L.append("Ни нетто заявки, ни его знак, ни его величина не участвуют "
             ...
             "вердикта не меняют.\n")
    L.append("Форма (медиана дня, укус, просадка) потолком тоже НЕ "
             "судится — судится только то, будет ли её чем измерить: "
             "сколько суток записи кандидат закрывал сделки. Сама форма "
             "есть главный критерий владельца, и вердикт по ней выносит "
             "правило вылета ВПЕРЁД, по форварду со дня объявления. "
             "Судить её здесь значило бы отбирать по прошлому, которое "
             "ассистент уже видел, когда предлагал.\n")
