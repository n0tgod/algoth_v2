...
def _days(obj):
    ...
    JSON ключей-чисел не знает, и ряд, прочитанный со строковыми
    ключами, не пересечётся с рядом соседа ни одним днём — связь вышла
    бы неизмеримой на исправных данных.
    """
    out = {}
...
def judge(run, min_tpd=MIN_TRADES_PER_DAY, max_corr=MAX_CORR,
    ...
    n_days = len({d for s in live.values() for d in s} | set(p_daily))
    base = {"id": key, "rule": rule, "days": n_days,
            "trades": pend.get("trades"),
            "min_trades_per_day": round(min_tpd, 3),
            "max_corr": max_corr}
    if n_days == 0:
        return _res(UNDET, "в прогоне нет ни одних суток с записью — "
    ...
    if p_tr < need:
        return _res(CLOSED,
                    ...
                    f"{rate:.2f} в сутки при требуемых {min_tpd:.2f}; в "
                    f"окне вылета в {PL.WINDOW_D} суток это "
                    f"{rate * PL.WINDOW_D:.1f} сделки при "
                    f"{MIN_TRADES_IN_WINDOW}, то есть подбрасывание "
    ...
    if not links:
        return _res(PASS, f"объявлять можно: сделок {p_tr} за {n_days} "
                    f"суток ({rate:.2f} в сутки при требуемых "
    ...
    if best["r"] >= max_corr:
        return _res(CLOSED,
                    ...
                    **base)
    return _res(PASS,
                f"объявлять можно: {p_tr} сделок за {n_days} суток "
...
def write_report(path, res, log=print):
    ...
    if res.get("links"):
        ...
        L.append("Прочерк означает «не измерено», а не «связи нет»: "
                 ...
                 f"{MIN_PAIR_DAYS}, и пул такую пару в свой знаменатель "
                 "тоже не берёт.\n")
