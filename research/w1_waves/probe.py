...
def section_stats(pred, actual, rev, sim, acc, w, h):
    ...
    d = acc.setdefault(key, {"ic": [], "spread": [], "rev_ic": [],
                             "resid_ic": [], "sigma": [], "bands": {},
                             "n": 0})
    d["n"] += 1
    # Разброс исходов сечения — не украшение, а знаменатель порога.
    # По нему считается, КАКОЙ IC вообще способен окупить круг
    # издержек в этой ячейке; без него «IC 0.01» читается одинаково
    # на часовом и суточном горизонте, а платят они по-разному.
    d["sigma"].append(float(np.std(a)))
...
# Удвоенное среднее нормальной величины в хвосте 10 % — множитель, по
# которому спред длинно-короткого дециля выражается через IC и разброс:
# спред ≈ IC · σ · 3.51. Взят из вывода порога спеки 03, где он и был
# выведен из издержек площадки, а не из литературы.
DECILE_K = 3.51


def ic_break_even(sigma, cost_bp=None):
    """Какой IC вообще способен окупить круг издержек в этой ячейке.

    Прибыль книги равна половине спреда дециля (конвенция R4), спред
    равен `IC · σ · 3.51`, значит окупаемость требует
    `IC ≥ 2 · издержки / (σ · 3.51)`.

    Число считается ДО чтения результата и печатается рядом с
    измеренным IC. Без него «IC 0.01» читается одинаково на часовом и
    суточном горизонте, а платят они совершенно по-разному: разброс
    исходов за час и за сутки различается втрое-впятеро. Так спека 04
    отсекала ячейки, мёртвые по построению, — до прогона, а не после.
    """
    cost = (ROUND_COST_BP if cost_bp is None else cost_bp) * 1e-4
    if not np.isfinite(sigma) or sigma <= 0:
        return float("nan")
    return 2.0 * cost / (sigma * DECILE_K)
...
def write_report(path, rows, zz, meta):
    ...
    L.append("\nКолонка «IC нужен» — сколько его требуется, чтобы книга "
             f"окупила круг издержек в {ROUND_COST_BP:.0f} б.п. при "
             "измеренном в этой же ячейке разбросе исходов: спред "
             f"дециля ≈ IC · σ · {DECILE_K}, прибыль книги — половина "
             "спреда. Число считается ДО чтения результата: без него "
             "«IC 0.01» читается одинаково на часовом и суточном "
             "горизонте, а платят они по-разному.\n")
    L.append("\n| W, ч | h, ч | сечений | σ сечения, б.п. | IC | IC нужен "
             "| IC нуля | IC возврата | IC остатка | спред дециля, б.п. "
             "| книга, б.п. |")
    L.append("|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|")
    for r in rows:
        need = (f"{r['ic_need']:.4f}" if np.isfinite(r.get("ic_need", np.nan))
                else "—")
        L.append(f"| {r['W']} | {r['h']} | {r['sections']:,} | "
                 f"{r['sigma_bp']:,.0f} | "
                 f"{r['ic']:+.4f} | {need} | {r['ic_null']:+.4f} | "
