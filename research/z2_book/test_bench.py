...
def test_report_names_the_price_of_both_passes():
    ...
    try:
        ...
        # Число чистого разбора — нижняя граница, и отчёт обязан это
        # говорить: наблюдённые проходы проекта шли втрое медленнее
        # (D1: 7.5 мин на сутки при 3.0 расчётных). Без оговорки «13
        # часов» читается как обещание.
        check("чистый разбор назван нижней границей",
              "ЧИСТЫЙ разбор, нижняя граница" in txt
              and "суток счёта" in txt,
              txt[txt.find("Сколько стоит"):][:800])
