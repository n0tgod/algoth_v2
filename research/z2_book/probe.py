...
def write_report(path, cells, null, drift, meta):
    ...
    L.append("| условие | стор | гор | событий | покр | корзин | медиана | "
             "СРЕДНЕЕ | снос гор. | сверх сноса | сырая медиана | z | "
             "β-утечка | побед | сечение | доля | вердикт |\n")
    L.append("|---|:--|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|"
             "--:|--:|:--|\n")
    ...
    L.append("\n**Колонка «снос гор.» и есть цена доверия к строке.** "
             "Снос — это средний исход СЛУЧАЙНОЙ ноги той же стороны на "
             "том же горизонте, и при контроле равновзвешенной корзиной "
             "он обязан быть около нуля. Где он не около нуля, там "
             "часть превышения ячейки принадлежит не условию, а "
             "горизонту: колонка «сверх сноса» вычитает его и показывает "
             "остаток. **Ячейка, у которой среднее не перебивает "
             "собственный снос, описывает снос, а не условие** — и "
             "порога тут нет намеренно, число стоит рядом, чтобы его "
             "видели, а не чтобы им отсеивали задним числом.\n")
