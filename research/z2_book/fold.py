...
def write_report(path=None, store=None, book=None, syms=None, log=log_):
    ...
    if not st and not have:
        # Ни склада, ни сырья — значит машина не та. Отчёт не трогаем:
        # файл на диске подхватит любая следующая публикация, и
        # серверный отчёт окажется затёрт пустым (так и случилось с
        # отчётом ладдерного склада).
        log(f"ни склада, ни сырья по пути {store} — отчёт не трогаю "
            "(это песочница, а не сервер)")
        return {"path": path or os.path.join(store, "Z2-store.md"),
                "days": 0, "missing": [], "minutes": 0}
