...
def write_report(res, path=None, log=log_):
    ...
    if sn and ld:
        ...
        full = h * 25
        verdict = ("то есть около {:.0f} ч на двадцать пять суток записи "
                   "— это ночь счёта, проход выполним"
                   if full < 24 else
                   "то есть {:.1f} суток счёта на двадцать пять суток "
                   "записи — считать так всю запись нельзя")
        L.append(f"**Сплошной проход по лесенке: {h:.2f} ч на КАЖДЫЕ "
                 f"сутки записи**, "
                 + verdict.format(full if full < 24 else full / 24)
                 + ".\n\n")
