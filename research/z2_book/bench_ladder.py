...
def snaps_of_day(day, store=None):
    """Число снимков за сутки — из склада, а не из допущения."""
...
def write_report(res, path=None, log=log_):
    ...
    snaps = res.get("snaps")
    if snaps and ld:
        h = snaps * ld * 1e-6 / 3600
        L.append(f"\n### Сколько стоит проход\n\n"
                 f"Снимков за сутки записи — {snaps / 1e6:.1f} млн "
                 "(из склада, не из допущения).\n\n")
        L.append(f"**Сплошной проход по лесенке: {h:.1f} ч на КАЖДЫЕ сутки "
                 f"записи**, то есть {h * 25 / 24:.1f} суток счёта на "
                 "двадцать пять суток записи. Считать так всю запись "
                 "нельзя.\n\n")
        dens = snaps / (1440.0 * max(1, len(rows)))
        L.append("Но меры лесенки событийные: нужны секунды ВОКРУГ "
                 "события, а события дёшево находит скрин по складу. "
                 f"Точечный проход при окне {WINDOW_S} с на событие "
                 "(одно имя, обе стороны):\n\n"
                 "| событий | строк | часов |\n|--:|--:|--:|\n")
        for e in (1000, 10000, 100000):
            n = e * WINDOW_S * dens
            L.append(f"| {e:,} | {n / 1e6:.1f} млн | "
                     f"{n * ld * 1e-6 / 3600:.2f} |\n"
                     .replace(",", " "))
        L.append("\nЭто и есть довод за порядок работ: скрин по складу "
                 "сначала не из аккуратности, а потому что он же "
                 "составляет список мест, куда лесенке стоит "
                 "заглядывать.\n")
