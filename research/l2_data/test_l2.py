    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        check("окно пилота не считается готовностью",
              not OB.is_done({"start": "2025-03-01", "end": "2025-03-05",
                              "rows": 1440}, "X", days,
                             "2024-01-01", "2026-06-30"))
        check("записи без окна и без числа дней — не готово",
              not OB.is_done({"rows": 1440}, "X", days,
                             "2024-01-01", "2026-06-30"))
        # Записи прежнего формата: окна нет, но есть число дней плана.
        check("прежний формат, план совпал — готово, не пересобирать",
              OB.is_done({"days": len(days), "days_missing": 0,
                          "rows": 262522}, "X", days,
                         "2024-01-01", "2026-06-30"))
        check("прежний формат, план пилота — не готово",
              not OB.is_done({"days": 5, "days_missing": 0, "rows": 1440},
                             "X", days, "2024-01-01", "2026-06-30"))
