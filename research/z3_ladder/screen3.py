...
def build_conditions():
    ...
    Пороги выбраны до всякого прогона и по смыслу величины, а не по
    таблице: доля половины показанного, единица восполнения, тройка от
    своей нормы. Подкрутить их, увидев результат, — ошибка R5 без
    поправки.
    """
    C = []
    ...
    for s in (+1, -1):
        # Снятие: ушло, не будучи тронутым.
        add("сняли половину показанных бидов", s,
            lambda p: p["cancel_b"] >= 0.5, "снятие")
        add("сняли половину показанных асков", s,
            lambda p: p["cancel_a"] >= 0.5, "снятие")
        # Самый чистый случай: сняли, хотя почти не ели.
        add("биды растворились, их почти не ели", s,
            lambda p: (p["cancel_b"] >= 0.5) & (p["eat_b"] <= 0.1),
            "снятие")
        add("аски растворились, их почти не ели", s,
            lambda p: (p["cancel_a"] >= 0.5) & (p["eat_a"] <= 0.1),
            "снятие")
        # Смерть уровня без единого принта — этого нет ни в свечах, ни
        # в ленте вовсе.
        add("треть бидов умерла без сделок", s,
            lambda p: p["dead_b"] >= 0.3, "смерть уровня")
        add("треть асков умерла без сделок", s,
            lambda p: p["dead_a"] >= 0.3, "смерть уровня")
        # Восполнение: тот знаменатель, которого не было у ленты.
        add("выеденные биды подставляют заново", s,
            lambda p: (p["refill_b"] >= 1.0) & (p["eat_b"] >= 0.2),
            "восполнение")
        add("выеденные аски подставляют заново", s,
            lambda p: (p["refill_a"] >= 1.0) & (p["eat_a"] >= 0.2),
            "восполнение")
        add("биды едят и НЕ подставляют", s,
            lambda p: (p["refill_b"] <= 0.1) & (p["eat_b"] >= 0.2),
            "восполнение")
        add("аски едят и НЕ подставляют", s,
            lambda p: (p["refill_a"] <= 0.1) & (p["eat_a"] >= 0.2),
            "восполнение")
        # Выедание против показанного.
        add("съедена половина показанных бидов", s,
            lambda p: p["eat_b"] >= 0.5, "выедание")
        add("съедена половина показанных асков", s,
            lambda p: p["eat_a"] >= 0.5, "выедание")
        ...
        add("двигают дёшево: втрое меньше нотионала на б.п.", s,
            lambda p: p["sweep_rel"] <= 0.33, "цена хода")
        # Асимметрии сторон.
        ...
        add("едят биды, а не аски", s,
            lambda p: p["eat_skew"] <= -0.3, "перекос")
        # Показанное против своей нормы — контрольная группа: она есть
        # и в Z2, и её совпадение подтверждает, что склады описывают
        # один рынок.
