...
def burst_px(H, L, lookback=RECENT_MIN):
    """Крупнейшая минутная свеча окна — в ценах.

    `noise_px` берёт МЕДИАНУ хода свечи, а вход случается не в медианный
    момент: детектор срабатывает на разгоне, где свечи втрое-впятеро
    крупнее обычного. Стоп в 1.3 медианы оказывается внутри той самой
    свечи, которая его и снимает, — владелец увидел это на FILUSDT
    (стоп 8.3 б.п. при свечах разгона в полтора-два раза шире), а числа
    подтвердили: пересчёт дал стоп 8.3 против 7.6 у прежнего правила,
    то есть «за структуру» не сдвинуло почти ничего.

    Это тот же класс ошибки, что суточная медиана вместо получасовой,
    только на шаг глубже: величина переменная, а зафиксирована средним.
    Меру берём ту, что описывает худший ход за минуту в окне, а не
    типичный: стоп, который снимается одной обычной для этого получаса
    свечой, ограничивает не риск, а срок жизни сделки.
    """
    if H is None or L is None or len(H) == 0:
        return float("nan")
    r = np.asarray(H[-lookback:], dtype=np.float64) \
        - np.asarray(L[-lookback:], dtype=np.float64)
    r = r[np.isfinite(r) & (r > 0)]
    if len(r) == 0:
        return float("nan")
    return float(r.max())
