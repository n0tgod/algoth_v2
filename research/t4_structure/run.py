...
def report(cfg, rows):
    ...
    md.append("**R — кратность риска: итог, делённый на собственный стоп.** "
              "Проценты между монетами прямо не сравнимы — один процент "
              "для BTC и для разогнанного альта разные величины, — а R "
              "безразмерен и означает одно и то же везде. Стоп при этом "
