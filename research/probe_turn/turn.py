...
def book_trades(mdir):
    ...
    for t in tr:
        ...
        out.append({"day": int(ts // DAY), "pnl": float(t["pnl"]),
                    ...
                    "tid": t.get("tid"),
                    # моменты входа и денег нужны реплеям правил
                    # (тормоз дня действует на ВХОД, а реализованное
                    # дня набегает моментами денег) — день один их не
                    # разрешает
                    "t_in": t.get("opened_at"), "t_money": ts,
                    "arm": t.get("arm")})
    return out, mman
