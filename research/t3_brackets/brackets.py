...
def stats(trades, cost_bp):
    """Сводка по сделкам: ожидание, безубыточная доля побед и R.

    **R — кратность риска: итог сделки, делённый на её собственный
    стоп.** Мера безразмерна и потому сравнима между монетами: процент
    для BTC и для разогнанного альта — разные величины, а «плюс два R»
    означает одно и то же везде. Базисные пункты остаются рядом, потому
    что издержки площадки заданы именно в них.
    """
    if not trades:
        return None
...
def report(cfg, rows):
    ...
    md.append("")
    md.append("\n## Как читать\n")
    md.append("**R — кратность риска: итог, делённый на собственный стоп "
              "сделки.** Мера безразмерна и потому сравнима между "
              "монетами: один процент для BTC и для разогнанного альта — "
              "разные величины, а «плюс два R» означает одно и то же "
              "везде. Базисные пункты стоят рядом, потому что издержки "
              "площадки заданы в них.\n")
