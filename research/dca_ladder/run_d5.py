...
# опубликованные числа якоря (D3 `2.0|0.1`, D4 базовая книга)
ANCHOR_D3 = {"median": 0.0191, "mean": 0.0288, "liq_freq": 0.00058,
             "median_lev": 3.03, "frac_1x": 0.233}
ANCHOR_D4 = {"final": 0.0732, "max_dd": -0.1911}
...
def run(limit=None, src=None, log=print):
    ...
    # встроенная сверка якоря с опубликованными D3 и D4
    a = out["cells"][f"{ANCHOR[0]}|{ANCHOR[1]}"]
    bad = []
    for f, want in ANCHOR_D3.items():
        got = a.get(f)
        if got is None or abs(got - want) > TOL[f]:
            bad.append({"поле": f, "D3": want, "D5": got})
    for f, want in ANCHOR_D4.items():
        got = a["book"].get(f)
        if got is None or abs(got - want) > TOL[f]:
            bad.append({"поле": f, "D4": want, "D5": got})
    out["anchor"] = {"cell": f"{ANCHOR[0]}|{ANCHOR[1]}",
...
def report(s):
    ...
    if a:
        if a.get("mismatch"):
            ...
            L1 += [f"> Встроенная сверка: ячейка `{a['cell']}` "
                   "воспроизвела опубликованные D3 (`2.0|0.1`) и D4 "
                   "(базовая книга) по всем сверяемым полям — расхождений 0.",
                   ""]
