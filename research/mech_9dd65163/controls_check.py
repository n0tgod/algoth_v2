...
`test_funding_margin.py`, файл восстанавливается и сверяется по sha256.
...
правиле, ради которого написан.
    ...
    .venv/bin/python research/mech_9dd65163/controls_check.py \\
...
то есть врала бы в ту сторону, ради которой её и заводят. Байткод она же
кладёт в свой каталог на каждый прогон: подделки пишутся в один файл
подряд, и `.pyc` предыдущей врёт в обе стороны.
"""
...
SUITE = "research/mech_9dd65163/test_funding_margin.py"
...
def main():
    ...
    for i, c in enumerate(controls, 1):
        ...
        if fell:
            ...
            print(f"{i:2d}. УПАЛО НЕ ТО: обещано {want!r}; было: "
                  + " / ".join(ln for ln in txt.splitlines()
                               if ln.startswith("ПРОВАЛ"))[:200])
            bad += 1
        else:
